...
**OVERGROW-II: The Runner Is the Most Ecological Sound in the Fleet** — The `RunnerGenerator` spawns sympathetic sub-harmonics from notes played at high velocity when the aggression accumulator is elevated. A runner appears 2-8 seconds after the triggering note, at an octave or fifth below, and fades over 2 seconds. No other engine in the XOlokun fleet produces new sounds from played sounds through ecological stress simulation. The runner is not an effect — it is a consequence. Play forcefully and the soil sends up a new shoot. This is synthesis as ecosystem dynamics rather than synthesis as signal processing.
