...
ORGANISM is the only XOceanus engine whose sound is not designed — it is **computed.** Every other engine responds to parameters directly: raise the filter cutoff, hear more brightness. In ORGANISM, you specify a mathematical rule and an initial condition, and the sound emerges from the evolution. You are a biologist, not a designer.
