...
def generate_keygroup_xpm_stub(preset_name: str, engine: str,
                                wav_map: dict) -> str:
    """
    Minimal keygroup XPM stub for non-ONSET engines.
    Full implementation would mirror the keygroup exporter.
    Generates a program referencing the preset's WAV samples.
    """
    prog_name = f"XO_OX-{engine}-{preset_name}"
    # Determine root note and zone spacing from wav_map
    # Standard: 21 notes per preset at minor 3rd intervals C1-C6
    note_nums = sorted([int(k.split("__")[1]) for k in wav_map.keys()
                       if "__" in k and k.split("__")[1].isdigit()])

    if not note_nums:
        return f"<!-- Keygroup XPM stub for {preset_name} (no WAVs provided) -->"

    # Simple single-zone placeholder — full keygroup XPM would use the
    # complete keygroup exporter (xpn_export.py from XOdyssey tools)
...
<MPCVObject>
  ...
  <Program type="Keygroup">
    <Name>{prog_name}</Name>
    <!-- Full keygroup zones generated by xpn_export.py -->
    <!-- {len(note_nums)} sample zones loaded -->
  </Program>
</MPCVObject>
"""
...
def build_bundle(spec: BundleSpec, wavs_dir: Optional[Path],
    ...
    if spec.keygroup_presets:
        ...
        for p in spec.keygroup_presets:
            ...
            wav_map = {}
            if wavs_dir:
                # Look for keygroup WAVs: {slug}__NOTE__v1.WAV convention
