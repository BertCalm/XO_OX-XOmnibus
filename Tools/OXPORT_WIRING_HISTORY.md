...
- `select_presets()` was returning presets in farthest-point-sampling (diversity)
  order, not XOceanus browser order.
...
- This guarantees MIDI program change `N` loads the same preset that the XOceanus
...
python3 Tools/oxport.py build packs/mpce-perc-001.oxbuild \
    ...
    --midi-port "XOceanus" --audio-device "BlackHole"
