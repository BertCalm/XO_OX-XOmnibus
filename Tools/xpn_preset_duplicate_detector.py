#!/usr/bin/env python3
"""
XOmnibus Preset Duplicate Detector — Catch copy-paste presets across the library.
...
def load_presets(presets_dir: Path, engine_filter, mood_filter) -> list:
    ...
    for path in glob.glob(pattern, recursive=True):
        ...
        p = Path(path)
        # Derive mood from directory structure: Presets/XOmnibus/<mood>/...
        parts = p.parts
        mood_from_path = ""
        for i, part in enumerate(parts):
            if part == "XOmnibus" and i + 1 < len(parts):
...
def main():
    ...
    parser.add_argument("presets_dir", help="Path to presets root (e.g. Presets/XOmnibus)")
