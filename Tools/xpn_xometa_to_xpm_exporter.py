...
Exports XOmnibus .xometa preset files to Akai MPC Keygroup XPM programs.

This implements the params_sidecar concept in a simplified form: instead of
requiring XOmnibus to be running, it embeds preset parameters directly into
...
card inside any MPC project that references the XOmnibus AU plugin.
...
Usage (batch — whole directory):
    python xpn_xometa_to_xpm_exporter.py Presets/XOmnibus/Foundation --batch
    python xpn_xometa_to_xpm_exporter.py Presets/XOmnibus/ --batch --output-dir /tmp/xpms
...
def generate_xpm(meta: dict, source_path: Path) -> str:
    ...
    _sub(metadata_el, "Source",       "XOmnibus")
    ...
    sidecar_comment = (
        "<!-- XOmnibus Parameter Snapshot XPM\n"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=(
            "Export XOmnibus .xometa preset files to Akai MPC Keygroup XPM programs.\n"
