...
Exports XOlokun .xometa preset files to Akai MPC Keygroup XPM programs.

This implements the params_sidecar concept in a simplified form: instead of
requiring XOlokun to be running, it embeds preset parameters directly into
...
card inside any MPC project that references the XOlokun AU plugin.
...
Usage (batch — whole directory):
    python xpn_xometa_to_xpm_exporter.py Presets/XOlokun/Foundation --batch
    python xpn_xometa_to_xpm_exporter.py Presets/XOlokun/ --batch --output-dir /tmp/xpms
...
def generate_xpm(meta: dict, source_path: Path) -> str:
    ...
    _sub(metadata_el, "Source",       "XOlokun")
    ...
    sidecar_comment = (
        "<!-- XOlokun Parameter Snapshot XPM\n"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=(
            "Export XOlokun .xometa preset files to Akai MPC Keygroup XPM programs.\n"
