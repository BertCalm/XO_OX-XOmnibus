#!/usr/bin/env python3
"""
xpn_pack_qa_ci_runner.py — Single-command CI quality gate for the XOmnibus preset fleet.
...
Usage:
    ...
    python Tools/xpn_pack_qa_ci_runner.py --presets-dir Presets/XOmnibus
...
def print_banner(args: argparse.Namespace) -> None:
    ...
    print(colorize(f"\n{'='*62}", ANSI_BOLD))
    print(colorize(f"  XOmnibus Pack QA CI Runner{mode_tag}", ANSI_BOLD))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Path to Presets/XOmnibus/ (auto-detected if not set).",
    )
    p.add_argument(
        "--tools-dir",
        type=Path,
...
def auto_detect_paths(args: argparse.Namespace) -> None:
    ...
    if args.presets_dir is None:
        # Walk up from Tools/ to find repo root, then look for Presets/XOmnibus
        repo_root = args.tools_dir.parent
        candidates = [
            repo_root / "Presets" / "XOmnibus",
            repo_root / "Presets",
        ]
        for c in candidates:
            if c.is_dir():
