...
static const std::unordered_set<std::string> PARAM_ALLOWLIST = {
    "pan",  "width",      "spread",  "tune", "_pan",  "detune",    "transpose", "glide",
    "pan_", "portamento", "routing", "mode", "scale", "voicemode", "unison",
};
...
static juce::AudioBuffer<float> renderWithParam(SynthEngine& engine, juce::AudioProcessorValueTreeState& apvts,
                                                const std::string& paramId, float normalisedValue)
{
...
TEST_CASE("ParameterSweep D004 - no NaN/Inf at parameter extremes for any engine", "[sweep][d004][nan]")
{
    ...
    for (const auto& id : ids)
    {
        ...
        apvtsMin = std::make_unique<juce::AudioProcessorValueTreeState>(procMin, nullptr, "PARAMS",
                                                                        engineMin->createParameterLayout());
        apvtsMax = std::make_unique<juce::AudioProcessorValueTreeState>(procMax, nullptr, "PARAMS",
                                                                        engineMax->createParameterLayout());
...
TEST_CASE("ParameterSweep D004 - no dead (zero-delta RMS) parameters for any engine", "[sweep][d004][dead-params]")
{
    auto& registry = EngineRegistry::instance();
    auto ids = registry.getRegisteredIds();
    std::sort(ids.begin(), ids.end());
    ...
    for (const auto& id : ids)
    {
        ...
        for (const auto& pid : toTest)
        {
            ...
            else
            {
                ...
                INFO("Engine: " << id << " | Param: " << pid << " | RMS min=" << rmsMin << " max=" << rmsMax
                                << " delta=" << rmsDelta);
