...
KSResult runKS(xoceanus::FamilyDelayLine& dl, xoceanus::FamilyDampingFilter& df, float delayLen, int numSamples,
               float damping)
{
...
int main()
{
    ...
    // -----------------------------------------------------------------------
    {
        ...
        check(std::isfinite(v0) && std::isfinite(v2) && std::isfinite(v4), "DelayLine_Lagrange_finite",
              "Lagrange read returned NaN/Inf");
    }
    ...
    // -----------------------------------------------------------------------
    {
        ...
        for (int i = 0; i < 500; ++i)
            silentSum += std::abs(ae.tick(0.0f, 440.0f));
    ...
    // -----------------------------------------------------------------------
    {
        ...
        float zeroSum = 0.0f;
        for (int i = 0; i < 100; ++i)
            zeroSum += std::abs(be.tick(0.0f, 0.5f, 0.0f));
