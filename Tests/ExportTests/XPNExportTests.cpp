    ...
    for (int layers = 1; layers <= 4; ++layers)
    {
        ...
        for (auto* layer = layersElem->getFirstChildElement(); layer; layer = layer->getNextElement())
        {
            ...
            if (velEnd == 0 && activeLayerCount >= layers)
                continue;
            activeLayerCount++;
            if (velStart < minVelStart)
                minVelStart = velStart;
            if (velEnd > maxVelEnd)
                maxVelEnd = velEnd;
...
TEST_CASE("XPN Export - cancellation stops export before all presets are processed", "[export][cancel]")
{
    XOriginate exporter;
    XOriginate::BundleConfig config;
...
TEST_CASE("XPN Export - cover art engine labels resolve correctly", "[export][cover-art]")
{
    ...
    EngineCheck engines[] = {
        {"ONSET", "ONSET"},       {"OVERWORLD", "OVERWORLD"}, {"ODDFELIX", "OddfeliX"},   {"ODDOSCAR", "OddOscar"},
        {"OVERDUB", "OVERDUB"},   {"ODYSSEY", "ODYSSEY"},     {"OBLONG", "OBLONG"},       {"OBESE", "OBESE"},
        {"OPAL", "OPAL"},         {"ORGANON", "ORGANON"},     {"OUROBOROS", "OUROBOROS"}, {"OBSIDIAN", "OBSIDIAN"},
        {"OVERBITE", "OVERBITE"}, {"ORIGAMI", "ORIGAMI"},     {"ORACLE", "ORACLE"},       {"OBSCURA", "OBSCURA"},
        {"OCEANIC", "OCEANIC"},   {"OPTIC", "OPTIC"},         {"OBLIQUE", "OBLIQUE"},     {"ORBITAL", "ORBITAL"},
    };
    for (auto& e : engines)
    {
        INFO("Engine ID: " << e.id);
...
TEST_CASE("XPN Export - cover art legacy aliases resolve correctly", "[export][cover-art]")
{
    ...
    AliasCheck aliases[] = {
        {"SNAP", "OddfeliX"}, {"MORPH", "OddOscar"}, {"DUB", "OVERDUB"},
        {"DRIFT", "ODYSSEY"}, {"BOB", "OBLONG"},     {"FAT", "OBESE"},
    };
    for (auto& a : aliases)
    {
...
TEST_CASE("XPN Export - group normalization: hard layer louder than ghost", "[export][normalization]")
{
    ...
    auto readPeak = [&](const juce::File& f) -> float
    {
        ...
        if (!s)
            return 0.0f;
        auto r = std::unique_ptr<juce::AudioFormatReader>(wav.createReaderFor(s.release(), true));
        if (!r || r->lengthInSamples == 0)
            return 0.0f;
...
TEST_CASE("XPN Export - keygroup XPM has AfterTouch / ModWheel / PitchBendRange", "[export][expression][keygroup]")
{
    XOriginate exporter;
    XOriginate::BundleConfig config;
...
TEST_CASE("XPN Export - drum XPM: all pads have valid 6-digit hex PadColor", "[export][drum][pad-color]")
{
    XPNDrumExporter drumExporter;
    XPNDrumExporter::DrumExportConfig config;
...
TEST_CASE("XPN Export - drum XPM has hi-hat, kick, and snare mute groups", "[export][drum][mute-groups]")
{
    XPNDrumExporter drumExporter;
    XPNDrumExporter::DrumExportConfig config;
    ...
    for (auto* inst = instruments->getFirstChildElement(); inst; inst = inst->getNextElement())
    {
        ...
        if (mg == XPNDrumExporter::kChokeKick && name.containsIgnoreCase("KICK"))
            kickGroupFound = true;
        if (mg == XPNDrumExporter::kChokeSnare && (name.containsIgnoreCase("SNARE") || name.containsIgnoreCase("CLAP")))
