...
**XOceanus seance verdict:** High / unanimous — "The physics IS the synthesis."
...
OBSCURA is the only engine in the XOceanus fleet where the synthesis algorithm and the physical object being simulated are the same thing. When you adjust OBSCURA's stiffness, you are changing a real spring constant in a real (simulated) physical system. The sound you hear is not a metaphor for stiffness — it is what stiffness sounds like, exactly as physics dictates.
...
XOceanus standard 4-macro layout, each mapped to core physical quantities:
...
OBSCURA faces the same challenge. The solution in XOceanus is the macro layer: four macro parameters (CHARACTER, MOVEMENT, COUPLING, SPACE) each map to physically meaningful combinations of lower-level physics parameters. CHARACTER = stiffness + nonlinearity is an "acoustic hardness" control. SPACE = damping is an "acoustic liveness" control. These abstractions let players work at the level of "harder/softer" and "livelier/deader" without knowing spring constants.
