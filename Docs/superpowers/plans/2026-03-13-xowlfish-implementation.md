...
**Goal:** Build XOwlfish — a monophonic subharmonic organism synth — as a standalone JUCE plugin, then integrate it into XOceanus as engine #22.
...
This is the novel DSP — the core of the engine. No existing XOceanus DSP module does subharmonic generation.
...
Can adapt the FDN pattern from `Source/DSP/Effects/LushReverb.h` in XOceanus if it exists, but tune all defaults dark (low damping cutoff, long decay, long pre-delay).
...
**CytomicSVF:** Use from XOceanus shared DSP (`Source/DSP/CytomicSVF.h`). Copy it into `src/dsp/CytomicSVF.h`. If unavailable, implement inline — 2nd order SVF with LP/BP/HP modes, resonance, and key tracking.
...
private:
    ...
    // Coupling cache for XOceanus integration
...
## Chunk 5: XOceanus Adapter + Integration

### Task 13: Create OwlfishEngine.h — SynthEngine adapter

**Files:**
- Create: `~/Documents/GitHub/XOwlfish/src/adapter/OwlfishEngine.h`
- Copy: `~/Documents/GitHub/XOwlfish/src/adapter/SynthEngine.h` (from XOcelot or XOceanus)
...
Implements `xoceanus::SynthEngine` interface. Key differences from OcelotEngine:
...
- Guard with `#ifdef XOCEANUS_INTEGRATION` (will be removed in XOceanus copy)
...
```bash
git add src/adapter/ && git commit -m "feat: add OwlfishEngine adapter — SynthEngine interface for XOceanus"
```

### Task 14: Register OWLFISH in XOceanus

**Files:**
- Create: `~/Documents/GitHub/XO_OX-XOceanus/Source/Engines/Owlfish/` (all headers copied flat)
- Create: `~/Documents/GitHub/XO_OX-XOceanus/Source/Engines/Owlfish/OwlfishEngine.cpp`
- Modify: `~/Documents/GitHub/XO_OX-XOceanus/CMakeLists.txt`

**Reference:** How OCELOT was integrated (Task 10 from previous session).

- [ ] **Step 1: Copy all XOwlfish headers to XOceanus**

```bash
mkdir -p ~/Documents/GitHub/XO_OX-XOceanus/Source/Engines/Owlfish/
...
- `OwlfishEngine.h` (remove `#ifdef XOCEANUS_INTEGRATION` guard)
...
Note: Full XOceanus build has pre-existing errors (not caused by OWLFISH). Verify OwlfishEngine.cpp itself compiles.
...
- Copy: To `~/Documents/GitHub/XO_OX-XOceanus/Presets/XOceanus/{mood}/`
...
- [ ] **Step 2: Copy presets to XOceanus mood folders**

```bash
# Foundation presets → Presets/XOceanus/Foundation/
...
- [ ] OwlfishEngine.cpp compiles in XOceanus
