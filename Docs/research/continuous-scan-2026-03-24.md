...
- **License:** MIT
- **Relevance:** Direct enabler for XOmnibus Gallery Model's layered glow effects, engine card hover states, and the coupling arc visualizer. The aquarium site's CSS glow effects use blur heavily — this is the native JUCE equivalent.
...
- **License:** MIT
- **Relevance:** XOmnibus already uses CMake + Ninja. This template codifies CI/CD best practices that our build pipeline should mirror — particularly the notarization flow and pluginval integration for AU validation.
...
- **License:** GPL-3.0
- **Relevance:** Best-in-class reference for large-fleet JUCE UI patterns. Their skin XML system is conceptually adjacent to our Gallery Model color-per-engine approach. Study their accessibility implementation for XOmnibus V1.1+.
...
- **License:** MIT
- **Relevance:** High relevance for the iOS AUv3 UI work and any future web-facing XOmnibus companion. Our CLAUDE.md already references a JUCE + React/Typescript architecture (WebUISynth); react-juce is the more mature path. Evaluate against the `tomduncalf/WebUISynth` approach.
...
- **License:** MIT
- **Relevance:** Direct candidate for the aquarium bioluminescence particle effects and the coupling arc visualizer. Force field attractors are semantically aligned with XOmnibus coupling — particles that "flow between engines" represent signal routing visually.
...
- **Relevance:** Single most relevant Figma file for XOmnibus UI work. The ROLI-inspired continuous keyboard and XY pad directly inform our PlaySurface fretless zone. The modulation matrix panel is a reference for our Coupling Strip UI redesign.
...
- **License:** Figma Community (free)
- **Relevance:** Reference for the Coupling Performance System's "live set" view (Phase B PerformanceViewPanel). The clip-launcher grid metaphor could extend to coupling preset launching from within XOmnibus.
...
- **License:** MIT
- **Relevance:** Implementation reference for our PlaySurface fretless zone, where vertical position on a zone maps to microtonal pitch offset. The CC74 → pitch subdivision mapping is directly applicable to XOmnibus PlaySurface Zone 2 (Fretless).
...
- **License:** MIT
- **Relevance:** Hex grid controller directly relevant to the XOmnibus PlaySurface's isomorphic mode. The visual hex tessellation pattern is also aesthetically aligned with ORBWEAVE's knot topology diagrams.
...
- **License:** MIT
- **Relevance:** Direct reference for the aquarium's web controller interface. The JSON layout config maps cleanly to our APVTS parameter scheme. Could serve as a browser-based performance controller for XOmnibus standalone until a native iOS app is built.
- **Priority:** P1 — adapt for web companion controller for XOmnibus
...
- **License:** GPL-3.0
- **Relevance:** Study its XY canvas widget implementation for our coupling XY performance interface. The bi-directional OSC pattern is relevant if we add OSC support to XOmnibus standalone. Note: now hosted on Framagit, not GitHub.
...
- **License:** MIT
- **Relevance:** Motion-to-music pipeline reference. Our iOS AUv3 build could expose accelerometer/gyroscope data as macro sources (CHARACTER, MOVEMENT, COUPLING, SPACE), turning phone tilt into live performance gestures within XOmnibus.
...
- **License:** MIT
- **Relevance:** XOmnibus standalone (macOS) could expose MacBook accelerometer as a live performance gesture source. Tilting the laptop modifies COUPLING or SPACE macro. Niche but memorable as a demo/feature.
...
- **License:** Figma Community (free)
- **Relevance:** Token reference for XOmnibus dark mode toggle. Our Gallery Model is light-mode-first, but the dark mode variant needs a systematic color token approach — this file provides the reference architecture.
...
- **Relevance:** Highest quality dark mode component reference. The one-click dark mode architecture (CSS variable override per theme) is the exact approach we should take for XOmnibus dark mode toggle.
...
1. **melatonin_blur** — Integrate into XOmnibus JUCE build for engine glow rendering. Direct 10-30x blur speedup enables the accent-color glow halos on engine cards.
