...
3. **Tempo sync completeness**: Does `owit_stepSync` correctly read host BPM and apply `owit_stepDiv`? This requires host transport access in the XOmnibus context. Several engines have had issues with this in the gallery context even when it worked correctly standalone.
...
9. **Monophonic voice model justification**: `getMaxVoices() = 1` makes OUTWIT fully monophonic in XOmnibus gallery contexts. Is this artistically correct for the Giant Pacific Octopus creature identity (which hunts alone), or does it limit the engine's usability in ensemble presets?
