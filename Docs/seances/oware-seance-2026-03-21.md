...
| Architecture Originality | 9.0 | Per-mode sympathetic resonance with `lastOutput` readback cache is novel fleet-wide. Material continuum with four tables + tri-segment morph + decay exponent is the most complete material model in XOmnibus. Thermal personality grid is genuinely fresh. |
