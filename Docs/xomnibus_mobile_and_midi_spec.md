# XOmnibus — Mobile Layout, Touch Interaction & MIDI Controller Specification
...
**Related Documents:**
- `xomnibus_technical_design_system.md` — authoritative design system (Gallery Model, color, typography, components)
...
XOmnibus mobile treats the phone and tablet as first-class instruments, not compromised views of a desktop application.
...
XOmnibus supports iPadOS multitasking:
...
Any automatable parameter in XOmnibus can be bound to an external MIDI controller via MIDI Learn.
...
XOmnibus supports full MPE via both MIDI 2.0 and legacy MPE (MCM — MPE Configuration Message).

**Per-Note Expression Mapping:**

| MPE Dimension | XOmnibus Target | PlaySurface Equivalent |
|---------------|----------------|----------------------|
...
MPE controllers map 1:1 to Fretless mode. When an MPE controller is detected, XOmnibus automatically suggests switching to Fretless mode (user can decline).
...
Pre-configured MIDI mappings for common hardware. Selectable from Settings > MIDI > Controller Preset. Each preset auto-maps the controller's physical controls to XOmnibus parameters.
...
| Preset browsing | Native XOmnibus preset browser within AUv3 view (not host preset menu) |
