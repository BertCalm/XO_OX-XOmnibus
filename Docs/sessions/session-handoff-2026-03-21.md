...
All 7 utilities are committed. They live in `/Source/DSP/` under the `xoceanus` namespace.
...
### Registration Status
- Registered in `Source/XOceanusProcessor.cpp` (PASS)
...
- `Source/XOceanusProcessor.cpp` / `.h` — OWARE registration (likely committed in CLAUDE.md update)
- `CLAUDE.md` — B016 amendment applied by a linter this session
- `Presets/XOceanus/Atmosphere/Tidal_Membrane.xometa` — a single preset was modified
...
find Presets/XOceanus -name "*.xometa" | wc -l
```
