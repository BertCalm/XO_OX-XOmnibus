...
> **Trigger:** `register ORBWEAVE in XOceanus — add to XOceanusProcessor.cpp, PresetManager.h, XOceanusEditor.h, CMakeLists.txt, and CLAUDE.md`

The ORBWEAVE DSP was built by the agent but needs wiring:
- `Source/XOceanusProcessor.cpp` — add `#include` + `registerEngine("Orbweave", ...)`
- `Source/Core/PresetManager.h` — add to engine list + alias map (`"Orbweave", "weave"`)
- `Source/UI/XOceanusEditor.h` — add accent color `0xFF8E4585`
...
register ORBWEAVE in XOceanus — add to XOceanusProcessor.cpp, PresetManager.h, XOceanusEditor.h, CMakeLists.txt, and CLAUDE.md
