...
**Per-engine decision log.** Why does OVERBITE use plain param IDs (`filter_cutoff`) instead of prefixed ones (`poss_filter_cutoff`)? The answer is "342 presets already use plain names" — but this lives only in MEMORY.md as a one-line note. An XOmnibus adapter author discovering this inconsistency has no way to find the rationale without full context search. Each engine with a non-standard convention should have a `decisions.md` in its engine directory.
...
Three different counts for presets appear across MEMORY.md, the fleet health dashboard, and session sprint reports. Run one script at session start and end: count `.xometa` files in `Presets/XOmnibus/`, count seance verdict files in `Docs/seances/`, compare both against registered engine count. Output a 4-line health summary. Paste it into MEMORY.md at session close. All other counts are stale by definition.
...
| `sweep_report_2026_03_20.md` | 15,199 | 2026-03-20 | `.xometa` files under `Presets/XOmnibus/` |
