...
The OVERBITE engine accumulated 4 generations of parameter naming across its life as a standalone plugin and XOmnibus engine. The `applyPreset()` loader silently drops any unrecognized parameter ID, meaning all legacy-named params were inert — presets loaded with incorrect parameter values (defaults instead of authored values).
...
No `poss_` prefix at all. These params never loaded in XOmnibus regardless.
...
### Gen 2 — Standalone Concept Params (pre-XOmnibus integration)
...
### Gen 3 — First XOmnibus Integration (shortened names)
...
See the `applyPreset()` function in `Source/XOmnibusProcessor.cpp` lines 1440-1448 for
