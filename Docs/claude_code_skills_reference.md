...
> Curated for **XO_OX Designs / XOmnibus** — C++ audio plugin development, DSP, CMake, cross-platform builds (AU/VST3/AUv3), preset management, and multi-engine architecture.
...
3. [Recommended Skills for XOmnibus](#recommended-skills-for-xomnibus)
...
7. [Building Custom XOmnibus Skills](#building-custom-xomnibus-skills)
...
## Recommended Skills for XOmnibus
...
- **Why for XOmnibus:** Enforces discipline on a 29-engine codebase. Tests must fail before implementation. Architecture review triggers after 3 failed fix attempts. Git worktree support for parallel engine work.
...
- **Why for XOmnibus:** Directly covers the tech stack. DSP code benefits from SIMD, lock-free, and sanitizer expertise. CMake build guidance.
...
- **Why for XOmnibus:** AU/VST3/AUv3 plugin development patterns. CMake scaffold for audio plugins. Xcode/code signing workflow.
...
- **Why for XOmnibus:** Architect + QA roles for engine design review. Python scripts complement the existing Tools/ directory.
...
- **Why for XOmnibus:** Plugin code runs in DAW host processes — security matters. Curated repo is staff-reviewed for safety.
...
- **Key skill:** `skill-creator` — interactively guides you through building new skills (useful for creating custom XOmnibus skills)
...
## Building Custom XOmnibus Skills

No audio DSP skill exists in the community. XOmnibus has unique requirements that warrant custom skills. These should live in `.claude/skills/` (project-level) for cross-environment compatibility.
...
Already referenced in CLAUDE.md. Walk through ideation, architecture, scaffold, and integration. Encode the rules from `Docs/xomnibus_new_engine_process.md`.
...
*Compiled for XO_OX Designs — XOmnibus multi-engine synthesizer platform*
