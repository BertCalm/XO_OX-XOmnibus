# XOceanus Ecosystem Landscape — 2026-03-14
...
**Total ecosystem presets:** 1,625 (XOceanus Presets/) + additional standalone repo presets
...
- **INTEGRATED** — Full adapter in Source/Engines/, builds, presets in XOceanus
...
| Engine (Gallery Code) | Source Instrument | Accent | Adapter Lines | Unique Params | Preset Count (XOceanus) | Standalone Presets | LFOs | Aftertouch | Coupling In | Coupling Out | Standalone Repo | auval | Seance Score |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
**Notes on preset counts:** XOceanus count = presets in `Presets/XOceanus/` that reference the engine by canonical name. Engines with count=1 typically have one legacy/stub preset.
...
| P2-23 | ENGINES: Osprey, Osteria, Oceanic, Orbital, Organon, Oracle, Ouroboros, Origami, Obsidian | Zero presets in XOceanus Presets/ (count = 0 or 1 stub) — no factory presets ship for these engines | All ghosts | Critical for usability |
...
No engine shows CC1 (mod wheel) processing in the XOceanus adapter layer. Some standalone instruments had mod wheel; the adapters strip it.
...
| Engine | XOceanus Presets | Standalone Presets | Target | Gap | Status |
|--------|-----------------|-------------------|--------|-----|--------|
...
| OVERWORLD | 53 | 280 | 120 | -67 | Thin in XOceanus |
...
| OCEANIC | 1 | 34 | 100 | -99 | No XOceanus presets |
...
| OBSIDIAN | Native | xobsidian_design_spec.md | xoceanus_engine_roadmap_v3.md | Seance 20 | |
...
| OVERBITE (Bite) | Yes (XOppossum) | xoverbite_concept_brief.md | docs/xoceanus_integration_spec.md | Seance 13 | Note: params use plain names, NOT poss_ prefix |
...
- Final coupling matrix smoke test (all 24 engines × 15 coupling types)
- Deliverable: XOceanus v1.0 feature-complete
...
|------|------------|---------|-----------|----------|-----------------|
| XOblongBob | 15,066 | 0 (in XOceanus: 347) | Yes | Yes | Yes (per MEMORY.md) |
| XOdyssey | 6,835 | 0 (in XOceanus: 379) | Yes | Yes | Yes (per MEMORY.md) |
| XOverdub | 3,563 | 0 (in XOceanus: 188) | Yes | Yes | Yes (per MEMORY.md) |
| XOppossum (OVERBITE) | 1,004,823* | 0 (in XOceanus: 71) | Yes | Yes | Yes (per MEMORY.md) |
| XOverworld | 9,764 | 280 | Yes | Yes | Yes (per MEMORY.md) |
| XOceanic | 2,032 | 34 | Yes | Yes | — |
| XOwlfish | 2,615 | 16 | Yes | Yes | — |
| XObese | 10,332 | 0 (in XOceanus: 159) | Yes | Yes | Yes (documented in CLAUDE) |
...
| 13 | Ocelot | 6.4/10 | EcosystemMatrix novel | Dead macros; 0 presets in XOceanus |
