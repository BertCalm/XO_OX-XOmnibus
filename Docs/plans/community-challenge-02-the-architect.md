...
The 4 macros in XOceanus — CHARACTER, MOVEMENT, COUPLING, SPACE — span all loaded engines simultaneously. That's the design: one gesture, the whole system responds. Most people use them as a global tone shaper. This challenge is about using them as an architectural tool instead.
...
The macro system in XOceanus is usually treated as a global tone control. CHARACTER makes everything brighter. SPACE adds reverb to the whole patch. That's valid — but it's not what the macros are for. The macros are a composition tool. This challenge is about using them as one.
