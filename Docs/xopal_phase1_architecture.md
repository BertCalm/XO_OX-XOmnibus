...
**Gallery gap filled:** Every existing XOceanus engine synthesizes *harmonically* — oscillators, wavetables, FM operators. None synthesizes in the *time domain*. OPAL introduces a fundamentally new synthesis dimension: granular time manipulation. Freeze, stretch, scatter, and cloud density become musical parameters rather than effects.
...
The cache is written at the end of `renderBlock()` from the final stereo output (post-FX, post-finish). This is the same pattern used by all XOceanus engines.
