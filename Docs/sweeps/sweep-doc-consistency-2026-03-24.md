...
**Scope:** Post-XOlokun rename + Session 8-9 audit
**Status:** COMPLETE — all fixable issues resolved
**Files audited:** CLAUDE.md, Site/aquarium.html, Site/design-tokens.css, Docs/INDEX.md, Docs/design/asset-registry.md, Docs/design/xolokun-definitive-ui-spec.md, Docs/design/playsurface-design-spec.md, Docs/design/figma-asset-compendium.md, Docs/xolokun-morning-briefing-2026-03-24.md, Docs/research/
...
| Stale `Docs/xolokun_*.md` paths | FIXED | 7 file paths corrected to `Docs/xomnibus_*.md` (actual filenames on disk) |
| Self-referential `(formerly X)` notes | FIXED | Two instances — lines 201 and 290 — had `(formerly Docs/xolokun_...)` pointing to the same file. Cleaned to plain paths. |
| Stale `xolokun_landscape_2026.md` | FIXED | Corrected to `xomnibus_landscape_2026.md` |
...
- **XOlokun references:** Fully renamed. No stale XOmnibus in body text.

### playsurface-design-spec.md
- Uses "XOlokun" throughout — PASS.
- No engine name issues found.
- No stale file paths found.

### asset-registry.md
- **Stale `XO_OX-XOlokun` repo paths:** FIXED — 10 occurrences replaced with `XO_OX-XOmnibus`.
...
- **Mockup path:** `Docs/mockups/xolokun-main-ui.html` → FIXED to `Docs/mockups/xomnibus-main-ui.html` (actual filename on disk).
...
## Check 3: Stale XOmnibus References in Docs

**Command run:** `grep -rn "XOmnibus" Docs/ --include="*.md" | grep -v "formerly|née|Formerly|renamed|historical|changelog|Presets/XOmnibus|SDK/include/xomnibus|Rename|rename|xomnibus_name_migration|formerly named|xomnibus_"`

**Count before fix:** 6
**Count after fix:** Reduced to informational-only records

Remaining legitimate references:
1. `Docs/xolokun-morning-briefing-2026-03-24.md` line 33 — FIXED (`Presets/XOmnibus/` → `Presets/XOlokun/`)
2. `Docs/architect-audit-session-9.md` — Multiple lines reporting "XOmnibus references: 0 — CLEAN" in code/source files. These are historical audit records. NOT stale — they are *reporting absence* of XOmnibus in source code, which is correct.
...
| Docs/design/xolokun-definitive-ui-spec.md | 73 (multiple mentions) | PASS |
...
### Command run:
```bash
grep -rn "Presets/XOmnibus|SDK/include/xomnibus" Docs/ --include="*.md"
```

**Results:**
- `Docs/xolokun-morning-briefing-2026-03-24.md:33` — `Presets/XOmnibus/Atmosphere/...` — FIXED
...
**SDK paths:** No stale `SDK/include/xomnibus` found. CLAUDE.md correctly uses `SDK/include/xolokun/`. The SDK directory at `~/Documents/GitHub/XO_OX-XOmnibus/SDK/include/xolokun/` is the canonical path.

### xolokun_name_migration_reference.md (CLAUDE.md Key Files)
File does not exist as `xolokun_name_migration_reference.md`. Actual file: `Docs/xomnibus_name_migration_reference.md`. **FIXED** in CLAUDE.md and INDEX.md.

### how_to_write_a_xolokun_adapter.md (INDEX.md)
File does not exist at that name. Actual file: `Docs/how_to_write_a_xomnibus_adapter.md`. **FIXED** in INDEX.md.
...
| File | Fix | Lines |
|------|-----|-------|
| `CLAUDE.md` | 7 `Docs/xolokun_*.md` paths → `Docs/xomnibus_*.md` | 201, 208, 225, 290, 331, 466, 471, 480, 487, 493 |
| `CLAUDE.md` | Removed two self-referential `(formerly xolokun_...)` parentheticals | 201, 290 |
| `Site/design-tokens.css` | `--xo-motto: "Omnibus — For All"` → `"XOlokun — For All"` | 756 |
| `Site/aquarium.html` | Added missing OUTLOOK engine card (Preview) | After line 1287 |
| `Docs/INDEX.md` | 9 `xolokun_*.md` links → `xomnibus_*.md` actual filenames | 8, 10-13, 16-21 |
| `Docs/INDEX.md` | `how_to_write_a_xolokun_adapter.md` → `how_to_write_a_xomnibus_adapter.md` | 21 |
| `Docs/design/asset-registry.md` | 10 `XO_OX-XOlokun` repo paths → `XO_OX-XOmnibus` | 68, 69, 77, 78, 79, 544, 545, 593, 642, 853 |
...
| `Docs/design/asset-registry.md` | Mockup path `xolokun-main-ui.html` → `xomnibus-main-ui.html` | 593 |
| `Docs/xolokun-morning-briefing-2026-03-24.md` | `Presets/XOmnibus/` → `Presets/XOlokun/` | 33 |
...
| `Docs/design/xomnibus_design_guidelines.md` line 836 still references `XO_OX-XOlokun repo` | Historical document compiled 2026-03-17, prior to rename. The sentence is self-dated. Low priority. |
| ~78 `XO_OX-XOlokun` references in older spec docs (`Docs/specs/`, `Docs/ebook/`, etc.) | These are historical research and planning documents from before the rename. They do not affect runtime behavior. Would require a broad sweep of ~20 files. Flagged for future Historical Society session. |
...
- `CLAUDE.md` stale `Docs/xolokun_` paths: **0** (was 7)
- `asset-registry.md` stale `XO_OX-XOlokun` paths: **0** (was 10)
- `design-tokens.css` `--xo-motto`: now says `"XOlokun — For All"`
