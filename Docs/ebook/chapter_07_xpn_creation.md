...
- You have rendered audio from XOmnibus and want to package it for MPC use
...
Oxport is a suite of Python tools living in `Tools/` at the root of the XOmnibus repository. The main entry point is:
...
**Stage 3: DNA Badge Assignment.** Each program receives a 6D Sonic DNA badge computed from the engine parameters or, for non-XOmnibus samples, inferred from acoustic analysis. You can override computed values with explicit values in the config. This stage writes a `.xometa` companion file for each program — this file follows the program into the manifest and is used by the pack browser for filtering.
