# XOceanus Coupling Audit — Round 4 (Prism Sweep)
**Date:** 2026-03-14
...
#### OCELOT (XOcelot)
- **`applyCouplingInput` status:** STUB — all 4 parameters cast to void. Comment says "Full routing wired when running inside XOceanus" but nothing is wired.
...
Both Ocelot and Owlfish share the same structural reason for their stubs: they were integrated into XOceanus from standalone instruments with their own internal modulation architectures (`EcosystemMatrix` / `OwlfishVoice + SubharmonicOsc` respectively). The coupling wiring was deferred with explicit "to be wired later" comments. Unlike earlier-integrated engines (Bob, Fat, Drift) which have basic coupling, these two newer integrations never received their Round 1 coupling pass.
