...
standalone XOdyssey schema to the XOmnibus `drift_` prefix schema. During migration, 1,353
...
were authored natively against the XOmnibus schema.
...
the XOmnibus MasterFXChain after all engines have contributed to the mix.
...
`reverb_size=0.8, reverb_mix=0.4` becomes a dry, close-mic sound in XOmnibus. A preset with
...
XOmnibus level as the CHARACTER/MOVEMENT/COUPLING/SPACE macros, but the per-preset macro
...
### Option C — XOmnibus Routing (Route through shared MasterFXChain)
...
by freshly-authored XOmnibus-native presets. If that is the intent, document it explicitly and
...
| C XOmnibus routing | Zero | None | None | Not viable |
...
```
Voice pool → buffer.addSample() → (XOmnibus MasterFXChain)
```
