...
standalone XOdyssey schema to the XOlokun `drift_` prefix schema. During migration, 1,353
...
were authored natively against the XOlokun schema.
...
the XOlokun MasterFXChain after all engines have contributed to the mix.
...
`reverb_size=0.8, reverb_mix=0.4` becomes a dry, close-mic sound in XOlokun. A preset with
...
XOlokun level as the CHARACTER/MOVEMENT/COUPLING/SPACE macros, but the per-preset macro
...
### Option C — XOlokun Routing (Route through shared MasterFXChain)
...
by freshly-authored XOlokun-native presets. If that is the intent, document it explicitly and
...
| C XOlokun routing | Zero | None | None | Not viable |
...
```
Voice pool → buffer.addSample() → (XOlokun MasterFXChain)
```
