...
they are authored by the XOceanus team and are guaranteed acyclic. The `isNormalled` flag can
...
in use across the XOceanus fleet.
...
`route.sourceSlot` identifies which engine slot in the XOceanus grid is the source. It is also
