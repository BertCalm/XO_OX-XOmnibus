...
**Mood Map** (full browser): 2D scatter plot of all presets. X/Y = two user-selected DNA axes (default Brightness × Warmth; any pair of Brightness / Warmth / Movement / Density / Space / Aggression). Each dot colored by mood. Click to load. Zoom and pan. Mood territory labels overlaid. This is an "explore by mood" map, not a PCA/t-SNE projection.
