...
**Scope**: Three play surfaces + expression controllers for XOmnibus (AU, Standalone, iPad)
...
The XOuija maps to MPC keygroup programs. When an XOmnibus XPM program is loaded on MPC, the XOuija's X-axis pitch range corresponds to the keygroup's zone layout. The Planchette shows which MPC zone the current pitch falls into:
...
A 4x4 velocity-sensitive pad grid that is 1:1 compatible with MPC pad layouts. This is not a decorative representation — it IS the MPC pad bank rendered in the XOmnibus visual language. Pad numbering, velocity curves, and note mapping match MPC conventions exactly.
...
**Velocity curves must match MPC exactly.** The MPC's logarithmic curve (called "Curve 2" in MPC software) uses `v = 127 * pow(input/127, 0.6)`. Our "Logarithmic" curve must produce identical output or drum programs will feel different when played on XOmnibus vs. MPC hardware.

**MPCe quad-corner integration:**
On MPC Live III / MPC Key 61, each pad has four corners. XOmnibus maps this to:
...
Pressing a pad corner on MPCe sends the note at full velocity PLUS a CC value for the corresponding macro. XOmnibus receives both and routes accordingly. The visual pad in XOmnibus shows the quad-corner zones as subtle diagonal lines when in "MPCe mode":
...
**30-second test (Kai):**
Load an MPC XPM drum program into XOmnibus. Play the 16-pad grid. Every pad should trigger the correct sample at the correct velocity response. Switch to Bank B. Pads should update. If a beatmaker can load their MPC kit and play it without reading documentation, the implementation is correct.
...
When an external MIDI keyboard sends notes to XOmnibus:
...
- The user can play the on-screen keyboard AND the external keyboard simultaneously — XOmnibus merges both streams
...
A novel expression controller unique to XOmnibus. A circular controller showing a simulated water surface that responds to touch:
...
XOmnibus running as a plugin on MPC hardware:
...
- The MPC's XY pad (if present on MPC Key 61) maps to the XOmnibus XY Pad
...
**VISIONARY**: The play surface is not a UI component. It is a *second instrument* that lives inside XOmnibus. The engine is one instrument (the sound generator). The play surface is another (the performance interface). When you couple them, you get a third thing that neither could produce alone.

Think about it: a Stradivarius and a bow are two separate instruments. Neither makes sound alone. The musician's body is a third instrument. XOmnibus has the engine (Strad), the play surface (bow), and the performer (body). We are designing the bow.
...
| Phase 5 | Mount into `XOmnibusEditor`, wire MIDI output | 1 day | Phase 1-4 |
