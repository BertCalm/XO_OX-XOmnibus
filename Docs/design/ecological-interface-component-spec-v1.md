...
Engine accent colors map directly from `GalleryColors::accentForEngine()` in `XOceanusEditor.h`. All 47 values are present there. The ecological arc color = engine accent color throughout this spec.
...
### Overview
The porthole container. Each of the 86 engines lives in one of up to 8 active engine slots. The slot is styled as a porthole — you are looking through rounded glass at the creature's habitat. Engine accent color anchors the header bar and all knob arcs within the slot.
