...
| **Glitch as mod source** | The glitch event itself becomes a modulation source in XOceanus's coupling matrix — `GlitchToFilter`, `GlitchToMorph`, `GlitchToPitch`. |
...
The theme only changes the XOverworld panel's accent colors within the gallery frame. The XOceanus shell (warm white walls, XO Gold macros) stays untouched — the gallery walls don't repaint for each exhibition, but the art inside changes.
...
- **Gallery rule:** The theme ONLY affects the XOverworld engine panel. The XOceanus shell, other engine panels, coupling strip, and preset browser are unaffected. Each engine is its own exhibition — XOverworld's exhibition just has multiple rooms.
...
**Flux mood impact:** XOverworld alone contributes 30 Flux presets — nearly matching the entire existing XOceanus Flux count (36). This engine fills the gallery's weakest category.
