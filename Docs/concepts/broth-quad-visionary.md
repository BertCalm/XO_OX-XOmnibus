...
This state is not reset between notes. It is not reset between phrases. It accumulates across the entire session. When you close XOceanus, the ReductionState is saved to disk. When you reopen the project, XOverworn is exactly as reduced as when you left it.
...
The BROTH quad is not six pads for ambient musicians. It is a transformation environment that any engine in XOceanus can be cooked inside.
