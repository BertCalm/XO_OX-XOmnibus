...
- **Unique capability:** A fluid energy model where sea state IS the core synthesis parameter. Calm = coherent, shimmering folk resonances. Storm = decorrelated chaos, spectral spray, and roaring turbulence. No other engine models an environment as the instrument — every other engine is a sound source *in* a space; XOsprey makes the space itself the source.
