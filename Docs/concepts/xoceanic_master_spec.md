...
In XOmnibus, any engine's audio can enter the chromatophore pedalboard. OVERWORLD's chip audio scattered through SCATTER. ODYSSEY's Climax frozen by FREEZE. OVERBITE's bass warped by TIDE. The pedalboard reveals hidden colors in *any* engine — this is OCEANIC's gallery role.
...
| M3 | COUPLING | `couplingLevel` ↑, `freezeMix` context, `scatterMix` context | linear | **The portal.** In XOmnibus: scales coupling input depth. In standalone: could control internal feedback routing. |
...
## Section 15: Coupling Interface (XOmnibus Integration)
...
| getMaxVoices() return | 1 (for XOmnibus CPU budgeting — it's one paraphonic "voice") |
