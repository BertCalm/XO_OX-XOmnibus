# Macro Audit — XOmnibus 8-Engine Survey (2026-03-14)
...
**Problem:** The OverworldEngine adapter had zero macro parameters. The XOverworld standalone binary defines all its parameters via `xoverworld::addParameters()` but that function never included XOmnibus macros. From an XOmnibus performance perspective, turning the 4 macro knobs on an OVERWORLD slot produced zero audio change.

**Solution:** 4 new parameters declared directly in `addParameters()` of the XOmnibus adapter (after the `xoverworld::addParameters()` delegate call), attached in `attachParameters()`, and applied at the top of `renderBlock()` before FX units are configured:
