...
orchestrator, and the `XPNExporter.h` C++ export module inside XOmnibus.
...
The actual audio render requires a manual step through XOmnibus standalone or a DAW.
...
The XOmnibus plugin itself is already open source (the CLAUDE.md describes it as
...
**`renderNoteToWav()` has real competitive value once built.** A working headless XOmnibus
...
The Hold-tier tools remain in the XOmnibus monorepo.
