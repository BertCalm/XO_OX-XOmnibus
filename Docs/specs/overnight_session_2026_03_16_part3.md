...
| Tool | Purpose |
|------|---------|
| `xpn_coupling_recipes.py` | Generates XPN coupling-aware keygroup programs where sample selection follows XOmnibus coupling route logic |
...
10 utility engines have concepts but no source code (from `utility_engine_concepts.md` and related docs). These are not registered in XOmnibus yet. Exact list in `utility_engine_concepts.md` and `utility_engine_rapper_bundle.md`.
