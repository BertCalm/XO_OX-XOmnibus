...
Recipes solve a real problem. XOceanus has 34 engines and 15 coupling types. The combinatorial space is vast. Most users will never explore OBLONG + OUROBOROS with Audio->Ring coupling on their own. Recipes collapse that space into named, curated starting points that say: "try this combination — we already wired the interesting connections."
...
When a recipe is active, a small badge appears in the XOceanus header bar:
...
4. **XOceanusProcessor** — Add `RecipeManager` as a member. Wire `applyRecipe()` to the message thread handler that currently manages engine slot changes.
...
|------|------|--------|----------|
| Factory | `Presets/XOceanus/{mood}/` | Ships with app | No |
| User | `~/XOceanus/Presets/User/` | Saved by user | Yes |
| Community | `~/XOceanus/Presets/Community/` | Downloaded/shared | No (read-only locally) |
...
Community recipes follow the same pattern: `~/XOceanus/Recipes/User/` and `~/XOceanus/Recipes/Community/`.
