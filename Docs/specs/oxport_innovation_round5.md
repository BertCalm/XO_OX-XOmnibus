...
**Authors:** Rex (XPN format android) + Atlas (XOlokun bridge android)
**Date:** 2026-03-16
...
This means pressing harder on a pad after the initial strike opens the filter — the same behavior that the XOlokun engine implements via D006. The sample pack now behaves consistently with the plugin.
...
| `coupling_pairs` | Recommended engine combos with coupling type — direct link to XOlokun coupling setup |
...
- Live synthesis substitute: an MPC-only performer gets timbral evolution without owning XOlokun
...
Render each sample in XOlokun with these exact macro settings:
