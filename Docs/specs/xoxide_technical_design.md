...
XOxide is a continuous 2D character shaper. It is XOceanus's first utility engine that processes
...
1. `renderBlock()` must have an audio input path. XOceanus's architecture passes the mixed
...
In `XOceanusEditor.h`, add case to `prefixForEngine()`:
```cpp
...
### Phase 4 — XOceanus Integration
13. `REGISTER_ENGINE(OxideEngine)` in EngineRegistry.h
14. Add `"oxide_"` case to `prefixForEngine()` in XOceanusEditor.h
