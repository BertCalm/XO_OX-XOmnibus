...
**Scope:** `VoiceAllocator.h`, `MegaCouplingMatrix.h`, `XOceanusProcessor.cpp/h`, `SynthEngine.h`
...
`processAudioRoute` (lines 285–313 in MegaCouplingMatrix.h) calls `getSampleForCoupling` inside the same block where `renderBlock` also ran. Because `renderBlock` runs first (lines 1183–1218 in XOceanusProcessor.cpp), the cache is already populated before coupling reads it. Voice stealing happens *inside* `renderBlock`, not between the two calls.
...
**File:** `Source/XOceanusProcessor.h`
...
```cpp
// Called by XOceanusProcessor::loadEngine() after an engine swap.
...
**File:** `Source/XOceanusProcessor.cpp`, in `loadEngine()`, after the atomic swap:
...
**File:** `Source/XOceanusProcessor.cpp`, in `loadEngine()`:
...
| # | File | Change | Type |
|---|------|--------|------|
| 1 | `XOceanusProcessor.h` | Add `std::mutex crossfadeMutex` to private section | Bug fix — data race |
| 2 | `XOceanusProcessor.cpp` | Wrap crossfade mutations in `loadEngine`/`unloadEngine` with `scoped_lock` | Bug fix — data race |
| 3 | `XOceanusProcessor.cpp` | Wrap crossfade reads in `processBlock` with `scoped_lock` | Bug fix — data race |
| 4 | `MegaCouplingMatrix.h` | Add `notifyCouplingMatrixOfSwap(slot, newEngineId)` | Feature — route integrity |
| 5 | `XOceanusProcessor.cpp` | Call `notifyCouplingMatrixOfSwap` after atomic swap in `loadEngine` | Feature — route integrity |
| 6 | `VoiceAllocator.h` | Add `findFreeVoiceCouplingAware()` variant | Forward-compat only |
| 7 | `XOceanusProcessor.cpp` | Add `newEngine->wakeSilenceGate()` after prepare in `loadEngine` | Bug fix — mid-note swap |
