# JUCE to MPC Workflow: XOceanus to XPN Pack

**Status:** Internal reference guide | March 2026
**Scope:** Complete pipeline from sound design in XOceanus to production-ready XPN pack on MPC pads

---

## Overview

This is the end-to-end process for turning an XOceanus preset into a deployable MPC expansion pack.
...
## Phase 1: Sound Design in XOceanus
...
XOceanus presets expose 4 macros: CHARACTER, MOVEMENT, COUPLING, SPACE. These do not survive
...
requires loading XOceanus as a plugin, playing each note, naming files manually. For a 20-preset
pack at 4-layer velocity × 13 notes per octave, that is 1,040 individual recordings.

**Standalone rendering:** Load XOceanus standalone, use system audio recording (e.g., BlackHole or
...
3. **`xpn_coupling_docs_generator.py`** — Writes CouplingDNA comment blocks into the XPM files.
   ...
   XOceanus coupling configurations were active when the samples were rendered. Preserves the
