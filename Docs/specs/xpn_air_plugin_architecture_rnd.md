...
└── plugin_bundle/
    ├── manifest.json
    ├── AU/
    │   └── XOmnibus.component      (macOS AU)
    └── VST3/
        └── XOmnibus.vst3/          (cross-platform VST3)
```
...
## 3. XOmnibus-as-XPN-Plugin Path

XOmnibus is the natural companion plugin for XO_OX packs. The user flow would be:

1. User downloads XO_OX expansion (e.g., XObese Character Pack).
2. MPC Software detects XOmnibus is not installed.
3. Prompts: "This expansion was designed for XOmnibus. Install the free companion plugin?"
4. XOmnibus installs to AU/VST3 locations.
5. Expansion Programs reference XOmnibus engines directly via plugin parameter blocks.
...
**Near-term feasible path**: Ship XOmnibus as a standalone free download with a landing page linked from pack metadata. No firmware changes needed. The `params_sidecar` approach (Section 4) handles the preset bridge.
...
XOmnibus would expose a **Sidecar Import** feature: scan a directory for `params_sidecar.json` files, parse them, and load the named preset into the matching engine. This is entirely within XOmnibus's control — no MPC firmware changes required.

The sidecar functions as a "preset import protocol": the XPN pack ships sounds, the sidecar ships the engine voice character, and a user with XOmnibus installed gets both layers automatically.
...
| XOmnibus sidecar importer (scan + load) | Sonnet | Yes | None |
| XOmnibus notarization + signed distribution | Sonnet | Yes (process) | Apple Dev account |
...
2. Build XOmnibus sidecar importer as a V1.1 feature.
3. Pursue Apple notarization so XOmnibus can distribute as a signed free download linked from pack pages.
