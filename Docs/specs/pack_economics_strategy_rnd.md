...
**Rationale**: 1-year mark from the XOceanus 31-engine milestone (March 2026). The Anthology collects the best 4-6 kits per engine across all 31+ engines. A prestige product that demonstrates the full fleet. March timing echoes the Pi Day 2026 Theorem announcements — brand resonance across years.
...
- $10/mo: Early access + source preset files (.xometa bundles for XOceanus)
...
No new architecture is required. The offline render infrastructure already exists in the XOceanus codebase. This is wiring, not invention.
...
XPM 3 non-negotiable rules (per XOceanus CLAUDE.md):
- `KeyTrack` = `True`
- `RootNote` = `0`
