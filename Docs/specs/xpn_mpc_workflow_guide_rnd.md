...
Some XO_OX packs include **Entangled** programs — presets that use the COUPLING macro to route one engine's output as a modulation source into another. These require **XOmnibus** running as a plugin.

**Standalone MPC (no DAW)**
XOmnibus cannot run natively on MPC standalone. Entangled presets in standalone mode will function as standard programs — the COUPLING macro affects internal modulation only, not engine-to-engine routing.

**Plugin mode (MPC connected to DAW)**
1. Open MPC Software in **Plugin** mode inside your DAW (Ableton, Logic, etc.).
2. Load XOmnibus on a separate instrument track. Set its MIDI input to the MPC's virtual MIDI output port.
3. On the MPC pad bank, assign the Entangled program pads to send MIDI to that port.
4. Route XOmnibus audio output back into the DAW mixer or directly into the MPC via the audio input if using hardware loopback.
...
**MIDI file export and DAW reimport with XOmnibus**
In MPC, go to File → Export → MIDI. Select the sequence or pattern. In your DAW, import the MIDI file and route it to XOmnibus on an instrument track. The MIDI note layout from XO_OX drum programs maps directly to XOmnibus's drum engine pad layout — no remapping needed.
...
4. Load XOmnibus on a parallel instrument track for live plugin layering over the rendered stems
5. Automate XOmnibus Q-Links (CHARACTER/MOVEMENT/COUPLING/SPACE) via DAW automation lanes for final mix movement
