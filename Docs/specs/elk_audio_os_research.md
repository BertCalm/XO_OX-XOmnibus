...
Run XOlokun as a standalone hardware instrument on Raspberry Pi 5,
...
- XOlokun DSP is all inline .h headers — portable C++ with no platform deps
...
- XOlokun on ARM: not tested, but DSP layer has no x86 dependencies
- JUCE on Elk: documented workflow exists (JUCE cross-compile for ARM Linux)

## Open Questions

1. Can XOlokun's full 4-engine architecture fit in Pi 5's thermal envelope?
...
1. Cross-compile XOlokun DSP layer for ARM64 Linux (test build only)
