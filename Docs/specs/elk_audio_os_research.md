...
Run XOmnibus as a standalone hardware instrument on Raspberry Pi 5,
...
- XOmnibus DSP is all inline .h headers — portable C++ with no platform deps
...
- XOmnibus on ARM: not tested, but DSP layer has no x86 dependencies
- JUCE on Elk: documented workflow exists (JUCE cross-compile for ARM Linux)

## Open Questions

1. Can XOmnibus's full 4-engine architecture fit in Pi 5's thermal envelope?
...
1. Cross-compile XOmnibus DSP layer for ARM64 Linux (test build only)
