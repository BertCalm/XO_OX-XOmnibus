...
<MPCVObject>
  <Version value="2.10"/>
  <Program type="VST3Plugin" name="XOceanus - OPAL Cephalopod Drift">
...
1. **VST3 state serialization**: Oxport needs a pathway to read an XOceanus `.xometa` preset, serialize it as a VST3 state chunk (base64), and embed it in the VST3 program XPM.
2. **PluginUID registry**: XO_OX needs a registered VST3 UID that MPC 3.5 can look up. Current plugin codes (PLUGIN_CODE varies per engine) need consolidation to a single XOceanus UID.
3. **Fallback behavior**: If MPC version < 3.5 or XOceanus VST3 not installed, Oxport should fall back to a rendered keygroup program with equivalent samples. Document this fallback in the XPN expansion README.

**Priority**: Medium. VST3 program type enables the richest playback experience but requires XOceanus to be installed on the target machine. Rendered keygroup programs remain the universal path.
...
1. Render representative single-cycle waveforms from XOceanus engines (OVERWORLD era snapshots, OPAL grain freezes, OUROBOROS chaos attractors as pitch).
