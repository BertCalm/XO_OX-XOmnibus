# XOceanus — Sound Shape XPN Rendering Specification
...
Every XOceanus preset is assigned one of six Sound Shapes. The shape is stored in the `.xometa` file as `"soundShape"` and inferred automatically if absent.
...
MPC keygroup programs support sample looping (`LoopOnOff`, `LoopStart`, `LoopEnd`, `LoopCrossfade`). The question: should XOceanus-rendered WAVs use MPC looping for sustaining sounds?
