...
| `pack_name` | string | Yes | Human-readable pack name, e.g. `"ONSET Drum Essentials"`. Used for display in XOlokun import UI. |
| `xolokun_version_min` | string | Yes | Minimum XOlokun version required to load all engines referenced in `mappings`. Semver format: `"major.minor.patch"`. |
| `mappings` | array | Yes | Ordered list of program-to-preset mappings. May be empty (`[]`) if no matches were found; the file presence alone signals XOlokun to look. |
...
| `mappings[].preset_file` | string | Yes | Bare filename of the `.xometa` preset, e.g. `"Kick Hard.xometa"`. XOlokun resolves this against its preset library root. |
| `mappings[].engine` | string | Yes | Short engine name as registered in XOlokun, uppercase. E.g. `"ONSET"`, `"OPAL"`, `"OVERWORLD"`. Must match a registered engine ID. |
...
| `xolokun_version_min` | Semver string matching `\d+\.\d+\.\d+`. Must be `"1.0.0"` or higher. |
...
| `engine` | Uppercase string. Must match a registered XOlokun engine short name (see `CLAUDE.md` engine table). Unknown engine names are logged as warnings, not errors, to allow forward compatibility. |
...
- `preset_file` should resolve to an existing file under the XOlokun preset library root.
...
The V1.1 per-program schema (`xpn_air_plugin_architecture_rnd.md §4`) embeds a `params` dictionary and a `qlinks` dictionary directly inside each program's directory. It uses `"xo_ox_version"` and `"target_plugin"` as top-level keys rather than `"version"` and `"pack_name"`. These two schemas do not conflict: the pack-level file lives at the archive root; the per-program file lives inside `Programs/<Name>/`. XOlokun importers should check for both locations and prefer the per-program file if present (it carries more specific data), falling back to the pack-level file.
