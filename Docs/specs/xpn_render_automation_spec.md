...
- Access the live `XOmnibusProcessor` instance (render is fully self-contained)
...
target_link_libraries(XPNRenderCLI PRIVATE
    ...
    juce::juce_core
    XOmnibusEngines)   # shared static lib containing all engine objects
```

`XOmnibusEngines` is a new `add_library(XOmnibusEngines STATIC ...)` target that
compiles all engine `.cpp` stubs and `EngineRegistry.cpp`. The main XOmnibus AU/VST3
...
- `preset` — exact name matching a `.xometa` file in `Presets/XOmnibus/`
...
source via the `XOmnibusEngines` static library target. No engine `.h` files change.
...
3. **APVTS construction for standalone use.** The live `XOmnibusProcessor` constructs
   a merged APVTS with all 34 engine layouts. For headless rendering, a single-engine
   APVTS is needed. The merge logic in `XOmnibusProcessor::createParameterLayout()`
   must be extractable into a reusable helper.

4. **CMake refactor.** Extracting `XOmnibusEngines` as a shared static library
...
- [ ] `auval` result for the main XOmnibus AU is unchanged after CMake refactor
