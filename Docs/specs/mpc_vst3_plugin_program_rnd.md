...
1. **VST3 plugin support** in MPC Desktop — not AIR instruments, not VST2, but proper VST3. This is the
   format XOceanus ships in. The collision is not an accident.
...
These two features together create a path we've never had before: XOceanus running *inside* an MPC
...
<MPCVObject>
  ...
  <Program Type="KeygroupSynth">
    ...
    <SynthPlugin>
      ...
      <!-- Plugin preset name to recall on load — maps to XOceanus preset selector -->
...
## Section 2: XOceanus VST3 → MPC Plugin Program Path
...
XOceanus JUCE parameters are registered in `PluginProcessor.cpp` via `AudioProcessorValueTreeState`.
...
<PresetName Value="Opal_Crystal_Drift" />
```
XOceanus must implement `setStateInformation()` / `getStateInformation()` with preset-by-name
...
XPM that references the installed XOceanus VST3. No audio rendering. No disk I/O beyond writing XML.
...
XO_OX_OPAL_EXPANSION.xpn/          ← standard ZIP with .xpn extension
  Programs/
    ...
    Opal_Crystal_Drift_LIVE.xpm     ← requires XOceanus VST3 on MPC 3.5+
  ...
  Samples/
    Opal_Crystal_Drift/
      ...
      [...]
  Plugins/
    README_PLUGINS.txt              ← instructions: download XOceanus at xo-ox.org
    XOceanus_version_required.txt   ← "Requires XOceanus v1.0 or later"
  expansion.json
  liner_notes.json
```
...
contain only a README pointing to `xo-ox.org/download`. XOceanus is a free open-source download.
...
```json
{
  ...
  "plugin_programs": {
    ...
    "fallback": "Use *_SAMPLES.xpm programs on MPC < 3.5 or without XOceanus installed"
  },
  "presets": 150,
  "program_pairs": 150
}
...
python xpn_plugin_program_builder.py \
  ...
  --xometa-file Presets/XOceanus/Atmosphere/Opal_Crystal_Drift.xometa \
...
## Section 4: Hex's Lab — Getting XOceanus on MPC Hardware

*This is where we get into the actual path to hardware. Rated step by step.*

### 4.1 MPC Desktop VST3 Plugin Path

**Step: Install XOceanus VST3 to standard macOS path**

```
~/Library/Audio/Plug-Ins/VST3/XOceanus.vst3
```
...
**Step: Launch MPC 3.5 Desktop and find XOceanus in the plugin browser**

**[PLAUSIBLE]** MPC 3.5 Desktop should enumerate `~/Library/Audio/Plug-Ins/VST3/` on launch or
on a manual plugin rescan. XOceanus should appear if it passes the VST3 validation check.
...
**Step: Create Keygroup Synth Engine program, load XOceanus, load preset**

**[PLAUSIBLE]** If MPC 3.5 Desktop recognizes XOceanus as a valid VST3, creating a Keygroup Synth
Engine program and selecting it should work identically to any AIR instrument workflow. Preset
selection would depend on whether MPC exposes XOceanus's internal preset browser or requires
loading state from a file.

**[SPECULATIVE]** MPC may surface the VST3's `getProgramName()` preset list. JUCE exposes factory
presets via this mechanism. If XOceanus implements it, preset names appear in MPC's program change
list. Rex: confirm JUCE `getNumPrograms()` / `getProgramName()` implementation in XOceanus before
testing.
...
parameter, done. JUCE exposes parameter names via `getParameterName(index)` — XOceanus parameter
...
**Step: Play 16 pads → MIDI notes into XOceanus → PROFIT**
...
```
Step 1: Build XOceanus.vst3 release binary
        → cmake -B build -G Ninja -DCMAKE_BUILD_TYPE=Release && cmake --build build
        [CONFIRMED WORKS — we do this already]

Step 2: Copy to VST3 path
        → cp -r build/XOceanus_artefacts/Release/VST3/XOceanus.vst3 \
...
Step 7: Assign Q-Links 1–4 to XOceanus params → CHARACTER / MOVEMENT / COUPLING / SPACE
        [PLAUSIBLE: stated Q-Link-to-VST3-param feature of Keygroup Synth Engine]

Step 8: Play pads → hear XOceanus synthesizing live in MPC → PROFIT
...
| XOceanus passes validation | PLAUSIBLE | Possible whitelist/signing issue |
...
1. **[Rex]** Verify XOceanus APVTS macro registration order — ensure CHARACTER/MOVEMENT/COUPLING/SPACE
   are at parameter indices 0–3. If not, add a frozen index guarantee block in `XOceanusProcessor.h`.

2. **[Rex]** Confirm JUCE `getNumPrograms()` / `getProgramName()` is implemented in XOceanus.
...
8. **[Both]** Design the first full hybrid XPN pack: OPAL as the pilot. 150 preset pairs.
   Every preset ships both `_SAMPLES.xpm` (works everywhere) and `_LIVE.xpm` (XOceanus required).
...
The moment Hex installs XOceanus.vst3 → opens MPC 3.5 Desktop → sees XOceanus in the plugin list,
