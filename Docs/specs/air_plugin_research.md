# AIR Plugin Research Brief — XOceanus Compatibility
...
Package XOceanus as an AU/VST3 that MPC Software discovers and loads, mirroring how AIR plugins
work on desktop.

**What this requires:**
- XOceanus already passes `auval` — the DSP is ready.
...
- A `params_sidecar.json` file per program (see full spec in `xpn_air_plugin_architecture_rnd.md`)
  that XOceanus parses to restore engine voice character alongside the MPC program.
...
- Write an XOceanus "Sidecar Import" feature: scan a user-selected directory for
...
- Pursue Apple notarization so XOceanus can be distributed as a signed free download, linkable
...
- **Full plugin bundle ZIP support**: Building the `plugin_bundle/` expansion architecture and
  ...
  work, not raw DSP.
- **Hardware MPC native port**: Running XOceanus natively on MPC hardware (ARM Linux, embedded
