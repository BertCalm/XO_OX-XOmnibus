...
The target is 80% original content: XOmnibus engine renders or custom recordings. The remaining 20% is ceiling, not standard.
...
1. **XOmnibus engine renders** — the primary source, using the renderNoteToWav() pipeline once available. Every pack should be a product of the instrument ecosystem.
...
3. **Pre-recorded samples processed through XOmnibus engines** — source material transformed through the engine signal chain. The transformation must be audible and meaningful, not cosmetic.
