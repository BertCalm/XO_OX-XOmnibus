...
XOceanus presets carry a 6D Sonic DNA vector: `brightness`, `warmth`, `movement`, `density`, `space`, `aggression`. Each dimension is a float 0.0–1.0. Currently these are assigned manually by a sound designer listening to each preset — accurate but slow, and prone to inconsistency across the fleet (2,369 presets and growing).
