...
static const float* const kConvergentTables[4] = {
    kPiRatios, kERatios, kPhiRatios, kSqrt2Ratios
};
...
public:
    ...
    static void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(std::make_unique<PI>(P("over_constant",1), "Constant",
            0, 3, 2));  // default: Phi (index 2) — most musical, Fibonacci
        params.push_back(std::make_unique<PF>(P("over_depth",1),    "Depth",
            nr(0.f, 7.f), 2.f));

        // --- 8 partial amplitudes ---
        params.push_back(std::make_unique<PF>(P("over_partial0",1), "Partial 0",
            nr(0.f, 1.f), 1.0f));
        params.push_back(std::make_unique<PF>(P("over_partial1",1), "Partial 1",
            nr(0.f, 1.f), 0.5f));
        params.push_back(std::make_unique<PF>(P("over_partial2",1), "Partial 2",
            nr(0.f, 1.f), 0.333333f));
        params.push_back(std::make_unique<PF>(P("over_partial3",1), "Partial 3",
            nr(0.f, 1.f), 0.25f));
        params.push_back(std::make_unique<PF>(P("over_partial4",1), "Partial 4",
            nr(0.f, 1.f), 0.2f));
        params.push_back(std::make_unique<PF>(P("over_partial5",1), "Partial 5",
            nr(0.f, 1.f), 0.166667f));
        params.push_back(std::make_unique<PF>(P("over_partial6",1), "Partial 6",
            nr(0.f, 1.f), 0.142857f));
        params.push_back(std::make_unique<PF>(P("over_partial7",1), "Partial 7",
            nr(0.f, 1.f), 0.125f));

        // --- D001: Velocity → partial brightness ---
        params.push_back(std::make_unique<PF>(P("over_velBright",1), "Vel Brightness",
            nr(0.f, 1.f), 0.4f));

        // --- Brightness filter ---
        params.push_back(std::make_unique<PF>(P("over_filterCutoff",1), "Filter Cutoff",
            nr(1000.f, 20000.f), 12000.f));
        params.push_back(std::make_unique<PF>(P("over_filterRes",1),    "Filter Resonance",
            nr(0.f, 0.8f), 0.3f));

        // --- Amp envelope ---
        params.push_back(std::make_unique<PF>(P("over_ampAtk",1), "Amp Attack",
            nr(0.001f, 4.0f), 0.02f));
        params.push_back(std::make_unique<PF>(P("over_ampDec",1), "Amp Decay",
            nr(0.05f,  5.0f), 0.3f));
        params.push_back(std::make_unique<PF>(P("over_ampSus",1), "Amp Sustain",
            nr(0.f, 1.f), 0.7f));
        params.push_back(std::make_unique<PF>(P("over_ampRel",1), "Amp Release",
            nr(0.05f, 8.0f), 1.0f));

        // --- D002: LFO 1 — modulates over_depth (convergent index sweep) ---
        params.push_back(std::make_unique<PF>(P("over_lfo1Rate",1),  "LFO1 Rate",
            nr(0.01f, 10.f), 0.25f));
        params.push_back(std::make_unique<PF>(P("over_lfo1Depth",1), "LFO1 Depth",
            nr(0.f, 1.f), 0.2f));

        // --- D002: LFO 2 — modulates partial rotation phase ---
        params.push_back(std::make_unique<PF>(P("over_lfo2Rate",1),  "LFO2 Rate",
            nr(0.01f, 10.f), 0.1f));
        params.push_back(std::make_unique<PF>(P("over_lfo2Depth",1), "LFO2 Depth",
            nr(0.f, 1.f), 0.15f));

        // --- Resonator allpass mix ---
        params.push_back(std::make_unique<PF>(P("over_resoMix",1), "Resonator Mix",
            nr(0.f, 1.f), 0.15f));

        // --- 4 Macros ---
        params.push_back(std::make_unique<PF>(P("over_macroDepth",1),    "DEPTH",
            nr(0.f, 1.f), 0.35f));
        params.push_back(std::make_unique<PF>(P("over_macroColor",1),    "COLOR",
            nr(0.f, 1.f), 0.5f));
        params.push_back(std::make_unique<PF>(P("over_macroCoupling",1), "COUPLING",
            nr(0.f, 1.f), 0.0f));
        params.push_back(std::make_unique<PF>(P("over_macroSpace",1),    "SPACE",
            nr(0.f, 1.f), 0.3f));
    }
                     ...
                     int                        numSamples) override
    {
        ...
        if (!p_constant) { buffer.clear(); return; }
        ...
        const float effectiveDepth = clamp(baseDepth + macroDepthAdd
                                           + modWheelDepthBoost + couplingDepthMod,
                                           0.f, 7.f);
