...
struct FormantFilter
{
        ...
        else
        {
            ...
            for (int partialIdx = 0; partialIdx < 64; ++partialIdx)
            {
                ...
                for (int formantIdx = 0; formantIdx < 3; ++formantIdx)
                {
                    ...
                    float deviation = (partialFreq - formantCenters[formantIdx]) / kFormantBandwidthHz[formantIdx];
                    gain += kFormantPeakAmplitude[formantIdx] * std::exp(-0.5f * deviation * deviation);
                }
    ...
    float apply(int partialIdx, float amplitude) const noexcept { return amplitude * envelope[partialIdx]; }
};
...
struct OrbitalVoice
{
    ...
    GroupEnvStage groupStage[4] = {GroupEnvStage::Off, GroupEnvStage::Off, GroupEnvStage::Off, GroupEnvStage::Off};
...
public:
    ...
    float getSampleForCoupling(int channel, int sampleIndex) const override
    {
        ...
        if (channel == 2)
            return envelopeOutput; // envelope follower for cross-engine mod
        return 0.0f;
    }
    ...
    {
        if (p_profileA == nullptr || numSamples <= 0)
            return;
        ...
        float groupAttackTimes[4] = {p_groupAttack1->load(), p_groupAttack2->load(), p_groupAttack3->load(),
                                     p_groupAttack4->load()};
        float groupDecayTimes[4] = {p_groupDecay1->load(), p_groupDecay2->load(), p_groupDecay3->load(),
                                    p_groupDecay4->load()};
        ...
        //-- Apply macros -------------------------------------------------------
        applyMacros(brightness, oddEven, morphPosition, stereoSpread, formantShift, groupAttackTimes, groupDecayTimes);
        ...
        {
            formantFilter.build(vowelIndex, lastFundamentalHz, brightness, oddEven, formantShift);
        ...
        float effectiveMorph =
            juce::jlimit(0.0f, 1.0f, morphPosition + morphOffset + 0.05f * spectralDriftLFO.process());
        ...
        for (const auto& meta : midi)
        {
            ...
            if (message.isNoteOn())
            {
                ...
                else if (voiceMode == 2) // Legato
                {
                    ...
                    for (auto& voice : voices)
                    {
                        if (voice.active && voice.envStage != OrbitalVoice::EnvStage::Release &&
                            voice.envStage != OrbitalVoice::EnvStage::Off)
                        {
                            ...
                            legatoSlideVoice(voice, message.getNoteNumber(), inharmonicity, fmRatio);
                            legatoHandled = true;
                            break;
                        }
                    }
        ...
        const float pitchRatio = pitchModActive ? fastExp(pitchOffset * 0.0578f) : 1.0f;
        ...
        const float* fmAudioPointer = externalFmActive ? couplingAudioBuffer.getReadPointer(1) : nullptr;
        const float* ringModPointer = hasRingCoupling ? couplingRingBuffer.getReadPointer(0) : nullptr;
        ...
        for (int sampleIdx = 0; sampleIdx < numSamples; ++sampleIdx)
        {
            ...
            const float externalFmSample = externalFmActive ? fmAudioPointer[sampleIdx] * fmAudioAmount : 0.0f;
            ...
            for (auto& voice : voices)
            {
                ...
                for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
                {
                    ...
                    const int groupIdx = (partialIdx < 8) ? 0 : (partialIdx < 16) ? 1 : (partialIdx < 32) ? 2 : 3;

                    // Morph between Profile A and B amplitudes, then apply
                    // formant shaping and group envelope
                    float amplitude =
                        lerp(profileAmplitudesA[partialIdx], profileAmplitudesB[partialIdx], effectiveMorph) *
                        formantFilter.envelope[partialIdx] * voice.groupEnvLevel[groupIdx];
                    ...
                    if (fmActive || externalFmActive)
                    {
                        ...
                        voice.phase[partialIdx] +=
                            static_cast<double>(voice.phaseIncrement[partialIdx] * pitchRatio * (1.0f + fmModulation));
                        voice.fmPhase[partialIdx] += static_cast<double>(voice.fmPhaseIncrement[partialIdx]);
                    }
                    else
                    {
                        voice.phase[partialIdx] += static_cast<double>(voice.phaseIncrement[partialIdx] * pitchRatio);
                    }

                    // Synthesize sine partial and pan to stereo
                    const float sineSample = amplitude * fastSin(static_cast<float>(voice.phase[partialIdx]));
...
private:
    ...
    void triggerVoice(int note, float vel, float inharmonicity, float fmRatio, float stereoSpread, float attackSeconds,
                      float decaySeconds, float sustainLevel, float releaseSeconds, const float groupAttackTimes[4],
                      const float groupDecayTimes[4])
    {
        ...
        for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
        {
            ...
            const float inharmonicRatio = std::sqrt(1.0f + stiffnessCoeff * partialNumber * partialNumber);

            const double partialFreq = static_cast<double>(voice.fundamentalHz * partialNumber * inharmonicRatio);

            // Phase increment = freq * 2pi / sampleRate (radians per sample)
            voice.phaseIncrement[partialIdx] = static_cast<float>(partialFreq * twoPi * inverseSampleRate);
            voice.fmPhaseIncrement[partialIdx] =
                static_cast<float>(partialFreq * static_cast<double>(fmRatio) * twoPi * inverseSampleRate);
        ...
        for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
        {
            float spreadAmount = stereoSpread * static_cast<float>(partialIdx) * kInvPartialCount;
    ...
    void noteOff(int noteNumber)
    {
        for (auto& voice : voices)
        {
            if (voice.active && voice.noteNumber == noteNumber && voice.envStage != OrbitalVoice::EnvStage::Release &&
                voice.envStage != OrbitalVoice::EnvStage::Off)
            {
                voice.envStage = OrbitalVoice::EnvStage::Release;
            }
    ...
    void legatoSlideVoice(OrbitalVoice& voice, int newNote, float inharmonicity, float fmRatio) noexcept
    {
    ...
    void applyMacros(float& brightness, float& oddEven, float& morph, float& stereoSpread, float& formantShift,
                     float groupAttackTimes[4], float groupDecayTimes[4]) noexcept
    {
        if (p_macroSpectrum == nullptr)
            return;
        ...
        for (auto& voice : voices)
        {
            ...
            for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
            {
                if (voice.phase[partialIdx] > wrapThreshold)
                    voice.phase[partialIdx] -= twoPi * std::floor(voice.phase[partialIdx] / twoPi);
                if (voice.fmPhase[partialIdx] > wrapThreshold)
                    voice.fmPhase[partialIdx] -= twoPi * std::floor(voice.fmPhase[partialIdx] / twoPi);
            }
        }
    }
    ...
    static std::array<float, kNumPartials> buildSquare()
    {
        ...
        std::array<float, kNumPartials> amplitudes{};
        for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
            amplitudes[partialIdx] = ((partialIdx + 1) % 2 == 1) ? 1.0f / static_cast<float>(partialIdx + 1) : 0.0f;
        return amplitudes;
    }
    ...
    static std::array<float, kNumPartials> buildTriangle()
    {
        ...
        {
            const float partialNumber = static_cast<float>(partialIdx + 1);
            amplitudes[partialIdx] = ((partialIdx + 1) % 2 == 1) ? 1.0f / (partialNumber * partialNumber) : 0.0f;
        }
        return amplitudes;
    }
    ...
    static std::array<float, kNumPartials> buildBell()
    {
        ...
        for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
        {
            ...
            amplitudes[partialIdx] = juce::jlimit(0.0f, 1.0f, exponentialDecay * inharmonicRipple);
        }
        return amplitudes;
    }
    ...
    static std::array<float, kNumPartials> buildGlass()
    {
        ...
        {
            const float partialNumber = static_cast<float>(partialIdx);
            amplitudes[partialIdx] = (partialIdx < 8) ? juce::jlimit(0.0f, 1.0f, 0.05f + 0.082f * partialNumber)
    ...
    static std::array<float, kNumPartials> buildVocal()
    {
        ...
        for (int partialIdx = 0; partialIdx < kNumPartials; ++partialIdx)
        {
            ...
            const float formantBoost = 0.4f * std::exp(-0.5f * std::pow((partialNumber - 4.0f) / 3.0f, 2.0f));
    ...
    inline static const std::array<std::array<float, kNumPartials>, 8> sProfiles = buildAllProfiles();
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            ...
            juce::StringArray{"Sawtooth", "Square", "Triangle", "Bell", "Organ", "Glass", "Vocal", "Custom"}, 0));
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            P{"orb_profileB", 1}, "Orbital Profile B",
            juce::StringArray{"Sawtooth", "Square", "Triangle", "Bell", "Organ", "Glass", "Vocal", "Custom"}, 3));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_morph", 1}, "Orbital Morph",
                                                                     NR(0.0f, 1.0f, 0.001f), 0.0f));

        //-- Spectral shaping ---------------------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_brightness", 1}, "Orbital Brightness",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.7f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_oddEven", 1}, "Orbital Odd/Even",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.5f));
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            P{"orb_formantShape", 1}, "Orbital Formant", juce::StringArray{"Off", "A", "E", "I", "O", "U"}, 0));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_formantShift", 1}, "Orbital Formant Shift",
                                                                     NR(-24.0f, 24.0f, 0.1f), 0.0f));

        //-- Inharmonicity + FM -------------------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_inharm", 1}, "Orbital Inharmonicity",
                                                                     NR(0.0f, 1.0f, 0.001f), 0.0f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_fmIndex", 1}, "Orbital FM Index",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_fmRatio", 1}, "Orbital FM Ratio",
                                                                     NR(0.5f, 8.0f, 0.01f, 0.4f), 2.0f));

        //-- Group envelopes (4 partial bands, each with attack + decay) --------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupAttack1", 1}, "Orbital Group1 Attack",
                                                                     NR(0.001f, 4.0f, 0.001f, 0.4f), 0.01f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupDecay1", 1}, "Orbital Group1 Decay",
                                                                     NR(0.01f, 4.0f, 0.001f, 0.4f), 0.5f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupAttack2", 1}, "Orbital Group2 Attack",
                                                                     NR(0.001f, 4.0f, 0.001f, 0.4f), 0.01f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupDecay2", 1}, "Orbital Group2 Decay",
                                                                     NR(0.01f, 4.0f, 0.001f, 0.4f), 0.4f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupAttack3", 1}, "Orbital Group3 Attack",
                                                                     NR(0.001f, 2.0f, 0.001f, 0.4f), 0.005f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupDecay3", 1}, "Orbital Group3 Decay",
                                                                     NR(0.01f, 2.0f, 0.001f, 0.4f), 0.3f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupAttack4", 1}, "Orbital Group4 Attack",
                                                                     NR(0.001f, 1.0f, 0.001f, 0.4f), 0.002f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_groupDecay4", 1}, "Orbital Group4 Decay",
                                                                     NR(0.01f, 1.0f, 0.001f, 0.4f), 0.2f));

        //-- Post filter --------------------------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_filterCutoff", 1}, "Orbital Filter Cutoff",
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            P{"orb_filterEnvDepth", 1}, "Orbital Filter Env Depth", NR(0.0f, 1.0f, 0.01f), 0.25f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_filterReso", 1}, "Orbital Filter Reso",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back(std::make_unique<juce::AudioParameterChoice>(P{"orb_filterType", 1}, "Orbital Filter Type",
                                                                      juce::StringArray{"LP", "BP", "HP"}, 0));

        //-- Output shaping -----------------------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_stereoSpread", 1}, "Orbital Stereo Spread",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.3f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_saturation", 1}, "Orbital Saturation",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.0f));

        //-- Global ADSR envelope -----------------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_ampAttack", 1}, "Orbital Amp Attack",
                                                                     NR(0.001f, 8.0f, 0.001f, 0.4f), 0.1f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_ampDecay", 1}, "Orbital Amp Decay",
                                                                     NR(0.05f, 4.0f, 0.001f, 0.4f), 0.5f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_ampSustain", 1}, "Orbital Amp Sustain",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.8f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_ampRelease", 1}, "Orbital Amp Release",
                                                                     NR(0.05f, 8.0f, 0.001f, 0.4f), 1.0f));

        //-- Master volume ------------------------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_volume", 1}, "Orbital Volume",
                                                                     NR(0.0f, 1.0f, 0.01f), 0.8f));
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(P{"orb_voiceMode", 1}, "Orbital Voice Mode",
                                                                      juce::StringArray{"Poly", "Mono", "Legato"}, 0));

        //-- Macros (4 performance knobs) ---------------------------------------
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_macroSpectrum", 1}, "Orbital Spectrum",
                                                                     NR(0.0f, 1.0f, 0.001f), 0.5f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_macroEvolve", 1}, "Orbital Evolve",
                                                                     NR(0.0f, 1.0f, 0.001f), 0.0f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(P{"orb_macroCoupling", 1}, "Orbital Coupling",
                                                                     NR(0.0f, 1.0f, 0.001f), 0.0f));
