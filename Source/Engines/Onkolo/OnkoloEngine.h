...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        float effectiveFunk =
            std::clamp(pFunk + macroChar * 0.4f + modWheelAmount * 0.5f + couplingFunkMod, 0.0f, 1.0f);
        float effectivePickup = std::clamp(pPickup + macroSpace * 0.3f + aftertouchAmount * 0.3f, 0.0f, 1.0f);
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                {
                    float migOdd =
                        fastSin(freq * 3.0f / srf * 6.28318530718f * voice.string.phases[0]) * migrationN * 0.1f;
                    wahOut += migOdd;
                }
