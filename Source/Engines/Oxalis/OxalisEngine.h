...
struct PhyllotaxisOscBank
{
    ...
    void reset() noexcept
    {
        for (int i = 0; i < kMaxPartials; ++i)
        {
            ...
            if (partials[i].phase >= 1.0f) partials[i].phase -= 1.0f;
        }
    }
};
...
public:
                            ...
                            const float* buf, int numSamples) override
    {
        ...
        switch (type)
        {
            ...
            case CouplingType::EnvToMorph:   couplingPhiMod += val; break;
            default: break;
        }
    }
            ...
            for (auto& voice : voices)
            {
                ...
                if (voice.growthMode)
                {
                    ...
                    for (int p = 0; p < PhyllotaxisOscBank::kMaxPartials; ++p)
                        voice.oscBank.partials[p].active = (p < activePartials)
                            && voice.oscBank.partials[p].phaseInc > 0.0f;
    ...
    void noteOn (int note, float vel) noexcept
    {
        ...
        v.ampEnv.setADSR (attackTime,
                          ...
                          paramSustain ? paramSustain->load() : 0.85f,
