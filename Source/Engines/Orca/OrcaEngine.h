...
struct OrcaCombFilter
{
    ...
    void setFeedback(float fb) noexcept { feedback = clamp(fb, 0.0f, 0.995f); }

    void setDamping(float damp) noexcept { dampingCoeff = clamp(damp, 0.0f, 0.99f); }
...
public:
        ...
        for (const auto metadata : midi)
        {
            ...
            if (msg.isNoteOn())
            {
                ...
                noteOn(msg.getNoteNumber(), msg.getFloatVelocity(), msg.getChannel(), maxPoly, monoMode, legatoMode,
                       glideCoeff, pAmpA, pAmpD, pAmpS, pAmpR, pModA, pModD, pModS, pModR, pLfo1Rate, pLfo1Depth,
                       pLfo1Shape, pLfo2Rate, pLfo2Depth, pLfo2Shape, effectiveCutoff, effectiveReso, formantFreqs,
                       formantQ);
            }
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_wtScanRate", 1}, "Orca WT Scan Rate",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_formantShift", 1}, "Orca Formant Shift",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.5f));

        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{"orca_glide", 1}, "Orca Glide", juce::NormalisableRange<float>(0.0f, 5.0f, 0.001f, 0.5f),
            0.3f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_echoReso", 1}, "Orca Echo Resonance",
                                                        juce::NormalisableRange<float>(0.0f, 0.995f, 0.001f), 0.85f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_echoDamp", 1}, "Orca Echo Damping",
                                                        juce::NormalisableRange<float>(0.0f, 0.99f, 0.001f), 0.3f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_echoMix", 1}, "Orca Echo Mix",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.0f));

        // --- Apex Hunt (Master Macro) ---
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_huntMacro", 1}, "Orca Hunt",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.0f));

        // --- Breach (Sub + Sidechain) ---
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_breachSub", 1}, "Orca Breach Sub Level",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.5f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_breachRatio", 1}, "Orca Breach Ratio",
                                                        juce::NormalisableRange<float>(1.0f, 20.0f, 0.1f), 8.0f));

        // --- Countershading (Bitcrusher) ---
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_crushBits", 1}, "Orca Crush Bits",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_crushMix", 1}, "Orca Crush Mix",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.0f));
        ...
        // --- Level ---
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{"orca_level", 1}, "Orca Level", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.8f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_ampSustain", 1}, "Orca Amp Sustain",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_modSustain", 1}, "Orca Mod Sustain",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_lfo1Depth", 1}, "Orca LFO1 Depth",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.5f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_lfo2Depth", 1}, "Orca LFO2 Depth",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
        ...
        // --- Voice Mode ---
        params.push_back(
            std::make_unique<juce::AudioParameterChoice>(juce::ParameterID{"orca_polyphony", 1}, "Orca Voice Mode",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"orca_macroSpace", 1}, "Orca Macro SPACE",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
    }
...
private:
    ...
    void noteOn(int noteNumber, float velocity, int midiChannel, int maxPoly, bool monoMode, bool legatoMode,
                ...
                float modR, float lfo1Rate, float lfo1Depth, int lfo1Shape, float lfo2Rate, float lfo2Depth,
                int lfo2Shape, float cutoff, float reso, const float formantFreqs[5], const float formantQ[5])
    {
