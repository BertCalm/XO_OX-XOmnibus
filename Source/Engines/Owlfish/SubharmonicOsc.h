...
public:
    ...
    void setParams (int div1, int div2, int div3, int div4,
                    float level1, float level2, float level3, float level4,
                    float subMix, float mixtur, float fundWave, float subWave,
                    float bodyFreq, float bodyLevel)
    {
    ...
    float processSample (float freq)
    {
        ...
        fundPhase += fundInc;
        if (fundPhase >= 1.0f) fundPhase -= 1.0f;
        ...
        for (int i = 0; i < 4; ++i)
        {
            if (divisions[i] == 0) continue;  // off

            float subInc = freq / static_cast<float> (divisions[i]) * invSr;
            subPhase[i] += subInc;
            if (subPhase[i] >= 1.0f) subPhase[i] -= 1.0f;
