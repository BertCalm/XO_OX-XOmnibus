...
public:
    void prepare (double sampleRate)
    {
        ...
        if (preDelayMaxLen < 1) preDelayMaxLen = 1;
    ...
    void setParams (float size01, float damp01, float preDelay01)
    {
        ...
        if (dampCoeff > 1.0f) dampCoeff = 1.0f;
        if (dampCoeff < 0.0f) dampCoeff = 0.0f;
        ...
        if (preDelaySamples >= preDelayMaxLen) preDelaySamples = preDelayMaxLen - 1;
    ...
    void processSample (float inputL, float inputR, float& outL, float& outR)
    {
        ...
        if (readPos < 0) readPos += preDelayMaxLen;
