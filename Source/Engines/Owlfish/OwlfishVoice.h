...
public:
    ...
    void applyPitchBend(float ratio) noexcept { targetFreq = baseNoteFreq * ratio; }
    ...
    void process(float* outL, float* outR, int numSamples, const OwlfishParamSnapshot& snap)
    {
        // ================================================================
        ...
        float blockGlideProgress =
            (portaCoeff > 0.0f && targetFreq > 1.0f)
                ? std::max(0.0f, std::min(1.0f, 1.0f - std::fabs(currentFreq - targetFreq) / targetFreq))
                : 1.0f;
        ...
        abyssOsc.setParams(snap.subDiv1, snap.subDiv2, snap.subDiv3, snap.subDiv4, snap.subLevel1, snap.subLevel2,
                           snap.subLevel3, snap.subLevel4, snap.subMix, glideModMixtur, snap.fundWave, snap.subWave,
        ...
        opticsFilter.setCoefficients(cutoffHz, snap.filterReso, static_cast<float>(sampleRate));
