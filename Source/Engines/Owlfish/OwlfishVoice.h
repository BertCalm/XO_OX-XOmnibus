...
//   8. OUTPUT:          Level + pan
//
// Coupling cache: lastOutL/R available via getLastSampleL/R() for XOmnibus
...
public:
                  ...
                  const OwlfishParamSnapshot& snap)
    {
        ...
        abyssOsc.setParams (snap.subDiv1, snap.subDiv2, snap.subDiv3, snap.subDiv4,
                            ...
                            snap.subMix, snap.mixtur, snap.fundWave, snap.subWave,
        ...
        diet.setParams (snap.grainSize, snap.grainDensity, snap.grainPitch, snap.feedRate);
    ...
    //--------------------------------------------------------------------------
    /// Get last output sample (left) for XOmnibus coupling cache.
    float getLastSampleL() const { return lastOutL; }

    /// Get last output sample (right) for XOmnibus coupling cache.
