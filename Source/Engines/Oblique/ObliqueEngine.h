...
public:
    ...
    void prepare(double sampleRate) noexcept
    {
        ...
        static constexpr float kDefaultColorFreqs[kNumFacets] = {100.0f, 300.0f, 800.0f, 2000.0f, 5000.0f, 12000.0f};

        // Default pan positions: spread evenly from hard left (0.0) to hard right (1.0)
        static constexpr float kDefaultPanPositions[kNumFacets] = {0.0f, 0.2f, 0.4f, 0.6f, 0.8f, 1.0f};
...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        for (const auto metadata : midi)
        {
            ...
            {
                silenceGate.wake();
                noteOn(msg.getNoteNumber(), msg.getFloatVelocity(), oscWaveIndex, oscDetuneCents, glideTime,
                       effectiveBounceRate, bounceClickTone, voiceMode);
            }
        ...
        for (int sampleIndex = 0; sampleIndex < numSamples; ++sampleIndex)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                if (!voice.releasing)
                {
                    if (voice.envelopeStage < 1.0f)
                    {
                        ...
                        float attackRate =
                            (attackTime > kMinEnvelopeTime) ? 1.0f / (attackTime * sampleRateFloat) : 1.0f;
                ...
                voice.oscillatorSecondary.setFrequency(frequency * detuneRatio, sampleRateFloat);
                ...
                float voicedOutput = oscillatorOutput * voice.envelopeLevel * voice.velocity * stealFadeGain *
                ...
                float bounceOutput = voice.bounce.process(bounceParams) * bounceClickLevel * voice.velocity;
            ...
            double effectiveLfoRate =
                static_cast<double>(lfo1Rate) + static_cast<double>(macroColor) * kLfo1MacroRateBoost;
            obliqueLfoPhase += effectiveLfoRate / hostSampleRate;
            if (obliqueLfoPhase >= 1.0)
                obliqueLfoPhase -= 1.0;
            ...
            if (obliqueLfo2Phase >= 1.0)
                obliqueLfo2Phase -= 1.0;
    ...
    float getSampleForCoupling(int channel, int sampleIndex) const override
    {
        ...
        if (channel == 0 && index < couplingCacheL.size())
            return couplingCacheL[index];
        if (channel == 1 && index < couplingCacheR.size())
            return couplingCacheR[index];
        if (channel == 2)
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_oscFold", 1}, "Oblique Wavefold",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.3f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_oscDetune", 1}, "Oblique Detune",
                                                        juce::NormalisableRange<float>(0.0f, 50.0f, 0.1f), 8.0f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_level", 1}, "Oblique Level",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.8f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_glide", 1}, "Oblique Glide",
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(juce::ParameterID{"oblq_voiceMode", 1},
                                                                      "Oblique Voice Mode",
                                                                      juce::StringArray{"Poly", "Mono", "Legato"}, 0));

        // --- Bounce (percussive ricochet) ---
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_percClick", 1}, "Oblique Click Level",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.6f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_percDecay", 1}, "Oblique Click Decay",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_bounceCnt", 1}, "Oblique Bounce Count",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_sustain", 1}, "Oblique Sustain",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_prismColor", 1}, "Oblique Prism Color",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.7f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_prismWidth", 1}, "Oblique Prism Width",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.8f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_prismMix", 1}, "Oblique Prism Mix",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.45f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_prismDamp", 1}, "Oblique Prism Damping",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.3f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_phaserMix", 1}, "Oblique Phaser Mix",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_lfo1Depth", 1}, "Oblique LFO1 Depth",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_lfo2Depth", 1}, "Oblique LFO2 Depth",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_macroFold", 1}, "Oblique FOLD",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_macroBounce", 1}, "Oblique BOUNCE",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_macroColor", 1}, "Oblique COLOR",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"oblq_macroSpace", 1}, "Oblique SPACE",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
    }
...
private:
    ...
    void noteOn(int noteNumber, float velocity, int oscWaveIndex, float detuneCents, float glideTime,
                float bounceRateMs, float bounceClickTone, int voiceMode) noexcept
    {
        // -----------------------------------------------------------------------
        ...
        case 1:
            ...
            break;
        case 2:
            waveform = PolyBLEP::Waveform::Square;
            break;
        case 3:
