...
struct DeepWaveguideBody
{
    ...
    float tick(float in, float freq, float feedback, int bodyChar)
    {
        ...
        if (bodyChar == 1)
            tuneOffset = 0.012f; // cave — slightly flat resonance
        if (bodyChar == 2)
            tuneOffset = -0.025f; // wreck — hull harmonic shift
...
struct DeepBioExciter
{
    ...
    float tick(float lfoRate, float bioBrightness, float sr)
    {
        ...
        if (lfoPhase >= 1.f)
            lfoPhase -= 1.f;
        float lfoVal = (lfoPhase < 0.5f) ? (lfoPhase * 4.f - 1.f)  // rising: -1 → +1
...
struct DeepAbyssalReverb
{
    ...
    void reset()
    {
        ...
        {
            std::fill(combBufL[i].begin(), combBufL[i].end(), 0.f);
            combStateL[i] = 0.f;
            combPosL[i] = 0;
            std::fill(combBufR[i].begin(), combBufR[i].end(), 0.f);
            combStateR[i] = 0.f;
            combPosR[i] = 0;
        }
    ...
    void process(float& inL, float& inR, float space, float mix)
    {
        ...
        auto runComb = [&](std::vector<float>& buf, int& pos, float& state, int len, float input) -> float
        {
...
public:
    ...
    static void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(std::make_unique<PF>(P("deep_subLevel", 1), "Sub Level", nr(0.f, 1.f), 0.7f));
        params.push_back(std::make_unique<PF>(P("deep_subOctMix", 1), "Sub Oct Mix", nr(0.f, 1.f), 0.4f));

        // --- Darkness filter ---
        params.push_back(std::make_unique<PF>(P("deep_filterCutoff", 1), "Filter Cutoff", nr(50.f, 800.f), 400.f));
        params.push_back(std::make_unique<PF>(P("deep_filterRes", 1), "Filter Resonance", nr(0.f, 0.95f), 0.5f));
        params.push_back(std::make_unique<PF>(P("deep_velCutoffAmt", 1), "Vel Cutoff Amount", nr(0.f, 1.f), 0.4f));

        // --- Waveguide body ---
        params.push_back(std::make_unique<PI>(P("deep_bodyChar", 1), "Body Character", 0, 2, 0));
        params.push_back(std::make_unique<PF>(P("deep_bodyFeedback", 1), "Body Feedback", nr(0.f, 0.9f), 0.45f));
        params.push_back(std::make_unique<PF>(P("deep_bodyMix", 1), "Body Mix", nr(0.f, 1.f), 0.3f));

        // --- Bioluminescent exciter ---
        params.push_back(std::make_unique<PF>(P("deep_bioRate", 1), "Bio Rate", nr(0.01f, 0.5f), 0.08f));
        params.push_back(std::make_unique<PF>(P("deep_bioMix", 1), "Bio Mix", nr(0.f, 1.f), 0.15f));
        params.push_back(std::make_unique<PF>(P("deep_bioBrightness", 1), "Bio Brightness", nr(200.f, 4000.f), 800.f));

        // --- Hydrostatic compressor ---
        params.push_back(std::make_unique<PF>(P("deep_pressureAmt", 1), "Pressure Amount", nr(0.f, 1.f), 0.6f));

        // --- Amp envelope ---
        params.push_back(std::make_unique<PF>(P("deep_ampAtk", 1), "Amp Attack", nr(0.001f, 0.5f), 0.01f));
        params.push_back(std::make_unique<PF>(P("deep_ampDec", 1), "Amp Decay", nr(0.1f, 5.0f), 0.5f));
        params.push_back(std::make_unique<PF>(P("deep_ampSus", 1), "Amp Sustain", nr(0.f, 1.f), 0.8f));
        params.push_back(std::make_unique<PF>(P("deep_ampRel", 1), "Amp Release", nr(0.2f, 8.0f), 1.5f));

        // --- LFO 1: creature modulation ---
        params.push_back(std::make_unique<PF>(P("deep_lfo1Rate", 1), "LFO1 Rate", nr(0.01f, 2.0f), 0.15f));
        params.push_back(std::make_unique<PF>(P("deep_lfo1Depth", 1), "LFO1 Depth", nr(0.f, 1.f), 0.3f));

        // --- LFO 2: pressure wobble ---
        params.push_back(std::make_unique<PF>(P("deep_lfo2Rate", 1), "LFO2 Rate", nr(0.01f, 0.5f), 0.05f));
        params.push_back(std::make_unique<PF>(P("deep_lfo2Depth", 1), "LFO2 Depth", nr(0.f, 1.f), 0.2f));

        // --- Reverb ---
        params.push_back(std::make_unique<PF>(P("deep_reverbMix", 1), "Reverb Mix", nr(0.f, 1.f), 0.35f));

        // --- Filter ADSR (Week 12 EQ sprint) ---
        params.push_back(std::make_unique<PF>(P("deep_filterA", 1), "Filter Attack", nr(0.001f, 4.0f, 0.001f), 0.01f));
        params.push_back(std::make_unique<PF>(P("deep_filterD", 1), "Filter Decay", nr(0.01f, 8.0f, 0.001f), 0.4f));
        params.push_back(std::make_unique<PF>(P("deep_filterS", 1), "Filter Sustain", nr(0.0f, 1.0f, 0.001f), 0.6f));
        params.push_back(std::make_unique<PF>(P("deep_filterR", 1), "Filter Release", nr(0.01f, 8.0f, 0.001f), 1.2f));
        params.push_back(
            std::make_unique<PF>(P("deep_filterEnvAmt", 1), "Filter Env Amount", nr(-1.0f, 1.0f, 0.001f), 0.5f));

        // --- 4 Macros ---
        params.push_back(std::make_unique<PF>(P("deep_macroPressure", 1), "PRESSURE", nr(0.f, 1.f), 0.5f));
        params.push_back(std::make_unique<PF>(P("deep_macroCreature", 1), "CREATURE", nr(0.f, 1.f), 0.3f));
        params.push_back(std::make_unique<PF>(P("deep_macroWreck", 1), "WRECK", nr(0.f, 1.f), 0.4f));
        params.push_back(std::make_unique<PF>(P("deep_macroAbyss", 1), "ABYSS", nr(0.f, 1.f), 0.5f));
    }
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        const int effectiveBodyChar = (macroWreck > 0.7f) ? 2 : (macroWreck > 0.35f) ? 1 : bodyChar;
        ...
        const float finalCutoff =
            clamp(baseCutoff + velCutoffBoost + modWheelCutoffMod - abyssFilterClose + couplingFilterMod, 50.f, 800.f);
        ...
        const float fundamentalFreq =
            midiToFreq(currentNote) * fastPow2((couplingPitchMod + pitchBendSemitones) / 12.f);
        ...
        for (int n = 0; n < numSamples; ++n)
        {
            ...
            // --- Mix osc + bio ---
            float mixedSignal = oscOut * (1.f - effectiveBioMix * 0.5f) + bioSample * effectiveBioMix;
