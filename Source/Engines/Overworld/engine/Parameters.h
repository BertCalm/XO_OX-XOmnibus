...
struct ParamSnapshot
{
    ...
    int echoDelay = 0;
    float echoFeedback = 0.0f, echoMix = 0.0f;
    float echoFir[8] = {};
    ...
    float filterCutoff = 8000.0f, filterReso = 0.5f;
    int filterType = 0;
    float crushBits = 16.0f;
    int crushRate = 1;
    float crushMix = 0.0f;
    float eraDriftRate = 0.0f, eraDriftDepth = 0.0f;
    int eraDriftShape = 0;
...
inline void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
{
    auto add = [&](const char* id, const char* name, float lo, float hi, float def)
    {
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{id, 1}, name, juce::NormalisableRange<float>(lo, hi, 0.01f), def));
    };
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{id, 1}, nm, juce::NormalisableRange<float>(-1.0f, 1.0f, 0.01f), (i == 0) ? 0.7f : 0.0f));
    }
