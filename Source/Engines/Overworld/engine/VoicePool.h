...
struct ChipWavetables {
    ...
    void build(float) {
        ...
        for (int slot = 4; slot < 8; ++slot) {
            int harm = slot - 2;
            for (int i = 0; i < kLen; ++i) {
                float raw = std::sin(twoPi * harm * i / kLen) * 0.8f
                          + std::sin(twoPi * (harm + 1) * i / kLen) * 0.2f;
                float q4 = std::round((raw + 1.0f) * 7.5f);
                q4 = std::max(0.0f, std::min(15.0f, q4));
...
struct ChipVoice {
    ...
    float processNES(const ParamSnapshot& snap, float eraX) {
        ...
        static const float kTriTable[32] = {
            15/15.0f, 14/15.0f, 13/15.0f, 12/15.0f, 11/15.0f, 10/15.0f,
             9/15.0f,  8/15.0f,  7/15.0f,  6/15.0f,  5/15.0f,  4/15.0f,
             3/15.0f,  2/15.0f,  1/15.0f,  0/15.0f,
             0/15.0f,  1/15.0f,  2/15.0f,  3/15.0f,  4/15.0f,  5/15.0f,
             6/15.0f,  7/15.0f,  8/15.0f,  9/15.0f, 10/15.0f, 11/15.0f,
            12/15.0f, 13/15.0f, 14/15.0f, 15/15.0f
        };
        ...
        static const float kNoisePeriods[16] = {
            4.0f, 8.0f, 16.0f, 32.0f, 64.0f, 96.0f, 128.0f, 160.0f,
    ...
    float processGenesisFM(const ParamSnapshot& snap, float eraX) {
        ...
        if (carPhase >= 1.0f) carPhase -= std::floor(carPhase);
    ...
    float processSNES(const ParamSnapshot& snap, float eraX) {
        ...
        float s0a = gWavetables.snes[slot0][im1] * g0
                  + gWavetables.snes[slot0][i0]  * g1
                  + gWavetables.snes[slot0][i1]  * g2
                  + gWavetables.snes[slot0][i2]  * g3;
        float s0b = gWavetables.snes[slot1][im1] * g0
                  + gWavetables.snes[slot1][i0]  * g1
                  + gWavetables.snes[slot1][i1]  * g2
                  + gWavetables.snes[slot1][i2]  * g3;
    ...
    float processGameBoy(const ParamSnapshot& snap, float eraX) {
        ...
        float waveOut = lerp(gWavetables.gb[waveSlot][wi0],
                             gWavetables.gb[waveSlot][wi1], wf);
    ...
    //--------------------------------------------------------------------------
    float process(float eraX, float eraY, float ghostEra, float ghostEraY,
                  float memMix, const ParamSnapshot& snap)
    {
        ...
        if (eraLo > 5) { eraLo = 5; eraHi = 5; eraT = 0.0f; }
        if (eraHi > 5) eraHi = 5;
        ...
        auto getSample = [&](int era, float sx) -> float {
            switch (era) {
                case 0: return processNES(snap, sx);
                case 1: return processGenesisFM(snap, sx);
                case 2: return processSNES(snap, sx);
                case 3: return processGameBoy(snap, sx);
                case 4: return processPCEngine(snap, sx);
                case 5: return processNeoGeo(snap, sx);
                default: return 0.0f;
            }
        };
        ...
        if (memMix > 0.001f) {
            ...
            if (ghostEraLo > 5) { ghostEraLo = 5; ghostEraHi = 5; ghostEraT = 0.0f; }
            if (ghostEraHi > 5)  ghostEraHi = 5;
    ...
    void noteOff(int midiNote) {
        for (auto& v : voices) {
            if (v.active && v.note == midiNote
                && v.envStage != ChipVoice::EnvStage::Release
                && v.envStage != ChipVoice::EnvStage::Idle) {
                v.noteOff();
            }
        }
