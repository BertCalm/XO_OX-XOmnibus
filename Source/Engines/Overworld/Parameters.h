...
inline void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
{
    ...
    // ERA extended
    params.push_back(
        std::make_unique<P>(PI{ParamID::ERA_DRIFT_RATE, 1}, "ERA Drift Rate", NR(0.f, 10.f, 0.001f), 0.1f));
    params.push_back(
        std::make_unique<P>(PI{ParamID::ERA_DRIFT_DEPTH, 1}, "ERA Drift Depth", NR(0.f, 1.f, 0.001f), 0.0f));
    params.push_back(std::make_unique<P>(PI{ParamID::ERA_DRIFT_SHAPE, 1}, "ERA Drift Shape", NR(0.f, 3.f, 1.f), 0.0f));
    params.push_back(
        std::make_unique<P>(PI{ParamID::ERA_PORTA_TIME, 1}, "ERA Porta Time", NR(0.f, 2.f, 0.001f), 0.05f));
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        params.push_back(
            std::make_unique<P>(juce::ParameterID{id, 1}, name, NR(-1.f, 1.f, 0.001f), i == 0 ? 0.7f : 0.0f));
    }
    ...
    // Filter
    params.push_back(
        std::make_unique<P>(PI{ParamID::FILTER_CUTOFF, 1}, "Filter Cutoff", NR(20.f, 20000.f, 1.f, 0.25f), 8000.f));
