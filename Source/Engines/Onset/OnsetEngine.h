...
public:
    ...
    float process() noexcept
    {
        ...
        if (spikeSamplesRemaining <= 0 && noiseSamplesRemaining <= 0) active = false;
        return out;
    }

    void reset() noexcept { active = false; spikeSamplesRemaining = 0; noiseSamplesRemaining = 0; }
...
public:
    ...
    float process() noexcept
    {
        ...
        float freqMultiplier = (pitchEnvelopeSemitones > 0.01f)
            ? fastExp (pitchEnvelopeSemitones * (0.693147f / 12.0f)) : 1.0f;
    ...
    void reset() noexcept { phase = 0.0; subPhase = 0.0; pitchEnvelopeSemitones = 0.0f; active = false; }
...
public:
    ...
    void reset() noexcept
    {
        bodyPhase1 = 0.0; bodyPhase2 = 0.0; noiseEnvelopeLevel = 0.0f;
        bodyPitchSemitones = 0.0f; active = false; clapBurstsRemaining = 0;
        noiseHPF.reset();
    }
    bool isActive() const noexcept { return active; }
...
public:
    ...
    void trigger (float pitchRatio, float toneAmt, float charAmt) noexcept
    {
        ...
        static constexpr float kBaseFrequencies[kNumMetallicOscillators] =
            { 205.3f, 304.4f, 369.6f, 522.7f, 800.0f, 1048.0f };
    ...
    void reset() noexcept
    {
        for (auto& p : oscillatorPhases) p = 0.0;
        active = false;
        lowBandpass.reset(); highBandpass.reset(); outputHighpass.reset();
    }
    bool isActive() const noexcept { return active; }
...
public:
    ...
    float process() noexcept
    {
        ...
        float modulatorSignal = fastSin (static_cast<float> (modulatorPhase) * 6.28318530718f)
                              * modulationIndex * modulatorEnvelopeLevel;
        ...
        float out = fastSin (static_cast<float> (carrierPhase) * 6.28318530718f
                           + modulatorSignal + lastCarrierOutput * feedbackAmount);
    ...
    void reset() noexcept
    {
        carrierPhase = 0.0; modulatorPhase = 0.0;
        modulatorEnvelopeLevel = 0.0f; lastCarrierOutput = 0.0f;
        active = false;
    }
    bool isActive() const noexcept { return active; }
...
public:
    ...
    void prepare (double sampleRate) noexcept
    {
        ...
        for (auto& resonator : resonators) resonator.setMode (CytomicSVF::Mode::BandPass);
        reset();
    }
    ...
    void trigger (float baseFreqHz, float charAmt, float toneAmt, float snapAmt) noexcept
    {
        ...
        static constexpr float kMembraneRatios[kNumModes] =
        ...
        for (int i = 0; i < kNumModes; ++i)
        {
            ...
            float modeRatio = kMembraneRatios[i]
                * fastExp (logInharmonicity * (static_cast<float> (i) * 0.1f));
...
public:
    ...
    float process() noexcept
    {
        ...
        float filtered = averagingFilterCoeff * currentSample
                       + (1.0f - averagingFilterCoeff) * lastFilterOutput;
...
public:
    ...
    float process() noexcept
    {
        ...
        if (dcwEnvelopeLevel < 1e-6f && dcwAmount < 0.01f) return 0.0f;
    ...
    void reset() noexcept { phase = 0.0; dcwEnvelopeLevel = 0.0f; active = false; }
    bool isActive() const noexcept { return active; }
...
public:
    ...
    void process (float& left, float& right, float timeSec, float feedback, float mix) noexcept
    {
        ...
        if (readPosition < 0) readPosition += maxDelaySamples;
...
//
// Coupling interface (XOmnibus integration):
...
public:
    ...
    struct VoiceCfg { int circuit; int defaultAlgorithm; int midiNote; float baseFreq; float defaultBlend; float defaultDecay; bool isClap; };
                      ...
                      juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        // atomic loads. This is the XOmnibus-wide ParamSnapshot convention.
        ...
        for (int v = 0; v < kNumVoices; ++v)
        {
            ...
            vDecay[v] = clamp ((vp.decay ? vp.decay->load() : kVoiceCfg[v].defaultDecay) + couplingDecayMod, 0.001f, 8.0f);
...
private:
    ...
    int noteToVoice (int midiNoteNumber) const noexcept
    {
        for (int i = 0; i < kNumVoices; ++i)
            if (kVoiceCfg[i].midiNote == midiNoteNumber) return i;
        // GM note 35 (Acoustic Bass Drum) is an alternate trigger for the kick (V1)
        if (midiNoteNumber == 35) return 0;
        return -1;
    }
