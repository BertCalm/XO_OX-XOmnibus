...
public:
    ...
    juce::Colour getAccentColour() const override { return juce::Colour (0xFFDAA520); }
                            ...
                            const float* buf, int numSamples) override
    {
        ...
        switch (type)
        {
            ...
            case CouplingType::EnvToMorph:   couplingFormantMod += val; break;
            default: break;
        }
    }
                      ...
                      int numSamples) override
    {
        ...
        float effectiveCutoff = std::clamp (pCutoff + macroChar * 4000.0f
            - warmthRolloff * 4000.0f + seasonBrightness * 2000.0f
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                float freq = baseFreq * PitchBendUtil::semitonesToFreqRatio (
                    bendSemitones + couplingPitchMod + vibrato * 0.1f
                ...
                float fCut = std::clamp (cutNow + envLevel * pFilterEnvAmt * 6000.0f
                ...
                if (voice.velocity > accumulators.aThreshold)
                    network.sendStress (&voice - voices.data(),
                                       voice.velocity * accumulators.A * 0.1f,
