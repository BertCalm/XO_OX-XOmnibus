...
struct OvenHammerModel
{
    ...
    float process() noexcept
    {
        ...
        if (reboundActive && sampleCounter >= reboundSample && sampleCounter < reboundSample + contactSamples / 2)
        {
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        for (const auto metadata : midi)
        {
            ...
            else if (msg.isController())
            {
                ...
                if (cc == 1)
                    modWheelAmount = ccVal; // mod wheel
                else if (cc == 64)
                    handleDamperPedal(ccVal >= 0.5f); // damper/sustain
            }
        }
        ...
        float effectiveHardness = std::clamp(pHardness + macroCharacter * 0.3f, 0.0f, 1.0f);
        float effectiveSympathetic = std::clamp(pSympathetic + macroCoupling * 0.4f, 0.0f, 1.0f);
        ...
        if (thermalTimer > static_cast<int>(srf * 6.0f)) // new target every ~6 seconds (slow thermal mass)
        {
            ...
            thermalTarget = (static_cast<float>(thermalNoiseState & 0xFFFF) / 32768.0f - 1.0f) * pTemperature *
                            5.0f; // max +-5 cents (cast iron is MASSIVE)
            thermalTimer = 0;
        }
