...
{
    void trigger (float velocity, float hardness, float baseFreq,
                  float fragility, int instrument, float sampleRate) noexcept
    {
        active = true;
        sampleCounter = 0;
        ...
        switch (instrument)
        {
            case 0: contactMs = 4.0f - hardness * 3.0f; break;     // Celesta: felt hammer
            case 1: contactMs = 2.0f - hardness * 1.5f; break;     // Toy Piano: short hard hit
            case 2: contactMs = 12.0f - hardness * 8.0f; break;    // Glass Harp: slow friction
            default: contactMs = 1.5f - hardness * 1.0f; break;    // Porcelain: spoon tap
        }
    ...
    float process() noexcept
    {
        ...
        if (crackTriggered && sampleCounter < crackSamples)
        {
            ...
            float crackEnv = std::exp (-3.0f * static_cast<float> (sampleCounter) / static_cast<float> (std::max (crackSamples, 1)));
...
public:
                            ...
                            const float* buf, int numSamples) override
    {
        ...
        switch (type) {
            ...
            case CouplingType::EnvToMorph:   couplingInstrumentMod += val; break;
            default: break;
        }
    }
                      ...
                      int numSamples) override
    {
        ...
        float effectiveHammer = std::clamp (pHammerHard + macroCharacter * 0.5f
        ...
        if (thermalTimer > static_cast<int> (srf * 3.0f))  // new target every ~3 seconds
        {
            ...
            thermalTarget = (static_cast<float> (thermalNoiseState & 0xFFFF) / 32768.0f - 1.0f)
                          * pThermal * 12.0f;  // max +/-12 cents (glass is thermally sensitive)
            thermalTimer = 0;
        }
        ...
        switch (instrument)
        {
            case 0:  ratioTable = kCelestaRatios;    break;
            case 1:  ratioTable = kToyPianoRatios;   break;
            case 2:  ratioTable = kGlassHarpRatios;  break;
            default: ratioTable = kPorcelainRatios;  break;
        }
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                for (int m = 0; m < kNumModes; ++m)
                {
                    ...
                    if (voice.modes[m].amplitude < OpalineVoice::kPruneThreshold
                        && !voice.exciter.active)
                    {
                        continue;
                    }
                    ...
                    if (voice.cracked)
                        modeQ *= (1.0f - voice.crackIntensity * 0.5f * static_cast<float> (m) / static_cast<float> (kNumModes));
                ...
                if (hfNoiseNow > 0.001f && voice.exciter.active)
                {
                    ...
                    voice.hfNoiseSVF.setCoefficients (
                        std::clamp (baseFreq * 6.0f, 2000.0f, 16000.0f), 0.4f, srf);
    ...
    void noteOn (int note, float vel) noexcept
    {
        ...
        if (v.cracked)
        {
            ...
            v.crackDetuning = (static_cast<float> (crackSeed & 0xFFFF) / 32768.0f - 1.0f)
                             * 25.0f * v.crackIntensity;
        }
        else
        {
    ...
    static void addParametersImpl (std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back (std::make_unique<PI> (juce::ParameterID { "opal2_instrument", 1 },
            "Opaline Instrument", 0, 3, 0));

        // Pillar 3: Fragility — how easily the glass cracks under hard velocity
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_fragility", 1 },
            "Opaline Fragility",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        // Hammer/striker hardness
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_hammerHardness", 1 },
            "Opaline Hammer Hardness",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.4f));

        // Brightness (filter cutoff)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_brightness", 1 },
            "Opaline Brightness",
            juce::NormalisableRange<float> (200.0f, 20000.0f, 0.0f, 0.3f), 10000.0f));

        // Damping — how quickly modes lose energy
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_damping", 1 },
            "Opaline Damping",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.1f));

        // Decay time
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_decay", 1 },
            "Opaline Decay",
            juce::NormalisableRange<float> (0.1f, 15.0f, 0.0f, 0.4f), 3.0f));

        // Body size (pitch scaling — smaller body = higher pitch)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_bodySize", 1 },
            "Opaline Body Size",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Inharmonicity stretch (beam stiffness)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_inharmonicity", 1 },
            "Opaline Inharmonicity",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // Shimmer: crystalline beating between modes
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_shimmerAmount", 1 },
            "Opaline Shimmer",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        // Pillar 4: Thermal shock sensitivity
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_thermalShock", 1 },
            "Opaline Thermal Shock",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.2f));

        // HF noise character (stochastic upper partial simulation)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_hfNoise", 1 },
            "Opaline HF Noise",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        // Filter envelope amount
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_filterEnvAmount", 1 },
            "Opaline Filter Env",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        // Crystal drive (subtle waveshaping)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_crystalDrive", 1 },
            "Opaline Crystal Drive",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // Sustain pedal (CC64 latch or continuous)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_sustainPedal", 1 },
            "Opaline Sustain Pedal",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // Pitch bend range
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_bendRange", 1 },
            "Opaline Pitch Bend Range",
            juce::NormalisableRange<float> (1.0f, 24.0f, 1.0f), 2.0f));

        // Macros (4 standard: CHARACTER, MOVEMENT, COUPLING, SPACE)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_macroCharacter", 1 },
            "Opaline Macro CHARACTER",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_macroMovement", 1 },
            "Opaline Macro MOVEMENT",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_macroCoupling", 1 },
            "Opaline Macro COUPLING",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_macroSpace", 1 },
            "Opaline Macro SPACE",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // LFO1 (D002 / D005 compliance)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_lfo1Rate", 1 },
            "Opaline LFO1 Rate",
            juce::NormalisableRange<float> (0.005f, 20.0f, 0.0f, 0.3f), 0.3f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_lfo1Depth", 1 },
            "Opaline LFO1 Depth",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.1f));
        params.push_back (std::make_unique<PI> (juce::ParameterID { "opal2_lfo1Shape", 1 },
            "Opaline LFO1 Shape", 0, 4, 0));

        // LFO2
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_lfo2Rate", 1 },
            "Opaline LFO2 Rate",
            juce::NormalisableRange<float> (0.005f, 20.0f, 0.0f, 0.3f), 0.8f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "opal2_lfo2Depth", 1 },
            "Opaline LFO2 Depth",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PI> (juce::ParameterID { "opal2_lfo2Shape", 1 },
            "Opaline LFO2 Shape", 0, 4, 0));
    }
