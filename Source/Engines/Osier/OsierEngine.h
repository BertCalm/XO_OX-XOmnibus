...
struct CompanionPlanting
{
    ...
    void update(const bool* voiceActive, float companionStrength, float blockSizeSec) noexcept
    {
        int pairIdx = 0;
        for (int i = 0; i < 4; ++i)
        {
...
public:
    ...
    void noteOn(int note, float vel) noexcept
    {
        ...
        if (note >= 67)
            v.role = OsierQuartetRole::Soprano;
        else if (note >= 62)
            v.role = OsierQuartetRole::Alto;
        else if (note >= 55)
            v.role = OsierQuartetRole::Tenor;
        else
            v.role = OsierQuartetRole::Bass;
