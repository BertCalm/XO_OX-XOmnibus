...
public:
    ...
    void setFrequency(double frequencyHz) noexcept
    {
        if (frequencyHz > 0.0 && sampleRate > 0.0)
        {
            ...
            delaySamples = std::max(2.0, std::min(static_cast<double>(kMaxDelaySamples - 1), sampleRate / frequencyHz));
        }
    }

    void setDamping(float newDamping) noexcept { damping = std::max(0.0f, std::min(1.0f, newDamping)); }
...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        const int oscillatorModeIndex = (pOscMode != nullptr) ? static_cast<int>(pOscMode->load()) : 0;
        ...
        const int maxPolyphony = (pPolyphony != nullptr) ? (1 << std::min(3, static_cast<int>(pPolyphony->load()))) : 4;
        ...
        const float effectiveCutoff =
            std::max(20.0f, std::min(20000.0f, filterCutoff + macroSurface * 4000.0f + macroCharacter * 4000.0f));
        ...
        const float effectivePanSpread = std::max(0.0f, std::min(1.0f, 0.3f + macroDepth * 0.7f + macroSpace * 0.7f));

        // M2 MOVEMENT: LFO rate boost (×1 to ×4) so MacroSystem MOVEMENT knob animates Snap
        const float effectiveLfoRate = std::max(0.01f, std::min(20.0f, lfoRate * (1.0f + macroMovement * 3.0f)));
        ...
        for (const auto metadata : midi)
        {
            ...
            {
                silenceGate.wake();
                noteOn(message.getNoteNumber(), message.getFloatVelocity(), effectiveSnap, effectiveDecay, pitchLocked,
                       sweepDirection, effectiveDetune, unisonCount, effectiveCutoff, effectiveResonance, maxPolyphony,
                       oscillatorModeIndex);
            }
        ...
        for (int sampleIndex = 0; sampleIndex < numSamples; ++sampleIndex)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                float frequency = voice.cachedFreq * PitchBendUtil::semitonesToFreqRatio(pitchBendNorm * 2.0f);
                ...
                for (int unisonIndex = 0; unisonIndex < activeUnisonCount; ++unisonIndex)
                {
                    ...
                    case 1: // FM Percussion (DX7-lineage metallic transients)
                    {
                        ...
                        voice.fmModulatorOscillator.setFrequency(detunedFrequency * kFmModulatorRatio, sampleRateFloat);
                        voice.fmModulatorOscillator.setWaveform(PolyBLEP::Waveform::Sine);
                        float modulatorSignal =
                            voice.fmModulatorOscillator.processSample() * effectiveSnap * kFmIndexScale;

                        float modulatedFrequency = detunedFrequency + modulatorSignal * detunedFrequency;
                        if (modulatedFrequency < 0.0f)
                            modulatedFrequency = 0.0f;
                    ...
                    static constexpr float kUnisonPanSpread =
                        1.0f; // base multiplier; actual spread is effectivePanSpread
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        // ---- Oscillator mode ----
        params.push_back(
            std::make_unique<juce::AudioParameterChoice>(juce::ParameterID{"snap_oscMode", 1}, "Snap Osc Mode",
                                                         juce::StringArray{"Sine+Noise", "FM", "Karplus-Strong"}, 0));

        // ---- Core sound parameters ----
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{"snap_snap", 1}, "Snap", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.4f));

        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{"snap_decay", 1}, "Snap Decay", juce::NormalisableRange<float>(0.0f, 8.0f, 0.001f, 0.3f),
            0.5f));
        ...
        // ---- Unison & tuning ----
        params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_detune", 1}, "Snap Detune",
                                                                     juce::NormalisableRange<float>(0.0f, 50.0f, 0.1f),
                                                                     10.0f));

        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{"snap_level", 1}, "Snap Level", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.8f));

        params.push_back(std::make_unique<juce::AudioParameterBool>(juce::ParameterID{"snap_pitchLock", 1},
                                                                    "Snap Pitch Lock", false));
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID{"snap_polyphony", 1}, "Snap Polyphony", juce::StringArray{"1", "2", "4", "8"}, 2));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_lfoDepth", 1}, "Snap LFO Depth",
        ...
        // At default (0.08) behaviour is identical to pre-fix presets.
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_lfo2Depth", 1}, "Snap LFO2 Pan Depth",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_macroDart", 1}, "Snap Dart",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_macroSchool", 1}, "Snap School",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_macroSurface", 1}, "Snap Surface",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"snap_macroDepth", 1}, "Snap Depth",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
...
private:
    ...
    void noteOn(int noteNumber, float velocity, float snapAmount, float decayTime, bool pitchLocked, float sweepDir,
                float detuneCents, int unisonCount, float cutoffFrequency, float resonance, int maxPolyphony,
                int oscillatorModeIndex)
    {
