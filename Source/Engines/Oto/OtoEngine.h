...
//==============================================================================
constexpr float kShoAmps[11] = {1.0f, 0.9f, 0.85f, 0.8f, 0.75f, 0.65f, 0.6f, 0.5f, 0.4f, 0.3f, 0.25f};
constexpr float kShengAmps[11] = {1.0f, 0.7f, 0.5f, 0.35f, 0.25f, 0.18f, 0.13f, 0.09f, 0.06f, 0.04f, 0.03f};
constexpr float kKheneAmps[11] = {1.0f, 0.95f, 0.7f, 0.65f, 0.5f, 0.45f, 0.35f, 0.3f, 0.2f, 0.15f, 0.1f};
constexpr float kMelodicaAmps[11] = {1.0f, 0.5f, 0.33f, 0.25f, 0.2f, 0.16f, 0.14f, 0.12f, 0.0f, 0.0f, 0.0f};
...
struct OtoChiffGenerator
{
    void trigger(float velocity, float chiffAmount, int organModel, float sampleRate) noexcept
    {
        ...
        case 0:
            durationMs = 40.0f;
            break; // Sho: slow, breathy
        case 1:
            durationMs = 15.0f;
            break; // Sheng: sharp, quick
        case 2:
            durationMs = 25.0f;
            break; // Khene: medium, buzzy
        default:
            durationMs = 35.0f;
            break; // Melodica: airy
        }
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (int vi = 0; vi < kMaxVoices; ++vi)
            {
                ...
                auto synthesiseModel = [&](int model, bool usePrev) -> float
                {
                    ...
                    {
                    case 0:
                        mRatios = kShoAitakeRatios;
                        mAmps = kShoAmps;
                        mPartials = 11;
                        break;
                    case 1:
                        mRatios = kShengRatios;
                        mAmps = kShengAmps;
                        mPartials = 11;
                        break;
                    case 2:
                        mRatios = kKheneRatios;
                        mAmps = kKheneAmps;
                        mPartials = 11;
                        break;
                    default:
                        mRatios = kMelodicaRatios;
                        mAmps = kMelodicaAmps;
                        mPartials = 8;
                        break;
                    }
                    ...
                    for (int p = 0; p < nP; ++p)
                    {
                        ...
                        {
                            float frac = (clusterNow * static_cast<float>(mPartials - 1)) - static_cast<float>(nP - 1);
