...
public:
    ...
    void setFrequency(float hz) noexcept { inc = std::clamp(static_cast<float>(hz / sr), 0.0f, 0.5f); }
...
public:
    ...
    void processBlock(float* outBuffer, int numSamples, const ParamSnapshot& snap, const OxytocinMemory& memory,
                      int voiceIndex, int totalVoices) noexcept
    {
        if (!isActive())
            return;
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            float iEnv =
                intimacyEnv.tick(LoveEnvelopeType::Intimacy, snap.passionRate, snap.warmthRate, snap.commitRate);
            float cEnv =
                commitmentEnv.tick(LoveEnvelopeType::Commitment, snap.passionRate, snap.warmthRate, snap.commitRate);
