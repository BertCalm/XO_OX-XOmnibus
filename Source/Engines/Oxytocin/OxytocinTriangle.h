...
public:
    ...
    static LoveCoords fromCharacterPosition(float t) noexcept
    {
        ...
        for (int i = 0; i < N - 1; ++i)
        {
            if (t <= kf[i + 1].t)
            {
                ...
                return {kf[i].I + frac * (kf[i + 1].I - kf[i].I), kf[i].P + frac * (kf[i + 1].P - kf[i].P),
    ...
    static LoveType detectType(float I, float P, float C) noexcept
    {
        ...
        if (C > 0.85f && I < 0.2f && P < 0.2f)
            return LoveType::Obsession;
        if (I < 0.3f && P < 0.3f && C < 0.3f)
            return LoveType::NonLove;
        if (I > 0.3f && P > 0.3f && C > 0.3f)
            return LoveType::Consummate;
        if (I > 0.5f && P < 0.3f && C < 0.3f)
            return LoveType::Liking;
        if (P > 0.5f && I < 0.3f && C < 0.3f)
            return LoveType::Infatuation;
        if (C > 0.5f && I < 0.3f && P < 0.3f)
            return LoveType::EmptyLove;
        if (I > 0.4f && P > 0.4f && C < 0.3f)
            return LoveType::Romantic;
        if (I > 0.4f && C > 0.4f && P < 0.3f)
            return LoveType::Companionate;
        if (P > 0.4f && C > 0.4f && I < 0.3f)
            return LoveType::Fatuous;
        return LoveType::NonLove;
    }
    ...
    static LoveCoords coordsForLockType(int lockType) noexcept
    {
        ...
        {
        case 1:
            return {0.70f, 0.15f, 0.15f}; // Liking
        case 2:
            return {0.10f, 0.80f, 0.10f}; // Infatuation
        case 3:
            return {0.10f, 0.10f, 0.80f}; // Empty Love
        case 4:
            return {0.45f, 0.45f, 0.10f}; // Romantic
        case 5:
            return {0.45f, 0.10f, 0.45f}; // Companionate
        case 6:
            return {0.10f, 0.45f, 0.45f}; // Fatuous
        case 7:
            return {0.34f, 0.33f, 0.33f}; // Consummate
        case 8:
            return {0.05f, 0.05f, 0.90f}; // Obsession
        default:
