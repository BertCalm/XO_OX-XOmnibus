...
enum class LoveEnvelopeType { Passion, Intimacy, Commitment };
...
public:
    ...
    void noteOn()  noexcept { stage = Stage::Attack;  phase = 0.0f; }
    void noteOff() noexcept { if (stage != Stage::Idle) { stage = Stage::Release; phase = 0.0f; releaseStart = value; } }
                ...
                float commitRate)    // seconds
    {
        ...
        switch (type)
        {
            case LoveEnvelopeType::Passion:    tickPassion    (passionRate); break;
            case LoveEnvelopeType::Intimacy:   tickIntimacy   (warmthRate);  break;
            case LoveEnvelopeType::Commitment: tickCommitment (commitRate);  break;
        }
