...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        int activePartials = std::max(1, std::min(OverwashVoice::kNumPartials, static_cast<int>(pHarmonics)));
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            for (int v = 0; v < kMaxVoices; ++v)
            {
                ...
                const float voiceNormT = (pDiffTime > 0.0f) ? std::min(voiceDiffT / pDiffTime, 1.0f) : 0.0f;
                ...
                for (int p = 0; p < activePartials; ++p)
                {
                    ...
                    float partialContrib = sine * partial.amplitude * diffusionAttenuation * warmthAtten * velBright;
    ...
    {
        if (bin >= 0 && bin < 32)
            return spectralField[static_cast<size_t>(bin)];
        return 0.0f;
    }
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_diffusionRate", 1}, "Diffusion Rate",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_viscosity", 1}, "Viscosity",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_harmonics", 1}, "Harmonic Density",
                                              juce::NormalisableRange<float>(1.0f, 16.0f, 1.0f), 12.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_spreadMax", 1}, "Spread Maximum",
                                              juce::NormalisableRange<float>(10.0f, 500.0f, 0.0f, 0.4f), 200.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_diffusionTime", 1}, "Diffusion Time",
                                              juce::NormalisableRange<float>(1.0f, 60.0f, 0.0f, 0.4f), 10.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_interference", 1}, "Interference",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.3f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_brightness", 1}, "Brightness",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.7f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_warmth", 1}, "Warmth",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_stereoWidth", 1}, "Stereo Width",
        ...
        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_ampDecay", 1}, "Amp Decay",
                                              juce::NormalisableRange<float>(0.01f, 10.0f, 0.0f, 0.3f), 1.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_ampSustain", 1}, "Amp Sustain",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.8f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_ampRelease", 1}, "Amp Release",
                                              juce::NormalisableRange<float>(0.01f, 15.0f, 0.0f, 0.3f), 2.0f));

        // Filter ADSR
        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_filtAttack", 1}, "Filter Attack",
                                              juce::NormalisableRange<float>(0.001f, 5.0f, 0.0f, 0.3f), 0.1f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_filtDecay", 1}, "Filter Decay",
                                              juce::NormalisableRange<float>(0.01f, 10.0f, 0.0f, 0.3f), 0.5f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_filtSustain", 1}, "Filter Sustain",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.4f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_filtRelease", 1}, "Filter Release",
                                              juce::NormalisableRange<float>(0.01f, 10.0f, 0.0f, 0.3f), 1.0f));

        // LFOs
        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_lfo1Rate", 1}, "LFO 1 Rate",
                                              juce::NormalisableRange<float>(0.005f, 20.0f, 0.0f, 0.3f), 0.1f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_lfo1Depth", 1}, "LFO 1 Depth",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.2f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_lfo2Rate", 1}, "LFO 2 Rate",
                                              juce::NormalisableRange<float>(0.005f, 10.0f, 0.0f, 0.3f), 0.05f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_lfo2Depth", 1}, "LFO 2 Depth",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.15f));

        // Output
        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_level", 1}, "Level",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.8f));

        // Macros
        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_macroCharacter", 1}, "Character",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_macroMovement", 1}, "Movement",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_macroCoupling", 1}, "Coupling",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"wash_macroSpace", 1}, "Space",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));
    }
