...
//
//  Coupling (XOmnibus inter-engine modulation):
...
public:
                      ...
                      int numSamples) override
    {
        ...
        switch (voiceModeIndex)
        {
            ...
            case 2: maxPolyphony = 8; break;                                        // Poly8
            case 3: maxPolyphony = 16; break;                                       // Poly16
            default: maxPolyphony = 8; break;
        }
        ...
        // intentional CPU/quality tradeoff per XOmnibus Architecture Rules.
        for (auto& voice : voices)
        {
        ...
        for (int sampleIndex = 0; sampleIndex < numSamples; ++sampleIndex)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                if (voice.phaseStage1 >= 1.0f) voice.phaseStage1 -= 1.0f;
                ...
                float finalPhaseLeft  = voice.phaseStage1 + depthEnvelopeAmount * (distortedPhaseLeft - voice.phaseStage1);
                float finalPhaseRight = voice.phaseStage1 + depthEnvelopeAmount * (distortedPhaseRight - voice.phaseStage1);
                ...
                if (effectiveCascade > 0.001f)
                {
                    ...
                    if (voice.phaseStage2 >= 1.0f) voice.phaseStage2 -= 1.0f;
                    ...
                    float distortedPhase2Right = lookupDistortion (
                        clamp (stage2Density + stereoOffset, 0.0f, 1.0f),
    ...
    float getSampleForCoupling (int channel, int sampleIndex) const override
    {
        ...
        if (channel == 2) return envelopeFollowerOutput;    // Envelope follower for amplitude-driven coupling
        return 0.0f;
    }
    ...
    void attachParameters (juce::AudioProcessorValueTreeState& apvts) override
    {
        ...
        pFormantResonance      = apvts.getRawParameterValue ("obsidian_formantResonance"); // P0-04 fix: was colliding with pFormantIntensity
        ...
        for (int densityIndex = 0; densityIndex < kLutDensitySteps; ++densityIndex)
        {
            ...
            for (int tiltIndex = 0; tiltIndex < kLutTiltSteps; ++tiltIndex)
            {
                ...
                for (int phaseIndex = 0; phaseIndex < kLutPhaseSteps; ++phaseIndex)
                {
                    ...
                    float distortedPhase = phase * (1.0f - blendFactor)
                                         + (tiltedPhase * (1.0f - blendFactor * 0.5f)
                                            + foldedPhase * blendFactor * 0.5f)
                                         * blendFactor;
                    ...
                    distortionLUT[static_cast<size_t> (densityIndex)]
                                 [static_cast<size_t> (tiltIndex)]
