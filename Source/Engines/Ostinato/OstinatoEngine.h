...
{
    Djembe = 0, Dundun, Conga, Bongos, Cajon, Taiko,
    Tabla, Doumbek, FrameDrum, Surdo, TongueDrum, Beatbox,
    Count = 12
};
...
static constexpr OstiInstrumentData kInstrumentTable[static_cast<int>(OstiInstrument::Count)] =
{
    ...
    // Modes: classic Bessel zeros for a clamped circular membrane
    {
        ...
        8, BodyModelType::Cylindrical, 150.0f, 4, 8.0f, 0.65f,
        {
    ...
    // Modes: slightly detuned from ideal membrane (thick hide)
    {
        ...
        8, BodyModelType::Cylindrical, 80.0f, 4, 12.0f, 0.70f,
        {
    ...
    // Modes: stretched circular membrane (high tension)
    {
        ...
        8, BodyModelType::Cylindrical, 200.0f, 4, 6.5f, 0.55f,
        {
    ...
    // Modes: small diameter = higher fundamental, tighter mode spacing
    {
        ...
        6, BodyModelType::Conical, 350.0f, 3, 3.5f, 0.40f,
        {
    ...
    // Modes: rectangular membrane modes (different from circular)
    {
        ...
        8, BodyModelType::Box, 100.0f, 4, 5.0f, 0.50f,
        {
    ...
    // Modes: compressed ratio spacing due to thick hide + barrel body
    {
        ...
        8, BodyModelType::Cylindrical, 60.0f, 4, 15.0f, 0.75f,
        {
    ...
    // Raman (1934): the syahi forces modes into near-harmonic series
    {
        ...
        6, BodyModelType::Conical, 260.0f, 4, 4.0f, 0.35f,
        {
    ...
    // Modes: similar to djembe but brighter due to thinner membrane
    {
        ...
        8, BodyModelType::Conical, 220.0f, 4, 4.5f, 0.45f,
        {
    ...
    // Modes: similar to ideal circular membrane (thin, well-tensioned)
    {
        ...
        8, BodyModelType::Open, 120.0f, 4, 2.0f, 0.20f,
        {
    ...
    // Modes: large diameter shifts everything lower, more energy in fundamental
    {
        ...
        6, BodyModelType::Cylindrical, 55.0f, 3, 14.0f, 0.72f,
        {
    ...
    // Modes: near-harmonic (designed tongues act like tuning forks)
    {
        ...
        6, BodyModelType::Open, 330.0f, 3, 1.5f, 0.15f,
        {
    ...
    // Modes: non-physical, designed for electronic character
    {
        ...
        6, BodyModelType::Open, 100.0f, 4, 1.0f, 0.10f,
        {
...
public:
    ...
    void prepare (double sampleRate) noexcept
    {
        ...
        for (auto& r : resonators) r.setMode (CytomicSVF::Mode::BandPass);
        reset();
    }
                  ...
                  float userExciterMix = 0.5f) noexcept
    {
        ...
        {
            float modeFreq = clamp (
                baseFreqHz * inst.modeRatios[i] * tuneMultiplier,
                20.0f, sr * 0.45f
            );
            ...
            float modeQ = clamp (
                0.990f * inst.modeDecays[i] + brightness * 0.008f + art.brightnessOffset * 0.005f,
                0.5f, 0.9999f
            );
            ...
            modeAmplitudes[i] = lerp (1.0f - modePos * 0.6f, 0.5f + modePos * 0.5f, brightness)
                              * inst.modeDecays[i];
        }
...
public:
    ...
    void configure (BodyModelType type, float bodyDelayMs, float reflection,
                    float bodyAmount, float brightness) noexcept
    {
        bodyType = type;
        ...
        reflectionFilter.setCoefficients (
            clamp (reflectionCutoff, 200.0f, sr * 0.45f), 0.2f, sr
        );
        allpassFilter.setCoefficients (
            clamp (allpassFreq, 100.0f, sr * 0.45f), 0.5f, sr
        );
    }

    float process (float input) noexcept
    {
        if (currentBodyAmount < 0.001f) return input;
        ...
        if (bodyType == BodyModelType::Box)
        {
            ...
            if (tap2Pos < 0) tap2Pos += kMaxDelay;
            int tap3Pos = writePos - (delayLength / 3);
            if (tap3Pos < 0) tap3Pos += kMaxDelay;
...
static const OstiPattern kPatternLibrary[12][8] =
{
    // INSTRUMENT 0: DJEMBE (tone=0, slap=1, bass=2, mute=3)
    {
        ...
        {{ {0.8f,2}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.4f,0}, {0.3f,3},
            {0.7f,2}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.4f,0}, {0.0f,0} }},
        // 1: Variation with slaps
        {{ {0.7f,2}, {0.0f,0}, {0.4f,0}, {0.5f,1}, {0.0f,0}, {0.6f,1}, {0.3f,0}, {0.0f,0},
            {0.8f,2}, {0.0f,0}, {0.4f,0}, {0.5f,1}, {0.0f,0}, {0.6f,1}, {0.4f,3}, {0.0f,0} }},
        // 2: Fill — dense
        {{ {0.9f,2}, {0.4f,0}, {0.6f,1}, {0.3f,0}, {0.7f,1}, {0.4f,0}, {0.5f,0}, {0.6f,1},
            {0.8f,2}, {0.3f,0}, {0.7f,1}, {0.4f,0}, {0.6f,1}, {0.5f,0}, {0.7f,1}, {0.8f,2} }},
        // 3: Sparse ambient
        {{ {0.6f,2}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        // 4: Kuku rhythm approximation
        {{ {0.7f,2}, {0.0f,0}, {0.5f,0}, {0.6f,1}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.4f,0},
            {0.7f,2}, {0.5f,1}, {0.0f,0}, {0.5f,0}, {0.6f,1}, {0.0f,0}, {0.4f,0}, {0.0f,0} }},
        // 5: Djembe muted groove
        {{ {0.5f,3}, {0.3f,3}, {0.5f,0}, {0.0f,0}, {0.6f,3}, {0.3f,3}, {0.5f,0}, {0.0f,0},
            {0.5f,3}, {0.3f,3}, {0.5f,0}, {0.4f,1}, {0.6f,3}, {0.3f,3}, {0.5f,0}, {0.0f,0} }},
        // 6: Sangban-style
        {{ {0.8f,2}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0},
            {0.7f,2}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.4f,0} }},
        // 7: Double-time
        {{ {0.7f,2}, {0.4f,0}, {0.6f,1}, {0.4f,0}, {0.7f,2}, {0.4f,0}, {0.6f,1}, {0.4f,0},
            {0.7f,2}, {0.4f,0}, {0.6f,1}, {0.4f,0}, {0.7f,2}, {0.4f,0}, {0.6f,1}, {0.5f,3} }},
    },

    // INSTRUMENT 1: DUNDUN (open=0, mute=1, bell-on=2, bell-off=3)
    {
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0},
            {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0} }},
        {{ {0.7f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.5f,2}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.5f,3}, {0.0f,0} }},
        {{ {0.9f,0}, {0.4f,2}, {0.6f,0}, {0.5f,2}, {0.7f,0}, {0.4f,2}, {0.6f,0}, {0.5f,2},
            {0.8f,0}, {0.4f,2}, {0.6f,1}, {0.5f,2}, {0.7f,0}, {0.5f,2}, {0.6f,0}, {0.8f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.7f,0}, {0.4f,2}, {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.4f,2}, {0.0f,0}, {0.0f,0},
            {0.7f,0}, {0.4f,2}, {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.4f,2}, {0.0f,0}, {0.5f,3} }},
        {{ {0.6f,1}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.5f,2}, {0.0f,0},
            {0.6f,1}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.5f,2}, {0.4f,3} }},
        {{ {0.8f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.4f,2},
            {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.6f,0}, {0.4f,2}, {0.0f,0} }},
        {{ {0.7f,0}, {0.3f,2}, {0.5f,0}, {0.3f,2}, {0.7f,0}, {0.3f,2}, {0.5f,0}, {0.3f,2},
            {0.7f,0}, {0.3f,2}, {0.5f,0}, {0.3f,2}, {0.7f,0}, {0.3f,2}, {0.5f,0}, {0.4f,3} }},
    },

    // INSTRUMENT 2: CONGA (open=0, slap=1, mute=2, fingertip=3)
    {
        {{ {0.7f,0}, {0.0f,0}, {0.5f,0}, {0.6f,1}, {0.0f,0}, {0.4f,0}, {0.0f,0}, {0.5f,0},
            {0.7f,0}, {0.0f,0}, {0.5f,0}, {0.6f,1}, {0.0f,0}, {0.4f,0}, {0.3f,3}, {0.0f,0} }},
        {{ {0.6f,0}, {0.3f,3}, {0.5f,1}, {0.0f,0}, {0.6f,0}, {0.3f,3}, {0.5f,1}, {0.0f,0},
            {0.6f,0}, {0.3f,3}, {0.5f,1}, {0.0f,0}, {0.7f,0}, {0.4f,3}, {0.5f,1}, {0.4f,3} }},
        {{ {0.8f,0}, {0.4f,3}, {0.6f,1}, {0.3f,3}, {0.7f,0}, {0.4f,3}, {0.6f,1}, {0.5f,0},
            {0.8f,0}, {0.4f,3}, {0.7f,1}, {0.3f,3}, {0.6f,0}, {0.5f,3}, {0.7f,1}, {0.6f,0} }},
        {{ {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,3}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,0}, {0.0f,0}, {0.0f,0}, {0.3f,3}, {0.0f,0} }},
        {{ {0.7f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.4f,2}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.5f,0}, {0.0f,0} }},
        {{ {0.5f,2}, {0.3f,2}, {0.5f,0}, {0.0f,0}, {0.5f,2}, {0.3f,2}, {0.5f,0}, {0.0f,0},
            {0.5f,2}, {0.3f,2}, {0.5f,0}, {0.4f,1}, {0.5f,2}, {0.3f,2}, {0.5f,0}, {0.0f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0},
            {0.6f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.4f,3} }},
        {{ {0.7f,0}, {0.4f,3}, {0.6f,0}, {0.4f,3}, {0.7f,1}, {0.4f,3}, {0.6f,0}, {0.4f,3},
            {0.7f,0}, {0.4f,3}, {0.6f,0}, {0.4f,3}, {0.7f,1}, {0.4f,3}, {0.6f,0}, {0.5f,1} }},
    },

    // INSTRUMENT 3: BONGOS (open=0, slap=1, mute=2)
    {
        {{ {0.7f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.5f,0}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.5f,0}, {0.4f,2} }},
        {{ {0.6f,0}, {0.4f,0}, {0.5f,1}, {0.0f,0}, {0.6f,0}, {0.4f,0}, {0.5f,1}, {0.3f,2},
            {0.6f,0}, {0.4f,0}, {0.5f,1}, {0.0f,0}, {0.6f,0}, {0.4f,0}, {0.5f,1}, {0.3f,2} }},
        {{ {0.8f,0}, {0.5f,0}, {0.6f,1}, {0.4f,0}, {0.7f,1}, {0.5f,0}, {0.6f,0}, {0.4f,1},
            {0.8f,0}, {0.5f,0}, {0.7f,1}, {0.4f,0}, {0.6f,0}, {0.5f,1}, {0.7f,0}, {0.6f,1} }},
        {{ {0.4f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.4f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0},
            {0.6f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.5f,1}, {0.4f,0} }},
        {{ {0.5f,2}, {0.4f,2}, {0.5f,0}, {0.4f,2}, {0.5f,2}, {0.4f,2}, {0.5f,0}, {0.4f,2},
            {0.5f,2}, {0.4f,2}, {0.5f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.5f,0}, {0.4f,2} }},
        {{ {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.4f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.4f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.3f,2} }},
        {{ {0.6f,0}, {0.3f,0}, {0.5f,1}, {0.3f,0}, {0.6f,0}, {0.3f,0}, {0.5f,1}, {0.3f,0},
            {0.6f,0}, {0.3f,0}, {0.5f,1}, {0.3f,0}, {0.7f,0}, {0.3f,0}, {0.6f,1}, {0.4f,0} }},
    },

    // INSTRUMENT 4: CAJON (bass=0, slap=1, ghost=2, fingertip=3)
    {
        {{ {0.8f,0}, {0.0f,0}, {0.3f,2}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.3f,2}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.3f,2}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.3f,2}, {0.4f,3} }},
        {{ {0.7f,0}, {0.3f,2}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.3f,2}, {0.7f,0}, {0.0f,0},
            {0.3f,2}, {0.6f,1}, {0.0f,0}, {0.3f,2}, {0.7f,0}, {0.0f,0}, {0.6f,1}, {0.3f,2} }},
        {{ {0.9f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.7f,1}, {0.4f,2}, {0.6f,0}, {0.4f,2},
            {0.8f,0}, {0.5f,2}, {0.7f,1}, {0.4f,2}, {0.6f,0}, {0.5f,2}, {0.7f,1}, {0.8f,0} }},
        {{ {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.0f,0}, {0.3f,2},
            {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.3f,2}, {0.7f,1}, {0.0f,0}, {0.0f,0}, {0.3f,2} }},
        {{ {0.6f,0}, {0.3f,3}, {0.0f,0}, {0.5f,1}, {0.3f,3}, {0.0f,0}, {0.6f,0}, {0.3f,3},
            {0.0f,0}, {0.5f,1}, {0.3f,3}, {0.0f,0}, {0.6f,0}, {0.3f,3}, {0.5f,1}, {0.0f,0} }},
        {{ {0.7f,0}, {0.0f,0}, {0.4f,2}, {0.5f,1}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.4f,2}, {0.5f,1}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.4f,3} }},
        {{ {0.7f,0}, {0.3f,2}, {0.5f,1}, {0.3f,2}, {0.7f,0}, {0.3f,2}, {0.5f,1}, {0.3f,2},
            {0.7f,0}, {0.3f,2}, {0.5f,1}, {0.3f,2}, {0.8f,0}, {0.3f,2}, {0.6f,1}, {0.4f,2} }},
    },

    // INSTRUMENT 5: TAIKO (center=0, edge=1, rim=2, flam=3)
    {
        {{ {0.9f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.9f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.9f,3}, {0.6f,0}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.6f,0}, {0.7f,0}, {0.0f,0},
            {0.9f,3}, {0.6f,0}, {0.7f,0}, {0.0f,0}, {0.7f,0}, {0.6f,0}, {0.8f,0}, {0.9f,3} }},
        {{ {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0} }},
        {{ {0.9f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.9f,0}, {0.0f,0},
            {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.9f,0}, {0.0f,0}, {0.0f,0}, {0.7f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.7f,0}, {0.0f,0},
            {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.8f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0} }},
        {{ {0.8f,0}, {0.5f,0}, {0.7f,0}, {0.5f,0}, {0.8f,0}, {0.5f,0}, {0.7f,0}, {0.5f,0},
            {0.9f,3}, {0.5f,0}, {0.7f,0}, {0.5f,0}, {0.8f,0}, {0.5f,0}, {0.7f,0}, {0.6f,0} }},
    },

    // INSTRUMENT 6: TABLA (na=0, tin=1, tun=2, ge=3)
    {
        {{ {0.6f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.4f,0}, {0.5f,1}, {0.0f,0}, {0.3f,0},
            {0.6f,2}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.4f,0}, {0.5f,1}, {0.0f,0}, {0.4f,3} }},
        {{ {0.5f,0}, {0.4f,0}, {0.6f,1}, {0.0f,0}, {0.5f,0}, {0.4f,0}, {0.6f,1}, {0.3f,3},
            {0.5f,0}, {0.4f,0}, {0.6f,1}, {0.0f,0}, {0.5f,2}, {0.4f,0}, {0.6f,1}, {0.3f,3} }},
        {{ {0.7f,0}, {0.5f,0}, {0.6f,1}, {0.4f,0}, {0.6f,0}, {0.5f,1}, {0.6f,0}, {0.4f,3},
            {0.7f,2}, {0.5f,0}, {0.6f,1}, {0.4f,0}, {0.6f,0}, {0.5f,1}, {0.7f,0}, {0.6f,3} }},
        {{ {0.4f,2}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,1}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,3}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.6f,0}, {0.3f,0}, {0.5f,1}, {0.3f,0}, {0.6f,2}, {0.3f,0}, {0.5f,1}, {0.3f,3},
            {0.6f,0}, {0.3f,0}, {0.5f,1}, {0.3f,0}, {0.6f,2}, {0.3f,0}, {0.5f,1}, {0.4f,3} }},
        {{ {0.5f,3}, {0.0f,0}, {0.5f,3}, {0.0f,0}, {0.5f,3}, {0.0f,0}, {0.5f,3}, {0.0f,0},
            {0.5f,3}, {0.0f,0}, {0.5f,3}, {0.0f,0}, {0.6f,3}, {0.0f,0}, {0.5f,3}, {0.0f,0} }},
        {{ {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0},
            {0.5f,2}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.4f,3} }},
        {{ {0.6f,0}, {0.4f,0}, {0.5f,1}, {0.4f,0}, {0.5f,2}, {0.4f,0}, {0.5f,1}, {0.4f,3},
            {0.6f,0}, {0.4f,0}, {0.5f,1}, {0.4f,0}, {0.6f,2}, {0.4f,0}, {0.5f,1}, {0.5f,3} }},
    },

    // INSTRUMENT 7: DOUMBEK (doum=0, tek=1, ka=2, snap=3)
    {
        {{ {0.8f,0}, {0.0f,0}, {0.5f,1}, {0.4f,2}, {0.0f,0}, {0.5f,1}, {0.4f,2}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.5f,1}, {0.4f,2}, {0.0f,0}, {0.5f,1}, {0.4f,2}, {0.0f,0} }},
        {{ {0.7f,0}, {0.4f,2}, {0.5f,1}, {0.0f,0}, {0.6f,0}, {0.4f,2}, {0.5f,1}, {0.3f,3},
            {0.7f,0}, {0.4f,2}, {0.5f,1}, {0.0f,0}, {0.6f,0}, {0.4f,2}, {0.5f,1}, {0.3f,3} }},
        {{ {0.8f,0}, {0.5f,1}, {0.5f,2}, {0.5f,1}, {0.7f,0}, {0.5f,1}, {0.5f,2}, {0.5f,1},
            {0.8f,0}, {0.5f,1}, {0.6f,2}, {0.5f,1}, {0.7f,0}, {0.5f,3}, {0.6f,1}, {0.7f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.7f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.4f,2},
            {0.7f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.4f,2}, {0.0f,0} }},
        {{ {0.6f,0}, {0.3f,2}, {0.5f,1}, {0.3f,2}, {0.6f,0}, {0.3f,2}, {0.5f,1}, {0.3f,2},
            {0.6f,0}, {0.3f,2}, {0.5f,1}, {0.3f,3}, {0.6f,0}, {0.3f,2}, {0.5f,1}, {0.3f,2} }},
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.4f,3} }},
        {{ {0.7f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.7f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2},
            {0.7f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.8f,0}, {0.4f,3}, {0.6f,1}, {0.5f,2} }},
    },

    // INSTRUMENT 8: FRAME DRUM (open=0, edge=1, finger-roll=2, slap=3)
    {
        {{ {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.4f,1}, {0.3f,2},
            {0.6f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.5f,3}, {0.0f,0}, {0.4f,1}, {0.0f,0} }},
        {{ {0.8f,0}, {0.4f,1}, {0.5f,0}, {0.4f,1}, {0.6f,3}, {0.4f,1}, {0.5f,0}, {0.5f,2},
            {0.7f,0}, {0.4f,1}, {0.5f,0}, {0.4f,1}, {0.6f,3}, {0.4f,1}, {0.6f,0}, {0.7f,3} }},
        {{ {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,2}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0},
            {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0} }},
        {{ {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0},
            {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.6f,2}, {0.0f,0}, {0.5f,2}, {0.4f,0} }},
        {{ {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.5f,3}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.5f,3}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,1} }},
        {{ {0.6f,0}, {0.3f,1}, {0.5f,0}, {0.3f,1}, {0.6f,3}, {0.3f,1}, {0.5f,0}, {0.3f,1},
            {0.6f,0}, {0.3f,1}, {0.5f,0}, {0.3f,1}, {0.7f,3}, {0.3f,1}, {0.5f,0}, {0.4f,2} }},
    },
    ...
    // INSTRUMENT 9: SURDO (open=0, mute=1, rim=2)
    {
        {{ {0.9f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.9f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.5f,2}, {0.6f,0},
            {0.8f,0}, {0.0f,0}, {0.5f,2}, {0.6f,0}, {0.7f,0}, {0.5f,2}, {0.6f,0}, {0.8f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.6f,1}, {0.0f,0}, {0.4f,2}, {0.0f,0} }},
        {{ {0.7f,1}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.7f,1}, {0.0f,0},
            {0.7f,1}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.7f,0}, {0.0f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.7f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.5f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.8f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.9f,0}, {0.0f,0}, {0.7f,0}, {0.5f,2} }},
    },

    // INSTRUMENT 10: TONGUE DRUM (strike=0, mallet=1, harmonic=2)
    {
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.5f,1}, {0.0f,0}, {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0},
            {0.0f,0}, {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.4f,1}, {0.0f,0} }},
        {{ {0.7f,0}, {0.4f,0}, {0.5f,2}, {0.4f,0}, {0.6f,0}, {0.4f,0}, {0.5f,2}, {0.4f,0},
            {0.7f,0}, {0.4f,0}, {0.5f,2}, {0.4f,0}, {0.6f,0}, {0.5f,2}, {0.6f,0}, {0.5f,2} }},
        {{ {0.4f,1}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.3f,2}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.5f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0},
            {0.5f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0}, {0.5f,0}, {0.0f,0}, {0.4f,2}, {0.0f,0} }},
        {{ {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0},
            {0.5f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.6f,2}, {0.0f,0}, {0.5f,2}, {0.0f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.5f,1}, {0.0f,0}, {0.0f,0},
            {0.0f,0}, {0.0f,0}, {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,2} }},
        {{ {0.5f,0}, {0.3f,1}, {0.4f,0}, {0.3f,1}, {0.5f,2}, {0.3f,1}, {0.4f,0}, {0.3f,1},
            {0.5f,0}, {0.3f,1}, {0.4f,0}, {0.3f,1}, {0.6f,2}, {0.3f,1}, {0.5f,0}, {0.4f,1} }},
    },

    // INSTRUMENT 11: BEATBOX (kick=0, snare=1, hi-hat=2, fx=3)
    {
        {{ {0.9f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.5f,2}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.5f,2}, {0.4f,2} }},
        {{ {0.8f,0}, {0.0f,0}, {0.4f,2}, {0.5f,2}, {0.7f,1}, {0.0f,0}, {0.4f,2}, {0.5f,2},
            {0.8f,0}, {0.4f,3}, {0.4f,2}, {0.5f,2}, {0.7f,1}, {0.0f,0}, {0.4f,2}, {0.5f,2} }},
        {{ {0.9f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.7f,0}, {0.4f,2}, {0.6f,1}, {0.5f,2},
            {0.8f,0}, {0.4f,2}, {0.7f,1}, {0.4f,2}, {0.6f,3}, {0.5f,2}, {0.7f,1}, {0.8f,0} }},
        {{ {0.6f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0},
            {0.5f,0}, {0.0f,0}, {0.0f,0}, {0.0f,0}, {0.4f,3}, {0.0f,0}, {0.0f,0}, {0.0f,0} }},
        {{ {0.8f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.5f,2}, {0.3f,2},
            {0.8f,0}, {0.0f,0}, {0.5f,2}, {0.0f,0}, {0.7f,1}, {0.0f,0}, {0.5f,2}, {0.4f,3} }},
        {{ {0.7f,0}, {0.3f,2}, {0.0f,0}, {0.5f,1}, {0.3f,2}, {0.0f,0}, {0.7f,0}, {0.3f,2},
            {0.0f,0}, {0.5f,1}, {0.3f,2}, {0.0f,0}, {0.7f,0}, {0.3f,2}, {0.6f,1}, {0.3f,2} }},
        {{ {0.8f,0}, {0.0f,0}, {0.4f,2}, {0.5f,3}, {0.7f,1}, {0.0f,0}, {0.4f,2}, {0.0f,0},
            {0.8f,0}, {0.0f,0}, {0.4f,2}, {0.5f,3}, {0.7f,1}, {0.0f,0}, {0.4f,2}, {0.5f,3} }},
        {{ {0.8f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.8f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2},
            {0.8f,0}, {0.4f,2}, {0.6f,1}, {0.4f,2}, {0.9f,0}, {0.4f,2}, {0.7f,1}, {0.5f,3} }},
    },
};
...
public:
    ...
    void advance (int numSamples, double bpm, float swing, float gatherAmount,
                  float patternVolume, float humanize) noexcept
    {
        if (pattern == nullptr || patternVolume < 0.001f) return;
...
struct OstiSubVoice
{
    ...
    void trigger (float vel, int instrument, int articulation,
                  float tuning, float decay, float brightness,
                  float bodyAmount, int bodyModelOverride,
                  float exciterMix, float pitchEnvAmount, float velSens) noexcept
    {
        ...
        BodyModelType bType = (bodyModelOverride >= 0 && bodyModelOverride <= 3)
            ? static_cast<BodyModelType> (bodyModelOverride) : inst.bodyType;
    ...
    void reset() noexcept { choke(); breathLFO.reset(); }
};
...
struct OstiSeat
{
    ...
    void prepare (double sampleRate)
    {
        for (auto& sv : subVoices) sv.prepare (sampleRate);
        sequencer.prepare (sampleRate);
        nextSubVoice = 0;
        peakLevel = 0.0f;
    }

    void triggerSeat (float vel, int instrument, int articulation,
                      float tuning, float decay, float brightness,
                      float bodyAmount, int bodyModelOverride,
                      float exciterMix, float pitchEnvAmount, float velSens) noexcept
    {
    ...
    void choke() noexcept
    {
        for (auto& sv : subVoices) sv.choke();
    }

    void reset() noexcept
    {
        for (auto& sv : subVoices) sv.reset();
        sequencer.reset();
        nextSubVoice = 0;
        peakLevel = 0.0f;
    }
...
public:
                      ...
                      juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        // --- Master filter ---
        masterFilterL.setCoefficients (clamp (gMasterFilt, 200.0f, static_cast<float>(sr) * 0.45f),
                                       gMasterReso, static_cast<float> (sr));
        masterFilterR.setCoefficients (clamp (gMasterFilt, 200.0f, static_cast<float>(sr) * 0.45f),
                                       gMasterReso, static_cast<float> (sr));
        ...
        for (int s = 0; s < kNumSeats; ++s)
        {
            seats[s].sequencer.advance (numSamples, gTempo, gSwing, mGather,
                                         sPatVol[s], gHumanize);
            float patVel = 0.0f;
            int patArt = 0;
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            for (int s = 0; s < kNumSeats; ++s)
            {
                ...
                if (absSamp > blockPeaks[s]) blockPeaks[s] = absSamp;
                             ...
                             const float* sourceBuffer, int numSamples) override
    {
        ...
        switch (type)
        {
            ...
            case CouplingType::RhythmToBlend: /* modulates pattern density externally */ break;
...
private:
    ...
    static void addParametersImpl (std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        static const juce::StringArray instrumentNames {
            "Djembe", "Dundun", "Conga", "Bongos", "Cajon", "Taiko",
            "Tabla", "Doumbek", "Frame Drum", "Surdo", "Tongue Drum", "Beatbox"
        };

        static const juce::StringArray bodyModelNames {
            "Cylindrical", "Conical", "Box", "Open"
        };

        static const juce::StringArray patternNames {
            "Basic", "Variation", "Fill", "Sparse",
        ...
        for (int s = 0; s < kNumSeats; ++s)
        {
            ...
            params.push_back (std::make_unique<Choice> (
                juce::ParameterID (pre + "articulation", 1), name + " Articulation",
                juce::StringArray { "Art 1", "Art 2", "Art 3", "Art 4" }, 0));

            params.push_back (std::make_unique<Float> (
                juce::ParameterID (pre + "tuning", 1), name + " Tuning",
            ...
            params.push_back (std::make_unique<Float> (
                juce::ParameterID (pre + "brightness", 1), name + " Brightness",
            ...
            params.push_back (std::make_unique<Choice> (
                juce::ParameterID (pre + "pattern", 1), name + " Pattern",
                patternNames, 0));

            params.push_back (std::make_unique<Float> (
                juce::ParameterID (pre + "patternVol", 1), name + " Pattern Vol",
                juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

            params.push_back (std::make_unique<Float> (
                juce::ParameterID (pre + "velSens", 1), name + " Vel Sens",
                juce::NormalisableRange<float> (0.0f, 1.0f), 0.7f));

            params.push_back (std::make_unique<Float> (
                juce::ParameterID (pre + "pitchEnv", 1), name + " Pitch Env",
                juce::NormalisableRange<float> (-1.0f, 1.0f), 0.0f));

            params.push_back (std::make_unique<Float> (
                juce::ParameterID (pre + "exciterMix", 1), name + " Exciter Mix",
                juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

            params.push_back (std::make_unique<Choice> (
                juce::ParameterID (pre + "bodyModel", 1), name + " Body Model",
                juce::StringArray { "Auto", "Cylindrical", "Conical", "Box", "Open" }, 0));
        }

        // ---- Macros ----
        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_macroGather", 1), "Gather",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_macroFire", 1), "Fire",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_macroCircle", 1), "Circle",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_macroSpace", 1), "Space",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // ---- Global parameters ----
        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_tempo", 1), "Osti Tempo",
        ...
        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_masterTune", 1), "Osti Master Tune",
            juce::NormalisableRange<float> (-24.0f, 24.0f), 0.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_masterDecay", 1), "Osti Master Decay",
            juce::NormalisableRange<float> (0.5f, 2.0f), 1.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_masterFilter", 1), "Osti Master Filter",
            juce::NormalisableRange<float> (200.0f, 20000.0f, 0.0f, 0.3f), 18000.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_masterReso", 1), "Osti Master Reso",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.1f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_reverbSize", 1), "Osti Reverb Size",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.4f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_reverbDamp", 1), "Osti Reverb Damp",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_reverbMix", 1), "Osti Reverb Mix",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.15f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_compThresh", 1), "Osti Comp Thresh",
            juce::NormalisableRange<float> (-40.0f, 0.0f), -12.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_compRatio", 1), "Osti Comp Ratio",
            juce::NormalisableRange<float> (1.0f, 20.0f, 0.0f, 0.5f), 4.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_compAttack", 1), "Osti Comp Attack",
            juce::NormalisableRange<float> (0.1f, 50.0f, 0.0f, 0.5f), 5.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_compRelease", 1), "Osti Comp Release",
            juce::NormalisableRange<float> (10.0f, 500.0f, 0.0f, 0.5f), 50.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_circleAmount", 1), "Osti Circle Amount",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_humanize", 1), "Osti Humanize",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        params.push_back (std::make_unique<Float> (
            juce::ParameterID ("osti_masterLevel", 1), "Osti Master Level",
