...
public:
    ...
    void prepare (double sampleRate, int maxBlockSize) override
    {
        ...
        static constexpr float baseDelaysMs[kFDNChannels] = {
            29.3f, 37.1f, 43.7f, 53.9f
        };
                      ...
                      int numSamples) override
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            if (exciterActive_ && exciterEnv_ > 0.0001f)
            {
                ...
                if (exciterPhase_ >= 1.0) exciterPhase_ -= 1.0;
                ...
                if (exciterEnv_ < 0.0001f) exciterActive_ = false;
            }
            ...
            for (int ch = 0; ch < kFDNChannels; ++ch)
            {
                int effectiveSize = clamp (static_cast<int> (fdnDelaySize_[ch] * roomScale),
                                           1, fdnDelaySize_[ch]);
    ...
    static void addParametersImpl (std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_anchor", 1 }, "Anchor",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.8f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_windChaos", 1 }, "Wind Chaos",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_undertowPull", 1 }, "Undertow Pull",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_splashSize", 1 }, "Splash Size",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Reverb shaping
        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_decay", 1 }, "Decay Time",
            juce::NormalisableRange<float> (0.1f, 30.0f, 0.0f, 0.3f), 4.0f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_damping", 1 }, "Damping",
            juce::NormalisableRange<float> (200.0f, 16000.0f, 0.0f, 0.3f), 6000.0f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_dryWet", 1 }, "Dry/Wet",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Exciter
        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_exciterDecay", 1 }, "Exciter Decay",
            juce::NormalisableRange<float> (0.001f, 0.1f, 0.0f, 0.5f), 0.02f));

        // Vacuum / Optical
        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_vacuumAttack", 1 }, "Vacuum Attack",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_opticalWarmth", 1 }, "Optical Warmth",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Macros (CHARACTER / MOVEMENT / COUPLING / SPACE)
        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_macroCharacter", 1 }, "Character",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_macroMovement", 1 }, "Movement",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_macroCoupling", 1 }, "Coupling",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        params.push_back (std::make_unique<PF> (
            juce::ParameterID { "out_macroSpace", 1 }, "Space",
            juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
    }
