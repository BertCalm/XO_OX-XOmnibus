...
struct KarplusStrongString
{
    ...
    {
        feedback = std::clamp (fb, 0.0f, 0.9999f);
    }
...
struct RunnerGenerator
{
    ...
    void checkSpawn (float velocity, float aggression, float wildness,
                     float baseFreqHz, double sessionTime) noexcept
    {
        if (active) return;  // already running
        if (velocity < 0.6f || aggression < 0.3f) return;  // not stressed enough
    ...
    float process() noexcept
    {
        ...
        timer++;
        if (timer < delayedOnset) return 0.0f;  // waiting for delayed onset
...
public:
    ...
    juce::Colour getAccentColour() const override { return juce::Colour (0xFF3A5F0B); }
                      ...
                      int numSamples) override
    {
        ...
        for (const auto& v : voices)
            if (v.active) { avgVel += v.velocity; activeCount++; }
        ...
        float effectiveCutoff = std::clamp (pCutoff + macroChar * 4000.0f
            + accumulators.getSeasonBrightness() * 2000.0f
        ...
        float effectiveVibratoDepth = pVibratoDepth + modWheelAmount * 0.5f
                                    + accumulators.getAggressionVibrato() * 0.4f;
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                if (wildNow > 0.01f)
                {
                    ...
                    pitchJitter = (static_cast<float> (ns & 0xFFFF) / 32768.0f - 1.0f)
                                 * wildNow * 0.05f;  // ±5 cents at max wildness
                }

                float freq = baseFreq * PitchBendUtil::semitonesToFreqRatio (
                    bendSemitones + couplingPitchMod + vibrato * 0.15f
                ...
                if (bowNow > 0.001f && voice.ampEnv.getStage() != FilterEnvelope::Stage::Idle)
                {
                    ...
                    float bowNoiseSignal = (static_cast<float> (bs & 0xFFFF) / 32768.0f - 1.0f)
                                          * bowNow * 0.05f;
                    stringOut += bowNoiseSignal;
                }

                // Runner generation — stressed notes sprout unexpected sub-harmonics
                voice.runner.checkSpawn (voice.velocity, accumulators.A, wildNow,
                                        freq, accumulators.sessionTime);
                ...
                if (voice.velocity > accumulators.aThreshold)
                    network.sendStress (&voice - voices.data(),
                                       voice.velocity * accumulators.A * 0.15f,
    ...
    void noteOn (int note, float vel) noexcept
    {
        ...
        v.ampEnv.setADSR (attackTime,
                          ...
                          paramSustain ? paramSustain->load() : 0.7f,
                          releaseTime);
