...
//==============================================================================
// OhmEngine — XOceanus adapter for XOhm (OHM).
//
...
struct OhmAdapterVoice
{
    ...
    OhmObedFM obed;
    OhmInLaw inlaw;
    OhmGlassPartial glass;
    OhmSpectralFreeze specFreeze;
    OhmGrainEngine grain;
...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buf, juce::MidiBuffer& midi, int ns) override
    {
        ...
        case 1:
            bodyQ = 8.0f;
            bodyFreqMul = 3.0f;
            break; // Metal: bright, ringy
        case 2:
            bodyQ = 5.0f;
            bodyFreqMul = 1.5f;
            break; // Gourd: mid bloom
        case 3:
            bodyQ = 1.5f;
            bodyFreqMul = 2.5f;
            break; // Air: diffuse
        ...
        for (int i = 0; i < ns; ++i)
        {
            ...
            for (auto& v : voices)
            {
                ...
                float exc =
                    (v.bowed ? v.bow.tick(pBowP * velIntens, pBowS, v.lastOut) : v.pick.tick(pBright * velIntens)) *
                    effIntens;
    ...
    static void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& p)
    {
        ...
        p.push_back(std::make_unique<C>("ohm_dadInstrument", "Dad Instrument",
                                        juce::StringArray{"Banjo", "Guitar", "Mandolin", "Dobro", "Fiddle", "Harmonica",
                                                          "Djembe", "Kalimba", "Sitar"},
                                        0));
