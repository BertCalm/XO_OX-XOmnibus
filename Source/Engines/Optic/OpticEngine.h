...
public:
    ...
    void prepare(double sampleRate) noexcept
    {
        ...
        static constexpr float crossoverFrequencies[kNumBands - 1] = {80.0f,   200.0f,  500.0f,  1000.0f,
        ...
        for (int bandIndex = 0; bandIndex < kNumBands; ++bandIndex)
        {
            ...
            envelopeFollowers[bandIndex].setCoefficients(30.0f, // 30 Hz envelope cutoff — tracks ~33ms transients
    ...
    void processSample(float input) noexcept
    {
        ...
        if (totalEnergy > 0.0001f)
        {
            ...
            spectralCentroid = flushDenormal(weightedBandSum / (totalEnergy * static_cast<float>(kNumBands - 1)));
        }
        else
        {
            spectralCentroid = 0.0f;
...
public:
    ...
    float process(const Params& params) noexcept
    {
        ...
        if (driftOscillatorPhase > 1.0)
            driftOscillatorPhase -= 1.0;
...
struct OpticModOutputs
{
    ...
    void store(int channelIndex, float value) noexcept { values[channelIndex].store(value, std::memory_order_relaxed); }

    float load(int channelIndex) const noexcept { return values[channelIndex].load(std::memory_order_relaxed); }
...
//
// Unique among XOceanus engines: 0 voices, 0 audio output, 16 parameters.
...
public:
    ...
    void prepare(double sampleRate, int maxBlockSize) override
    {
        ...
        for (int i = 0; i < OpticModOutputs::kNumModChannels; ++i)
        {
            ...
            modulationSmoothingFilters[i].setCoefficients(15.0f, // 15 Hz smoothing cutoff
                                                          0.0f,  // No resonance
                                                          static_cast<float>(cachedSampleRate));
        }
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        // This is the XOceanus standard pattern: read atomic parameter pointers
        ...
        for (int sampleIndex = 0; sampleIndex < numSamples; ++sampleIndex)
        {
            ...
            if (buffer.getNumChannels() > 0)
            {
                ...
                float rightChannel = buffer.getNumChannels() > 1 ? buffer.getSample(1, sampleIndex) : leftChannel;
            ...
            float midEnergy =
                (analyzer.getBandEnergy(2) + analyzer.getBandEnergy(3) + analyzer.getBandEnergy(4)) * 3.0f;
            float highEnergy =
                (analyzer.getBandEnergy(5) + analyzer.getBandEnergy(6) + analyzer.getBandEnergy(7)) * 5.0f;
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_reactivity", 1}, "Optic Reactivity",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.7f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_inputGain", 1}, "Optic Signal Level",
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID{"optic_autoPulse", 1}, "Optic AutoPulse", juce::StringArray{"Off", "On"}, 1));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_pulseShape", 1}, "Optic Pulse Shape",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.2f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_pulseSwing", 1}, "Optic Pulse Swing",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_pulseEvolve", 1}, "Optic Pulse Evolve",
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_pulseAccent", 1}, "Optic Pulse Accent",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.7f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_modDepth", 1}, "Optic Mod Depth",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.5f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_modMixPulse", 1}, "Optic Pulse Blend",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.5f));
        ...
        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_vizFeedback", 1}, "Optic Viz Feedback",
                                                        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.6f));

        params.push_back(
            std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"optic_vizSpeed", 1}, "Optic Viz Speed",
