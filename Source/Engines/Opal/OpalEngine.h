...
// OpalADSR replaced by xoceanus::StandardADSR (Source/DSP/StandardADSR.h).
...
// OpalLFO replaced by xoceanus::StandardLFO (Source/DSP/StandardLFO.h).
...
{
public:
    OpalGrain& spawn(int voiceIdx, float readPos, float pitchSemitones, float grainSizeMs, float panPos,
                     int windowShape, double sampleRate) noexcept
    {
...
struct OpalCloudVoice
{
    ...
    void noteOn(int note, float vel, double sr, float attack, float decay, float sustain, float release, float filterA,
                float filterD, float filterS, float filterR_, float prevFreq, float glideTime, bool legatoActive,
                bool lfo1Retrig, float lfo1StartPhase, bool lfo2Retrig, float lfo2StartPhase) noexcept
    {
        active = true;
        noteNumber = note;
        velocity = vel;
        ...
        if (lfo1Retrig)
            lfo1.reset(lfo1StartPhase);
        if (lfo2Retrig)
            lfo2.reset(lfo2StartPhase);
...
public:
    ...
    void process(float& inL, float& inR, float timeSec, float feedback, float spread) noexcept
    {
...
//==============================================================================
// OpalEngine — XOpal granular synthesis engine for XOceanus.
//
// Signal chain:
...
public:
    ...
    void reset() override
    {
        ...
        lastSampleL = 0.0f;
        lastSampleR = 0.0f;
        for (auto& s : extAudioBufL)
            s = 0.0f;
        for (auto& s : extAudioBufR)
            s = 0.0f;
    }
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        static const juce::StringArray modSources{"Off",     "LFO1",     "LFO2",      "Filter Env",
                                                  "Amp Env", "Velocity", "Key Track", "Mod Wheel"};
        static const juce::StringArray modDests{"Off",           "Grain Size",  "Density",       "Position",
                                                "Pitch Scatter", "Pan Scatter", "Filter Cutoff", "Shimmer",
                                                "Frost",         "Freeze",      "Osc Shape",     "Level"};
        ...
        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::OSC_SHAPE, 1}, "Opal Osc Shape", NR(0.0f, 1.0f, 0.01f), 0.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::OSC2_SHAPE, 1}, "Opal Osc 2 Shape",
                                                      NR(0.0f, 1.0f, 0.01f), 0.5f));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::OSC2_MIX, 1}, "Opal Osc 2 Mix", NR(0.0f, 1.0f, 0.01f), 0.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::OSC2_DETUNE, 1}, "Opal Osc 2 Detune",
                                                      NR(-24.0f, 24.0f, 0.01f), 0.1f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::COUPLING_LEVEL, 1}, "Opal Coupling Level",
        ...
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::EXTERNAL_MIX, 1}, "Opal External Mix",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 2. GRAIN SCHEDULER ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::GRAIN_SIZE, 1}, "Opal Grain Size",
                                                      NR(10.0f, 800.0f, 0.1f, 0.4f), 120.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::DENSITY, 1}, "Opal Density",
                                                      NR(1.0f, 120.0f, 0.1f, 0.5f), 20.0f));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::POSITION, 1}, "Opal Position", NR(0.0f, 1.0f, 0.001f), 0.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::POS_SCATTER, 1}, "Opal Pos Scatter",
                                                      NR(0.0f, 1.0f, 0.01f), 0.1f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::PITCH_SHIFT, 1}, "Opal Pitch Shift",
                                                      NR(-24.0f, 24.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::PITCH_SCATTER, 1}, "Opal Pitch Scatter",
                                                      NR(0.0f, 24.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::PAN_SCATTER, 1}, "Opal Pan Scatter",
                                                      NR(0.0f, 1.0f, 0.01f), 0.3f));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::WINDOW, 1}, "Opal Window",
                                                       juce::StringArray{"Hann", "Gaussian", "Tukey", "Rectangular"},
                                                       0));

        //==== 3. FREEZE ====
        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::FREEZE, 1}, "Opal Freeze", NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FREEZE_SIZE, 1}, "Opal Freeze Region",
                                                      NR(0.01f, 1.0f, 0.01f), 0.25f));

        //==== 4. FILTER ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_CUTOFF, 1}, "Opal Filter Cutoff",
                                                      NR(20.0f, 20000.0f, 0.1f, 0.3f), 8000.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_RESO, 1}, "Opal Filter Reso",
                                                      NR(0.0f, 1.0f, 0.01f), 0.15f));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::FILTER_MODE, 1}, "Opal Filter Mode",
                                                       juce::StringArray{"Low Pass", "Band Pass", "High Pass", "Notch"},
                                                       0));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_KEY_TRACK, 1}, "Opal Key Track",
                                                      NR(0.0f, 1.0f, 0.01f), 0.3f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_DRIVE, 1}, "Opal Filter Drive",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 5. CHARACTER ====
        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::SHIMMER, 1}, "Opal Shimmer", NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::FROST, 1}, "Opal Frost", NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 6. AMP ENVELOPE ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::AMP_ATTACK, 1}, "Opal Amp Attack",
                                                      NR(0.001f, 8.0f, 0.001f, 0.4f), 0.3f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::AMP_DECAY, 1}, "Opal Amp Decay",
                                                      NR(0.05f, 4.0f, 0.001f, 0.4f), 0.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::AMP_SUSTAIN, 1}, "Opal Amp Sustain",
                                                      NR(0.0f, 1.0f, 0.01f), 0.8f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::AMP_RELEASE, 1}, "Opal Amp Release",
                                                      NR(0.05f, 8.0f, 0.001f, 0.4f), 1.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::AMP_VEL_SENS, 1}, "Opal Velocity",
                                                      NR(0.0f, 1.0f, 0.01f), 0.4f));

        //==== 7. FILTER ENVELOPE ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_ENV_AMT, 1}, "Opal Filter Env Amt",
                                                      NR(-1.0f, 1.0f, 0.01f), 0.3f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_ATTACK, 1}, "Opal Filter Attack",
                                                      NR(0.001f, 8.0f, 0.001f, 0.4f), 0.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_DECAY, 1}, "Opal Filter Decay",
                                                      NR(0.05f, 4.0f, 0.001f, 0.4f), 0.8f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_SUSTAIN, 1}, "Opal Filter Sustain",
                                                      NR(0.0f, 1.0f, 0.01f), 0.3f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FILTER_RELEASE, 1}, "Opal Filter Release",
        ...
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::LFO1_RATE, 1}, "Opal LFO1 Rate",
                                                      NR(0.01f, 20.0f, 0.01f, 0.4f), 0.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::LFO1_DEPTH, 1}, "Opal LFO1 Depth",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::LFO1_SYNC, 1}, "Opal LFO1 Sync",
                                                       juce::StringArray{"Off", "On"}, 0));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::LFO1_RETRIGGER, 1}, "Opal LFO1 Retrig",
                                                       juce::StringArray{"Off", "On"}, 0));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::LFO1_PHASE, 1}, "Opal LFO1 Phase",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));
        ...
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::LFO2_RATE, 1}, "Opal LFO2 Rate",
                                                      NR(0.01f, 20.0f, 0.01f, 0.4f), 2.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::LFO2_DEPTH, 1}, "Opal LFO2 Depth",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::LFO2_SYNC, 1}, "Opal LFO2 Sync",
                                                       juce::StringArray{"Off", "On"}, 0));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::LFO2_RETRIGGER, 1}, "Opal LFO2 Retrig",
                                                       juce::StringArray{"Off", "On"}, 0));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::LFO2_PHASE, 1}, "Opal LFO2 Phase",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 10. MOD MATRIX (6 slots) ====
        const char* modSrcIds[] = {OpalParam::MOD1_SRC, OpalParam::MOD2_SRC, OpalParam::MOD3_SRC,
                                   OpalParam::MOD4_SRC, OpalParam::MOD5_SRC, OpalParam::MOD6_SRC};
        const char* modDstIds[] = {OpalParam::MOD1_DST, OpalParam::MOD2_DST, OpalParam::MOD3_DST,
                                   OpalParam::MOD4_DST, OpalParam::MOD5_DST, OpalParam::MOD6_DST};
        const char* modAmtIds[] = {OpalParam::MOD1_AMT, OpalParam::MOD2_AMT, OpalParam::MOD3_AMT,
                                   OpalParam::MOD4_AMT, OpalParam::MOD5_AMT, OpalParam::MOD6_AMT};

        for (int i = 0; i < kOpalModSlots; ++i)
        {
            juce::String num(i + 1);
            params.push_back(
                std::make_unique<ChoiceParam>(PID{modSrcIds[i], 1}, "Opal Mod " + num + " Src", modSources, 0));
            params.push_back(
                std::make_unique<ChoiceParam>(PID{modDstIds[i], 1}, "Opal Mod " + num + " Dst", modDests, 0));
            params.push_back(std::make_unique<FloatParam>(PID{modAmtIds[i], 1}, "Opal Mod " + num + " Amt",
                                                          NR(-1.0f, 1.0f, 0.01f), 0.0f));
        }

        //==== 11. MACROS ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::MACRO_SCATTER, 1}, "Opal SCATTER",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::MACRO_DRIFT, 1}, "Opal DRIFT", NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::MACRO_COUPLING, 1}, "Opal COUPLING",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::MACRO_SPACE, 1}, "Opal SPACE", NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 12. FX — SMEAR ====
        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::FX_SMEAR_AMT, 1}, "Opal Smear", NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_SMEAR_MIX, 1}, "Opal Smear Mix",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 13. FX — SCATTER REVERB ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_REVERB_SIZE, 1}, "Opal Reverb Size",
                                                      NR(0.0f, 1.0f, 0.01f), 0.4f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_REVERB_DECAY, 1}, "Opal Reverb Decay",
                                                      NR(0.1f, 10.0f, 0.01f, 0.4f), 2.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_REVERB_DAMP, 1}, "Opal Reverb Damp",
                                                      NR(0.0f, 1.0f, 0.01f), 0.5f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_REVERB_MIX, 1}, "Opal Reverb Mix",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        //==== 14. FX — DELAY ====
        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_DELAY_TIME, 1}, "Opal Delay Time",
                                                      NR(0.01f, 2.0f, 0.001f, 0.4f), 0.35f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_DELAY_FB, 1}, "Opal Delay FB",
                                                      NR(0.0f, 0.95f, 0.01f), 0.3f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_DELAY_MIX, 1}, "Opal Delay Mix",
                                                      NR(0.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::FX_DELAY_SYNC, 1}, "Opal Delay Sync",
                                                       juce::StringArray{"Off", "On"}, 0));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_DELAY_SPREAD, 1}, "Opal Delay Spread",
                                                      NR(0.0f, 1.0f, 0.01f), 0.3f));

        //==== 15. FX — FINISH ====
        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::FX_FINISH_GLUE, 1}, "Opal Glue", NR(0.0f, 1.0f, 0.01f), 0.1f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_FINISH_WIDTH, 1}, "Opal Width",
                                                      NR(0.0f, 2.0f, 0.01f), 1.0f));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::FX_FINISH_LEVEL, 1}, "Opal Finish Level",
                                                      NR(0.0f, 1.0f, 0.01f), 0.75f));

        //==== 16. VOICE ====
        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::VOICE_MODE, 1}, "Opal Voice Mode",
                                                       juce::StringArray{"Poly", "Mono"}, 0));

        params.push_back(std::make_unique<FloatParam>(PID{OpalParam::GLIDE_TIME, 1}, "Opal Glide",
                                                      NR(0.0f, 2.0f, 0.001f, 0.4f), 0.0f));

        params.push_back(std::make_unique<ChoiceParam>(PID{OpalParam::GLIDE_MODE, 1}, "Opal Glide Mode",
                                                       juce::StringArray{"Always", "Legato"}, 0));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::PAN, 1}, "Opal Pan", NR(-1.0f, 1.0f, 0.01f), 0.0f));

        params.push_back(
            std::make_unique<FloatParam>(PID{OpalParam::LEVEL, 1}, "Opal Level", NR(0.0f, 1.0f, 0.01f), 0.75f));
    }
    ...
    {
        if (slot < 0 || slot >= kOpalInputSlots)
            return nullptr;
    ...
    //
    void receiveAudioBuffer(const AudioRingBuffer& src, int numSamples, float mix, bool frozen) noexcept override
    {
        if (frozen)
            return; // FREEZE: hold blend cache at last valid content, skip external buffer

        if (mix < 0.001f)
            return; // external mix off — fast path
        if (src.capacity <= 0)
            return;
        ...
        for (int n = 0; n < numSamples; ++n)
        {
            ...
            float frac = static_cast<float>(numSamples - 1 - n) / static_cast<float>(src.capacity);
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        // ---- Apply macros ----
        applyMacros(grainSizeMs, density, posScatter, pitchScatter, panScatter, couplingLevel, freezeAmt, fxReverbMix,
                    fxSmearMix, fxDelayMix);
        ...
        for (const auto& msg : midi)
        {
            ...
            if (m.isNoteOn())
            {
                ...
                if (isMono)
                {
                    ...
                    voices[0].noteOn(m.getNoteNumber(), m.getFloatVelocity(), sr, ampAttack, ampDecay, ampSustain,
                                     ampRelease, filterAttack, filterDecay, filterSustain, filterRelease, prevFreq,
                                     glideTime, legato && legatoGlide, lfo1Retrig, lfo1StartPh, lfo2Retrig,
                                     lfo2StartPh);
                ...
                else
                {
                    ...
                    v.noteOn(m.getNoteNumber(), m.getFloatVelocity(), sr, ampAttack, ampDecay, ampSustain, ampRelease,
                             filterAttack, filterDecay, filterSustain, filterRelease, prevFreq, glideTime, false,
                             lfo1Retrig, lfo1StartPh, lfo2Retrig, lfo2StartPh);
            ...
            // producing a diffuse, shimmering iridescent cloud.
                ...
                pitchBendNorm = PitchBendUtil::parsePitchWheel(m.getPitchWheelValue());
        }
        ...
        if (externalMix > 0.001f)
        {
            ...
            for (int slot = 0; slot < kOpalInputSlots; ++slot)
            {
                ...
                if (rb.capacity <= 0)
                    continue;
                receiveAudioBuffer(rb, numSamples, externalMix / static_cast<float>(kOpalInputSlots), frozen);
            }
        }
        ...
        for (int n = 0; n < numSamples; ++n)
        {
            ...
            for (auto& v : voices)
            {
                ...
                while (v.triggerAccum >= triggerInterval)
                {
                    ...
                    spawnGrainForVoice(v, position, posScatter, pitchShift, pitchScatter, panScatter, grainSizeMs,
...
private:
    ...
    void spawnGrainForVoice(OpalCloudVoice& v, float position, float posScatter, float pitchShift, float pitchScatter,
                            float panScatter, float grainSizeMs, int windowShape, float basePan, bool frozen,
                            float freezeRegion) noexcept
    {
        ...
        float scatterRange = frozen ? (freezeRegion * static_cast<float>(bufSize)) : static_cast<float>(bufSize);
        ...
        grainPool.spawn(v.voiceIndex, readPos, pShift, grainSizeMs, panPos, windowShape, sr);
    }
                        ...
                        int activeCount, float avgVelocity, float avgNote) noexcept
    {
        ...
        for (int slot = 0; slot < kOpalModSlots; ++slot)
        {
            ...
            if (src == 0 || dst == 0 || std::fabs(amt) < 0.001f)
                continue;
            ...
            case 5:
                ...
                break; // Velocity (0..1)
            case 6:
                srcVal = keyTrackValue;
                break; // KeyTrack (bipolar, centred on C4)
            case 7:
                srcVal = modWheelAmount;
                break; // ModWheel (0..1)
            }
