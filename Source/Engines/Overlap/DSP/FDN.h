...
public:
    ...
    void setMatrix(const KnotMatrix::Matrix& m) noexcept { matrix = m; }

    //==========================================================================
    // setDelayBase() — recalculate delay lengths from base (ms) × ratios
    void setDelayBase(float baseMs, double /*sampleRate*/, const std::array<float, 6>& ratios) noexcept
    {
        for (int i = 0; i < kChannels; ++i)
        {
    ...
    void process(const std::array<float, 6>& fdnInputs) noexcept
    {
        ...
        for (int i = 0; i < kChannels; ++i)
        {
            dampState[static_cast<size_t>(i)] =
                dampAlpha * dampState[static_cast<size_t>(i)] + (1.0f - dampAlpha) * delayed[static_cast<size_t>(i)];
        ...
        for (int i = 0; i < kChannels; ++i)
        {
            ...
            for (int j = 0; j < kChannels; ++j)
                sum += matrix[static_cast<size_t>(i)][static_cast<size_t>(j)] * delayed[static_cast<size_t>(j)];
        ...
        for (int i = 0; i < kChannels; ++i)
        {
            ...
            if (++writePos[i] >= static_cast<int>(buf.size()))
                writePos[i] = 0;
        }
    ...
    float getOutput(int channel) const noexcept { return outputs[static_cast<size_t>(channel)]; }
