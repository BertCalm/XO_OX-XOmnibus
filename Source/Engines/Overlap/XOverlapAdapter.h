...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        if (params.lfo1Depth > 0.001f)
            applyLFOModulation(lfo1Val, params.lfo1Depth, params.lfo1Dest, modTangleDepth, modDampening, modPulseRate,
                               modDelayBase, modFilterCutoff, modSpread, modBioluminescence, modEntrain);
        if (params.lfo2Depth > 0.001f)
            applyLFOModulation(lfo2Val, params.lfo2Depth, params.lfo2Dest, modTangleDepth, modDampening, modPulseRate,
                               modDelayBase, modFilterCutoff, modSpread, modBioluminescence, modEntrain);
        ...
        // Apply expression routing
        {
            ...
            {
            case 0:
                modTangleDepth = clamp(modTangleDepth + mwMod, 0.0f, 1.0f);
                break;
            case 1:
                modEntrain = clamp(modEntrain + mwMod, 0.0f, 1.0f);
                break;
            case 2:
                modBioluminescence = clamp(modBioluminescence + mwMod, 0.0f, 1.0f);
                break;
            case 3:
                modFilterCutoff = clamp(modFilterCutoff * fastPow2(mwMod * 3.0f), 20.0f, 20000.0f);
                break;
            default:
                break;
            }
            ...
            {
            case 0:
                modTangleDepth = clamp(modTangleDepth + atMod, 0.0f, 1.0f);
                break;
            case 1:
                modEntrain = clamp(modEntrain + atMod, 0.0f, 1.0f);
                break;
            case 2:
                modBioluminescence = clamp(modBioluminescence + atMod, 0.0f, 1.0f);
                break;
            case 3:
                modPulseRate = juce::jmax(0.01f, modPulseRate * (1.0f + atMod));
                break;
            default:
                break;
            }
        ...
        // drift of ±olap_current semitones.  One block of advance is enough resolution.
        {
            ...
            if (currentDriftPhase >= 1.0f)
                currentDriftPhase -= 1.0f;
        }
        ...
        {
        case 0:
            knotMat = xoverlap::KnotMatrix::unknot();
            break;
        case 1:
            knotMat = xoverlap::KnotMatrix::trefoil();
            break;
        case 2:
            knotMat = xoverlap::KnotMatrix::figureEight();
            break;
        case 3:
            knotMat = xoverlap::KnotMatrix::torus(params.torusP, params.torusQ);
            break;
        default:
            knotMat = xoverlap::KnotMatrix::unknot();
            break;
        ...
        effectiveCutoff = clamp(effectiveCutoff, 20.0f, juce::jmin(20000.0f, static_cast<float>(sr) * 0.45f));
        ...
        for (int sample = 0; sample < numSamples; ++sample)
        {
            ...
            auto bioStereo =
                biolum.processStereo(fdnMono, modDelayBase, modBioluminescence * (0.2f + params.brightness * 0.8f));
            left += bioStereo.left;
    ...
    //==========================================================================
    float getSampleForCoupling(int ch, int /*sample*/) const override { return ch == 0 ? lastSampleL : lastSampleR; }
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& p)
    {
        // Topology
        p.push_back(std::make_unique<juce::AudioParameterChoice>(
            "olap_knot", "Knot Type", juce::StringArray{"Unknot", "Trefoil", "Figure-Eight", "Torus"}, 0));
        ...
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_pulseRate", "Pulse Rate", juce::NormalisableRange<float>(0.01f, 8.0f, 0.0f, 0.3f), 0.5f));
        ...
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_glide", "Glide Time", juce::NormalisableRange<float>(0.0f, 500.0f, 0.0f, 0.4f), 0.0f));

        // FDN
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_delayBase", "Delay Base", juce::NormalisableRange<float>(1.0f, 50.0f, 0.0f, 0.4f), 10.0f));
        ...
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_currentRate", "Current Rate", juce::NormalisableRange<float>(0.005f, 0.5f, 0.0f, 0.3f), 0.03f));

        // Envelope
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_attack", "Attack", juce::NormalisableRange<float>(0.001f, 2.0f, 0.0f, 0.3f), 0.05f));
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_decay", "Decay", juce::NormalisableRange<float>(0.01f, 30.0f, 0.0f, 0.3f), 1.0f));
        p.push_back(std::make_unique<juce::AudioParameterFloat>("olap_sustain", "Sustain",
                                                                juce::NormalisableRange<float>(0.0f, 1.0f), 0.7f));
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_release", "Release", juce::NormalisableRange<float>(0.01f, 10.0f, 0.0f, 0.3f), 2.0f));

        // Filter
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_filterCutoff", "Filter Cutoff", juce::NormalisableRange<float>(20.0f, 20000.0f, 0.0f, 0.25f),
            8000.0f));
        ...
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_filterEnvDecay", "Filter Env Decay", juce::NormalisableRange<float>(0.01f, 5.0f, 0.0f, 0.3f), 0.5f));

        // LFOs
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_lfo1Rate", "LFO 1 Rate", juce::NormalisableRange<float>(0.01f, 20.0f, 0.0f, 0.3f), 0.8f));
        p.push_back(std::make_unique<juce::AudioParameterChoice>(
            "olap_lfo1Shape", "LFO 1 Shape", juce::StringArray{"Sine", "Triangle", "Saw", "Square", "S&H"}, 0));
        ...
        p.push_back(std::make_unique<juce::AudioParameterChoice>("olap_lfo1Dest", "LFO 1 Dest",
                                                                 juce::StringArray{"Tangle", "Dampening", "Pulse Rate",
                                                                                   "Delay", "Filter", "Spread",
                                                                                   "Bioluminescence", "Entrainment"},
                                                                 0));
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_lfo2Rate", "LFO 2 Rate", juce::NormalisableRange<float>(0.01f, 20.0f, 0.0f, 0.3f), 0.15f));
        p.push_back(std::make_unique<juce::AudioParameterChoice>(
            "olap_lfo2Shape", "LFO 2 Shape", juce::StringArray{"Sine", "Triangle", "Saw", "Square", "S&H"}, 0));
        ...
        p.push_back(std::make_unique<juce::AudioParameterFloat>("olap_chorusMix", "Chorus Mix",
                                                                juce::NormalisableRange<float>(0.0f, 1.0f), 0.2f));
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            "olap_chorusRate", "Chorus Rate", juce::NormalisableRange<float>(0.01f, 0.5f, 0.0f, 0.3f), 0.08f));
        ...
        // Expression
        p.push_back(std::make_unique<juce::AudioParameterChoice>(
            "olap_modWheelDest", "Mod Wheel Dest", juce::StringArray{"Tangle", "Entrain", "Bioluminescence", "Filter"},
            0));
        p.push_back(std::make_unique<juce::AudioParameterFloat>("olap_modWheelDepth", "Mod Wheel Depth",
                                                                juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f));
        p.push_back(std::make_unique<juce::AudioParameterChoice>(
            "olap_atPressureDest", "Aftertouch Dest",
...
private:
    ...
    float computeLFO(float phase, int shape, float& shValue) noexcept
    {
        ...
        {
        case 0:
            return fastSin(phase * 6.28318530f);
        case 1:
    ...
    void applyLFOModulation(float lfoValue, float depth, int dest, float& tangleDepth, float& dampening,
                            float& pulseRate, float& delayBase, float& filterCutoff, float& spread,
                            float& bioluminescence, float& entrain) noexcept
    {
        ...
        {
        case 0:
            tangleDepth = clamp(tangleDepth + mod * 0.5f, 0.0f, 1.0f);
            break;
        case 1:
            dampening = clamp(dampening + mod * 0.5f, 0.0f, 1.0f);
            break;
        case 2:
            pulseRate = juce::jmax(0.01f, pulseRate * (1.0f + mod * 0.5f));
            break;
        case 3:
            delayBase = juce::jmax(1.0f, delayBase * (1.0f + mod * 0.3f));
            break;
        case 4:
            filterCutoff = clamp(filterCutoff * fastPow2(mod * 2.0f), 20.0f, 20000.0f);
            break;
        case 5:
            spread = clamp(spread + mod * 0.5f, 0.0f, 1.0f);
            break;
        case 6:
            bioluminescence = clamp(bioluminescence + mod * 0.5f, 0.0f, 1.0f);
            break;
        case 7:
            entrain = clamp(entrain + mod * 0.5f, 0.0f, 1.0f);
            break;
        }
    }
    ...
    void handleNoteOn(int note, float velocity) noexcept
    {
        ...
        filterEnvDecayRate = (decaySec > 0.0001f) ? 1.0f / (decaySec * static_cast<float>(sr)) : 1.0f;
    }
