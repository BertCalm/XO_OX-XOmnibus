...
struct TonewheelCrosstalk
{
    ...
    float process (float fundamentalFreq, int drawbarIndex,
                   float phase, float amount, float sampleRate) noexcept
    {
        ...
        (void) lowerFreq; (void) upperFreq; // suppress warnings
        (void) sampleRate;
...
struct KeyClick
{
    ...
    {
        if (clickLevel < 0.001f) { active = false; return; }
    ...
    void reset() noexcept { active = false; envelope = 0.0f; }
...
{
    void trigger (float velocity, float percLevel, float harmSelect,
                  float decayTime, float sampleRate) noexcept
    {
        if (percLevel < 0.001f) { active = false; return; }
...
struct CalliopePipe
{
    ...
    float process (float pressureWobble, float globalInstability) noexcept
    {
        ...
        float out = fastSin (phase * kTwoPi) * 0.8f
                  + fastSin (phase * 2.0f * kTwoPi) * 0.15f
    ...
    void reset() noexcept { active = false; phase = 0.0f; }
...
{
    void trigger (float freq, float velocity, float bendAmount,
    ...
    float process (float breathPressure, float expressionMod) noexcept
    {
        ...
        if (overblowActive)
        {
            ...
            if (overblowPhase >= 1.0f) overblowPhase -= 1.0f;
    ...
    void release() noexcept { active = false; }
    void reset() noexcept { active = false; phase = 0.0f; vibratoPhase = 0.0f; }
...
struct ZydecoReed
{
    ...
    float process (float bellowsPressure, float musetteAmount = 0.5f) noexcept
    {
        ...
        float reed1 = fastSin (phase1 * kTwoPi) * 0.5f
                     + fastSin (phase1 * 3.0f * kTwoPi) * 0.25f
                     + fastSin (phase1 * 5.0f * kTwoPi) * 0.15f
                     + fastSin (phase1 * 7.0f * kTwoPi) * 0.08f;

        // Reed 2: same spectrum, slightly detuned
        float reed2 = fastSin (phase2 * kTwoPi) * 0.5f
                     + fastSin (phase2 * 3.0f * kTwoPi) * 0.25f
                     + fastSin (phase2 * 5.0f * kTwoPi) * 0.15f
                     + fastSin (phase2 * 7.0f * kTwoPi) * 0.08f;
    ...
    void release() noexcept { active = false; }
    void reset() noexcept { active = false; phase1 = 0.0f; phase2 = 0.0f; }
...
public:
    ...
    void prepare (double sampleRate, int maxBlockSize) override
    {
        ...
        smoothDrawbar.fill ({});
        for (auto& s : smoothDrawbar) s.prepare (srf);
                            ...
                            const float* buf, int numSamples) override
    {
        ...
        switch (type)
        {
            ...
            case CouplingType::AudioToFM:    couplingFMMod += val; break;
            default: break;
        }
    }
                      ...
                      int numSamples) override
    {
        ...
        float effectiveLeslie = std::clamp (pLeslieSpeed + macroMovement * 0.4f
                                            + modWheelAmount * 0.5f, 0.0f, 1.0f);
        float effectiveBright = std::clamp (pBrightness + macroCharacter * 4000.0f
                                            + aftertouchAmount * 3000.0f
        ...
        // Smooth parameters
        for (int d = 0; d < 9; ++d) smoothDrawbar[d].set (drawbarLevels[d]);
        ...
        if (steamPressurePhase >= 1.0f) steamPressurePhase -= 1.0f;
                ...
                switch (organModel)
                {
                    ...
                    case 0:
                    {
                        ...
                        for (int d = 0; d < 9; ++d)
                        {
                            ...
                            if (drawbarLevel < 0.001f) continue;
                            ...
                            // Tonewheel crosstalk
                            float xtalk = voice.crosstalk.process (
                                freq, d, voice.phase, effectiveCrosstalk, srf);
                    ...
                    case 3:
                    {
                        ...
                        float bellowsPressure = 0.7f + aftertouchAmount * 0.3f
                                              + modWheelAmount * 0.2f;
    ...
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout() override
    {
        ...
        params.push_back (std::make_unique<PI> (juce::ParameterID { "otis_organ", 1 },
            "Otis Organ Model", 0, 3, 0));

        // Hammond drawbars (9)
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar1", 1 },
            "Otis Drawbar 16'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar2", 1 },
            "Otis Drawbar 5-1/3'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar3", 1 },
            "Otis Drawbar 8'", juce::NormalisableRange<float> (0.0f, 1.0f), 1.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar4", 1 },
            "Otis Drawbar 4'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar5", 1 },
            "Otis Drawbar 2-2/3'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar6", 1 },
            "Otis Drawbar 2'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar7", 1 },
            "Otis Drawbar 1-3/5'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar8", 1 },
            "Otis Drawbar 1-1/3'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drawbar9", 1 },
            "Otis Drawbar 1'", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // Leslie
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_leslie", 1 },
            "Otis Leslie Speed", juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Key click
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_keyClick", 1 },
            "Otis Key Click", juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Percussion
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_percussion", 1 },
            "Otis Percussion Level", juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_percHarmonic", 1 },
            "Otis Perc Harmonic (2nd/3rd)", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_percDecay", 1 },
            "Otis Perc Decay", juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        // Tonewheel crosstalk
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_crosstalk", 1 },
            "Otis Crosstalk", juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));

        // Shared tone controls
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_brightness", 1 },
            "Otis Brightness", juce::NormalisableRange<float> (200.0f, 20000.0f, 0.0f, 0.3f), 8000.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_drive", 1 },
            "Otis Drive", juce::NormalisableRange<float> (0.0f, 1.0f), 0.2f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_filterEnvAmount", 1 },
            "Otis Filter Env Amount", juce::NormalisableRange<float> (0.0f, 1.0f), 0.3f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_bendRange", 1 },
            "Otis Pitch Bend Range", juce::NormalisableRange<float> (1.0f, 24.0f, 1.0f), 2.0f));

        // ADSR
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_attack", 1 },
            "Otis Attack", juce::NormalisableRange<float> (0.001f, 2.0f, 0.0f, 0.3f), 0.005f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_decay", 1 },
            "Otis Decay", juce::NormalisableRange<float> (0.01f, 5.0f, 0.0f, 0.3f), 0.3f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_sustain", 1 },
            "Otis Sustain", juce::NormalisableRange<float> (0.0f, 1.0f), 0.8f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_release", 1 },
            "Otis Release", juce::NormalisableRange<float> (0.01f, 5.0f, 0.0f, 0.3f), 0.3f));

        // Model-specific params
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_bendAmount", 1 },
            "Otis Bend Amount (Harp)", juce::NormalisableRange<float> (0.0f, 5.0f, 0.1f), 2.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_instability", 1 },
            "Otis Instability (Calliope)", juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_musette", 1 },
            "Otis Musette (Accordion)", juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));

        // Macros
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_macroCharacter", 1 },
            "Otis Macro CHARACTER", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_macroMovement", 1 },
            "Otis Macro MOVEMENT", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_macroCoupling", 1 },
            "Otis Macro COUPLING", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_macroSpace", 1 },
            "Otis Macro SPACE", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));

        // LFOs
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_lfo1Rate", 1 },
            "Otis LFO1 Rate", juce::NormalisableRange<float> (0.005f, 20.0f, 0.0f, 0.3f), 0.5f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_lfo1Depth", 1 },
            "Otis LFO1 Depth", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
        params.push_back (std::make_unique<PI> (juce::ParameterID { "otis_lfo1Shape", 1 },
            "Otis LFO1 Shape", 0, 4, 0));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_lfo2Rate", 1 },
            "Otis LFO2 Rate", juce::NormalisableRange<float> (0.005f, 20.0f, 0.0f, 0.3f), 1.0f));
        params.push_back (std::make_unique<PF> (juce::ParameterID { "otis_lfo2Depth", 1 },
            "Otis LFO2 Depth", juce::NormalisableRange<float> (0.0f, 1.0f), 0.0f));
