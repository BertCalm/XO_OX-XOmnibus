...
static constexpr float kStoneRatios[kNumModes] = {1.000f,  // fundamental
                                                  ...
                                                  18.637f, 24.812f, 31.870f,
                                                  39.810f, // upper modes become very sparse
                                                  48.633f, 58.339f, 68.927f, 80.398f, 92.751f, 105.987f, 120.106f};
...
struct ObeliskPreparation
{
    ...
    static ModResult computeModification(int prepType, float position, float depth, int modeIndex, int totalModes,
                                         float fundamentalHz, float sampleRate) noexcept
    {
        ModResult result;
        if (prepType == 0 || depth < 0.001f)
            return result; // None
...
struct ObeliskVoice
{
    ...
    void reset() noexcept
    {
        ...
        for (auto& p : prepMods)
            p = {};
        for (auto& c : modeCoeffsB0)
            c = 0.0f;
        for (auto& c : modeCoeffsA1)
            c = 0.0f;
        for (auto& c : modeCoeffsA2)
            c = 0.0f;
...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        if (thermalTimer > static_cast<int>(srf * 5.0f)) // new target every ~5 seconds
        {
            ...
            thermalTarget = (static_cast<float>(thermalNoiseState & 0xFFFF) / 32768.0f - 1.0f) * pThermal *
                            5.0f; // max +-5 cents (stone is thermally stable)
            thermalTimer = 0;
        }
                    ...
                    if (needsUpdate)
                    {
                        for (int m = 0; m < kNumModes; ++m)
                        {
                            ...
                            voice.prepMods[m] = ObeliskPreparation::computeModification(
                                pPrepType, pPrepPosition, voicePrepDepth, m, kNumModes, freq, srf);
                            modeFreq *= voice.prepMods[m].freqMul;
                            if (modeFreq >= srf * 0.49f)
                                modeFreq = srf * 0.49f;
                            if (modeFreq < 10.0f)
                                modeFreq = 10.0f;
    ...
    void noteOn(int note, float vel) noexcept
    {
        ...
        {
            v.prepMods[m] =
                ObeliskPreparation::computeModification(prepType, prepPos, prepDepth, m, kNumModes, freq, srf);
        }
