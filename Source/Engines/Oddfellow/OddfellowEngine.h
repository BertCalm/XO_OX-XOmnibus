...
struct WurliReedModel
{
    ...
    static constexpr float kReedRatios[kNumPartials] = {
        1.0f, 2.01f, 3.03f, 4.08f, 5.15f
    };

    // Relative amplitudes — fundamental-heavy, warm, fewer upper partials
    static constexpr float kReedAmps[kNumPartials] = {
        1.0f, 0.5f, 0.2f, 0.08f, 0.03f
    };
    ...
    void trigger (float velocity, float reedStiffness) noexcept
    {
        ...
        for (int i = 0; i < kNumPartials; ++i)
        {
            ...
            if (stiffScale < 0.0f) stiffScale = 0.0f;
    ...
    float process (float fundamentalHz, float reedParam) noexcept
    {
        ...
        warblePhase += 4.5f / sr;
        if (warblePhase >= 1.0f) warblePhase -= 1.0f;
...
struct WurliPreamp
{
    float process (float input, float drive) noexcept
    {
        ...
        float saturated = fastTanh (driven * 0.8f) * 0.7f
                        + fastTanh (driven * 1.5f) * 0.3f;
...
public:
    ...
    juce::Colour getAccentColour() const override { return juce::Colour (0xFFFF6B35); }
                            ...
                            const float* buf, int numSamples) override
    {
        ...
        switch (type) {
            ...
            case CouplingType::EnvToMorph:   couplingReedMod += val; break;
            default: break;
        }
    }
                      ...
                      int numSamples) override
    {
        ...
        float effectiveReed = std::clamp (pReed + macroChar * 0.4f
                                + aftertouchAmount * 0.3f + couplingReedMod, 0.0f, 1.0f);
        float effectiveDrive = std::clamp (pDrive + macroChar * 0.3f
                                + modWheelAmount * 0.5f, 0.0f, 1.0f);
        float effectiveBright = std::clamp (pBrightness + macroMove * 3000.0f
                                + aftertouchAmount * 2000.0f + couplingFilterMod, 200.0f, 16000.0f);
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                if (migrationN > 0.01f)
                {
                    float migColor = fastTanh (preampOut * (1.0f + migrationN * 2.0f))
                                   * migrationN * 0.2f;
                    preampOut += migColor;
                }
