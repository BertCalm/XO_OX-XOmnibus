...
struct OlegCassotto
{
    ...
    void prepare (float sampleRate) noexcept
    {
        ...
        if (combDelaySamples < 1) combDelaySamples = 1;
        if (combDelaySamples >= kMaxDelay) combDelaySamples = kMaxDelay - 1;
        ...
        if (ap1DelaySamples < 1) ap1DelaySamples = 1;
        if (ap2DelaySamples < 1) ap2DelaySamples = 1;
        if (ap1DelaySamples >= kMaxDelay) ap1DelaySamples = kMaxDelay - 1;
        if (ap2DelaySamples >= kMaxDelay) ap2DelaySamples = kMaxDelay - 1;
    ...
    void reset() noexcept
    {
        ...
        std::fill (ap1Buf.begin(), ap1Buf.end(), 0.0f);
        std::fill (ap2Buf.begin(), ap2Buf.end(), 0.0f);
        combWritePos = 0; ap1WritePos = 0; ap2WritePos = 0;
        wallFilter.reset();
    }
...
struct OlegBuzzBridge
{
    ...
    {
        if (buzzIntensity < 0.001f) return input;
...
struct OlegReedOscillator
{
    ...
    float process (float freq, int model, float detune, float droneLevel,
                   float droneInterval1, float droneInterval2,
...
public:
    ...
    juce::Colour getAccentColour() const override { return juce::Colour (0xFFC5A036); } // Orthodox Gold
                            ...
                            const float* buf, int numSamples) override
    {
        ...
        switch (type) {
            ...
            case CouplingType::EnvToMorph:   couplingBuzzMod += val; break;
            default: break;
        }
    }
                      ...
                      int numSamples) override
    {
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                float raw = voice.osc.process (freq, pOrgan, detuneNow, droneNow,
                                               pDroneInt1, pDroneInt2,
                                               voice.isPush, effectiveWheelSpd, srf);
                ...
                if (ampLevel < 1e-6f) { voice.active = false; continue; }
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back (std::make_unique<PI> (juce::ParameterID { "oleg_organ", 1 }, "Oleg Organ Model",
            0, 3, 0)); // 0=Bayan, 1=HurdyGurdy, 2=Bandoneon, 3=Garmon
        ...
        params.push_back (std::make_unique<PF> (juce::ParameterID { "oleg_droneInterval1", 1 }, "Oleg Drone Interval 1",
            juce::NormalisableRange<float> (-24.0f, 0.0f, 1.0f), -7.0f)); // default: 5th below
        params.push_back (std::make_unique<PF> (juce::ParameterID { "oleg_droneInterval2", 1 }, "Oleg Drone Interval 2",
            juce::NormalisableRange<float> (-24.0f, 0.0f, 1.0f), -12.0f)); // default: octave below
