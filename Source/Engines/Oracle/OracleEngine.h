...
struct OracleDCBlocker
{
    ...
    void prepare (float sampleRate) noexcept
    {
        ...
        if (feedbackCoeff < 0.9f) feedbackCoeff = 0.9f;
        if (feedbackCoeff > 0.9999f) feedbackCoeff = 0.9999f;
    }
...
public:
                      ...
                      int numSamples) override
    {
        ...
        {
            case 0: maxPolyphony = 1; isMonophonic = true; break;                        // Mono
            case 1: maxPolyphony = 1; isMonophonic = true; isLegatoMode = true; break;   // Legato
        ...
        float effectiveTimeStep = clamp (timeStepAmount + macroEvolution * 0.4f   // EVOLUTION macro adds chaos speed
                                         + macroDrift * 0.2f, 0.0f, 1.0f);       // DRIFT macro adds gentle motion
        float effectiveAmpStep  = clamp (amplitudeStepAmount + macroEvolution * 0.3f  // EVOLUTION macro adds amplitude drift
                                         + macroProphecy * 0.2f, 0.0f, 1.0f);        // PROPHECY macro adds unpredictability
        float effectiveDistribution = clamp (distributionMorph + couplingDistributionMod  // Coupling: external envelope drives distribution
        ...
        for (int sampleIndex = 0; sampleIndex < numSamples; ++sampleIndex)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                float modulatedTimeStep = clamp (smoothedTimeStep + lfo1Value * 0.3f,
                                                 0.0f, 1.0f);
                ...
                if (voice.wavePhase >= 1.0f)
                {
                    ...
                    evolveBreakpoints (voice,
                                       modulatedTimeStep * stochasticDepth,
                                       modulatedAmpStep * stochasticDepth,
                ...
                if (voice.cycleBlendCounter > 0)
                {
                    ...
                    rawSampleL = rawSampleL * (1.0f - blendRatio)
                               + voice.prevCycleLastSample * blendRatio;
                ...
                if (rightChannelPhase >= 1.0f) rightChannelPhase -= 1.0f;
                ...
                if (voice.cycleBlendCounter > 0)
                {
                    float blendRatio = static_cast<float> (voice.cycleBlendCounter + 1)
                                     / static_cast<float> (OracleVoice::kCycleBlendSamples);
                    rawSampleR = rawSampleR * (1.0f - blendRatio)
                               + voice.prevCycleLastSample * blendRatio;
                }
    ...
    float getSampleForCoupling (int channel, int sampleIndex) const override
    {
        ...
        if (channel == 2) return envelopeOutput;  // Envelope follower for sidechain
        return 0.0f;
    }
...
private:
    ...
    static float mirrorBarrier (float value, float lowerBound, float upperBound,
                                float elasticity) noexcept
    {
        float range = upperBound - lowerBound;
        if (range <= 0.0001f) return (lowerBound + upperBound) * 0.5f;
            ...
            if (timeSpan > 0.001f)
            {
                for (int i = 0; i < breakpointCount; ++i)
                {
                    voice.breakpoints[i].timeOffset =
                        (voice.breakpoints[i].timeOffset - timeMin) / timeSpan;
                }
            }
            else
            {
                ...
                {
                    voice.breakpoints[i].timeOffset =
                        static_cast<float> (i) / static_cast<float> (breakpointCount - 1);
                }
            }
        }
    ...
    {
        int breakpointCount = voice.numBreakpoints;
        if (breakpointCount < 2) return 0.0f;
    ...
    void noteOn (int noteNumber, float velocity, int maxPolyphony,
                 bool isMonophonic, bool isLegatoMode, float glideCoeff,
                 int breakpointCount, int maqamIdx, float gravity,
                 float ampAttack, float ampDecay, float ampSustain, float ampRelease,
                 float stochAttack, float stochDecay, float stochSustain, float stochRelease,
