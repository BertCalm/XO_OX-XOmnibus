...
public:
    ...
    void renderBlock(juce::AudioBuffer<float>& buf, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        for (const auto midiEvent : midi)
        {
            ...
            if (msg.isNoteOn())
            {
                ...
                {
                case 0:
                    assignBroA = (targetSlot % 2 == 0);
                    break; // Alternate
                case 1:
                    assignBroA = (msg.getNoteNumber() < 60);
                    break; // Split at C4
                case 2:
                    assignBroA = true;
                    break; // Layer (second voice below)
                case 3:
                    assignBroA = (rrCounter++ % 2 == 0);
                    break; // Round Robin
                case 4:
                    assignBroA = (msg.getVelocity() < 80);
                    break; // Velocity: soft→A, hard→B
                default:
                    break;
                }
        ...
        for (int sampleIdx = 0; sampleIdx < numSamples; ++sampleIdx)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                float pan = std::clamp(basePan + (voice.isBroA ? -bondPanSpread : bondPanSpread), 0.05f, 0.95f);
    ...
    float getSampleForCoupling(int channel, int /*sampleIndex*/) const override { return channel == 0 ? lastL : lastR; }

    void applyCouplingInput(CouplingType couplingType, float amount, const float* couplingBuf,
    ...
    static void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back(std::make_unique<Choice>(
            "obbl_instrumentA", "Instrument A",
            juce::StringArray{"Flute", "Piccolo", "Pan Flute", "Shakuhachi", "Bansuri", "Ney", "Recorder", "Ocarina"},
            0));
        ...
        params.push_back(std::make_unique<Choice>(
            "obbl_instrumentB", "Instrument B",
            juce::StringArray{"Clarinet", "Oboe", "Bassoon", "Soprano Sax", "Duduk", "Zurna", "Shawm", "Musette"}, 0));
        params.push_back(std::make_unique<Float>("obbl_bodySizeB", "Body B", Range{0.0f, 1.0f}, 0.5f));

        // Section C: Shared voice controls
        params.push_back(
            std::make_unique<Choice>("obbl_voiceRouting", "Voice Routing",
