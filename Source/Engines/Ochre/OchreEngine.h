...
//==============================================================================
static constexpr float kCopperUprightRatios[16] = {1.000f,  2.756f,  4.318f,  6.891f,  9.274f,  12.510f,
                                                   15.830f, 19.620f, 24.100f, 28.900f, 34.200f, 40.100f,
...
static constexpr float kCopperModeAmps[16] = {1.00f, 0.82f, 0.68f, 0.55f,  0.44f,  0.35f,  0.28f,  0.22f,
                                              0.17f, 0.13f, 0.10f, 0.075f, 0.055f, 0.040f, 0.028f, 0.018f};
...
{
    void trigger(float velocity, float hardness, float conductivity, float baseFreq, float sampleRate,
...
struct OchreBody
{
    ...
    float process(float input, float depth) noexcept
    {
        ...
        float bodyOut =
            bodyMode1.process(input) * 0.45f + bodyMode2.process(input) * 0.35f + bodyMode3.process(input) * 0.20f;
...
struct OchreSympathetic
{
    ...
    void buildFromActiveNotes(const float* activeFreqs, int numActive) noexcept
    {
        ...
        for (int i = 0; i < numActive && activeStringCount < kNumStrings; ++i)
        {
            ...
            {
                if (activeStringCount >= kNumStrings)
                    break;
                float sf = f * intv;
                if (sf > 0.49f * sr)
                    continue;
    ...
    {
        if (amount < 0.001f || activeStringCount == 0)
            return 0.0f;
...
public:
    ...
    void prepare(double sampleRate, int maxBlockSize) override
    {
        ...
        for (int i = 0; i < kMaxVoices; ++i)
        {
            ...
            voices[i].thermalPersonality = (static_cast<float>(seed & 0xFFFF) / 32768.0f - 1.0f) * 2.5f;
    ...
    void renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi, int numSamples) override
    {
        ...
        float effectiveConductivity = std::clamp(
            pConductivity + macroCharacter * 0.6f + couplingConductivityMod + modWheelAmount * 0.3f, 0.0f, 1.0f);
        ...
        if (thermalTimer > static_cast<int>(srf * 2.5f)) // new target every ~2.5 seconds
        {
            ...
            thermalTarget = (static_cast<float>(thermalNoiseState & 0xFFFF) / 32768.0f - 1.0f) * pThermal *
                            12.0f; // max ±12 cents (copper drifts more than iron)
            thermalTimer = 0;
        }
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                for (int m = 0; m < OchreVoice::kNumModes; ++m)
                {
                    ...
                    float modeAmp = kCopperModeAmps[m] * (1.0f + hardNow * static_cast<float>(m) * 0.08f);
    ...
    static void addParametersImpl(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        // === Copper Piano Core ===
        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_conductivity", 1}, "Ochre Conductivity",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_hardness", 1}, "Ochre Hammer Hardness",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.4f));

        params.push_back(std::make_unique<PI>(juce::ParameterID{"ochre_bodyType", 1}, "Ochre Body Type", 0, 2, 0));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_bodyDepth", 1}, "Ochre Body Depth",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.4f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_brightness", 1}, "Ochre Brightness",
                                              juce::NormalisableRange<float>(200.0f, 20000.0f, 0.0f, 0.3f), 10000.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_damping", 1}, "Ochre Damping",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.4f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_decay", 1}, "Ochre Decay",
                                              juce::NormalisableRange<float>(0.05f, 8.0f, 0.0f, 0.4f), 1.2f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_caramel", 1}, "Ochre Caramel Saturation",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.3f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_sympathy", 1}, "Ochre Sympathetic Resonance",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.2f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_hfCharacter", 1}, "Ochre HF Body Character",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.3f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_thermalDrift", 1}, "Ochre Thermal Drift",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.4f));

        // === Envelope ===
        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_filterEnvAmount", 1}, "Ochre Filter Env Amount",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.4f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_bendRange", 1}, "Ochre Pitch Bend Range",
                                              juce::NormalisableRange<float>(1.0f, 24.0f, 1.0f), 2.0f));

        // === Macros (4) ===
        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_macroCharacter", 1}, "Ochre Macro CHARACTER",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_macroMovement", 1}, "Ochre Macro MOVEMENT",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_macroCoupling", 1}, "Ochre Macro COUPLING",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_macroSpace", 1}, "Ochre Macro SPACE",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        // === LFOs (D002/D005) ===
        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_lfo1Rate", 1}, "Ochre LFO1 Rate",
                                              juce::NormalisableRange<float>(0.005f, 20.0f, 0.0f, 0.3f), 0.5f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_lfo1Depth", 1}, "Ochre LFO1 Depth",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.1f));

        params.push_back(std::make_unique<PI>(juce::ParameterID{"ochre_lfo1Shape", 1}, "Ochre LFO1 Shape", 0, 4, 0));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_lfo2Rate", 1}, "Ochre LFO2 Rate",
                                              juce::NormalisableRange<float>(0.005f, 20.0f, 0.0f, 0.3f), 1.0f));

        params.push_back(std::make_unique<PF>(juce::ParameterID{"ochre_lfo2Depth", 1}, "Ochre LFO2 Depth",
                                              juce::NormalisableRange<float>(0.0f, 1.0f), 0.0f));

        params.push_back(std::make_unique<PI>(juce::ParameterID{"ochre_lfo2Shape", 1}, "Ochre LFO2 Shape", 0, 4, 1));
    }
