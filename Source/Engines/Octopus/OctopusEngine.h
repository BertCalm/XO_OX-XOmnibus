...
//==============================================================================
// LFO with multiple shapes — used by both main LFOs and the arm system.
//==============================================================================
struct OctoLFO
{
...
struct OctoFreezeBuffer
{
    ...
    float process() noexcept
    {
        ...
        if (frozenGain < 0.0f) frozenGain = 0.0f;
...
public:
                      ...
                      int numSamples) override
    {
        ...
        for (auto& voice : voices)
        {
            ...
            voice.suckerFilter.setCoefficients (
                clamp (pSuckerFreq, 200.0f, 8000.0f), pSuckerReso, srf);
        ...
        for (int sample = 0; sample < numSamples; ++sample)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                if (effectiveDrift > 0.001f)
                {
                    ...
                    if (voice.pitchDriftPhase >= 1.0f) voice.pitchDriftPhase -= 1.0f;
                ...
                float wtPos = clamp (smoothedWTPos
                    + lfo1Val * 0.3f
                    + modLevel * pWTScanRate * 0.5f
                ...
                if (smoothedSuckerMix > 0.001f)
                {
                    ...
                    if (suckerLevel > 0.001f)
                    {
                        ...
                        voiceSignal = voiceSignal * (1.0f - smoothedSuckerMix)
                                    + (voiceSignal + suckerSig) * smoothedSuckerMix;
                    }
                }
    ...
    static void addParametersImpl (std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_armCount", 1 }, "Octopus Arm Count",
            juce::NormalisableRange<float> (1.0f, 8.0f, 1.0f), 4.0f));

        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_armSpread", 1 }, "Octopus Arm Spread",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_inkDensity", 1 }, "Octopus Ink Density",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.001f), 0.8f));

        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_inkDecay", 1 }, "Octopus Ink Decay",
            juce::NormalisableRange<float> (0.5f, 30.0f, 0.1f, 0.3f), 5.0f));

        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_inkMix", 1 }, "Octopus Ink Mix",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.001f), 0.0f));
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_shiftDrift", 1 }, "Octopus Pitch Drift",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.001f), 0.0f));
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_suckerMix", 1 }, "Octopus Sucker Mix",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.001f), 0.0f));
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_wtScanRate", 1 }, "Octopus WT Scan Rate",
        ...
        // --- Level ---
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_level", 1 }, "Octopus Level",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_ampSustain", 1 }, "Octopus Amp Sustain",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_modSustain", 1 }, "Octopus Mod Sustain",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_lfo1Depth", 1 }, "Octopus LFO1 Depth",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_lfo2Depth", 1 }, "Octopus LFO2 Depth",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));
        ...
        // --- Voice Mode ---
        params.push_back (std::make_unique<juce::AudioParameterChoice> (
            juce::ParameterID { "octo_polyphony", 1 }, "Octopus Voice Mode",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "octo_macroSpace", 1 }, "Octopus Macro SPACE",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));
    }
...
private:
    ...
    void noteOn (int noteNumber, float velocity, int midiChannel, int maxPoly,
                 ...
                 float armBaseRate, float armSpread, int armCount,
                 float inkThreshold, float inkDensity, float inkDecay,
                 float microOffset)
    {
        ...
        for (int a = 0; a < 8; ++a)
        {
            ...
            if (voice.arms[a].phase >= 1.0f) voice.arms[a].phase -= 1.0f;
        }
