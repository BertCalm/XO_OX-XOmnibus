...
inline const juce::StringArray biome{"jungle", "underwater", "winter"};
inline const juce::StringArray floorModel{"berimbau", "cuica", "agogo", "kalimba", "pandeiro", "log_drum"};
inline const juce::StringArray creatureType{"bird_trill", "whale_song", "insect_drone",
                                            "frog_chirp", "wolf_howl",  "synth_call"};
inline const juce::StringArray creatureTrigger{"midi", "floor_amp", "canopy_peaks"};
inline const juce::StringArray couplingBus{"understory", "canopy", "floor", "emergent"};

} // namespace Enums

// ── Parameter Registration (vector-based, for XOceanus shared APVTS) ────
...
inline void addParameters(std::vector<std::unique_ptr<juce::RangedAudioParameter>>& p)
{
    auto F = [&](const char* id, const char* name, float lo, float hi, float def, float skew = 1.0f)
    {
        p.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID{id, 1}, name, juce::NormalisableRange<float>(lo, hi, 0.0f, skew), def));
    };
    ...
    { p.push_back(std::make_unique<juce::AudioParameterChoice>(juce::ParameterID{id, 1}, name, ch, def)); };
