        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            if (callEnvSample >= 0.0f)
            {
                ...
                else
                {
                    ...
                    envAmp = xoceanus::fastExp(-5.0f * decayPos);
                    if (envAmp < 0.0001f)
            ...
            float noise = static_cast<float>(static_cast<int32_t>(noiseSeed)) * 4.656612e-10f; // / 2^31

            // Pitch sweep: formants glide up during attack, relax during decay
            float envPhaseNorm =
                (callEnvSample >= 0.0f && totalSamp > 0.0f) ? std::clamp(callEnvSample / totalSamp, 0.0f, 1.0f) : 0.0f;
            // SRO: fastSin + fastPow2 replace std:: trig/pow (per-sample)
            float sweep = xoceanus::fastSin(envPhaseNorm * juce::MathConstants<float>::pi) * biome.emergentPitchRange;
            float sweepMul = xoceanus::fastPow2(sweep);
...
private:
    ...
    struct FormantFilter
    {
        ...
        float process(float x, float freq, float q, float sr)
        {
            ...
            float f = std::min(
                1.99f, 2.0f * xoceanus::fastSin(juce::MathConstants<float>::pi * std::min(freq, sr * 0.45f) / sr));
    ...
    void computeFormantFreqs(const OcelotParamSnapshot& snap, const BiomeProfile& biome,
                             const struct StrataModulation& mod, float& f1, float& f2, float& f3) const
    {
