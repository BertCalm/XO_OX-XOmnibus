...
public:
    ...
    float processSample() noexcept
    {
        ...
        switch (wave)
        {
            case 0: out = sampleOblongSine (t); break;
            case 1: out = sampleSoftTriangle (t); break;
            case 2: out = sampleVelvetSaw (t); break;
            case 3: out = sampleCushionPulse (t); break;
            default: out = sampleOblongSine (t); break;
        }
...
private:
    ...
    {
        float driftMul = (std::abs (driftSmooth) > 0.0001f)
            ? fastExp (driftSmooth * (0.693147f / 12.0f)) : 1.0f;
...
public:
    ...
    void syncToOscA() noexcept { if (syncEnabled) phase = 0.0; }
...
public:
    ...
    void setTone (float t) noexcept { tone = clamp (t, 0.0f, 1.0f); updateFilters(); }
    ...
    {
        if (level < 0.001f) { outL = outR = 0.0f; return; }
...
private:
    ...
    {
        lpCoeff = clamp (0.01f + tone * 0.5f, 0.01f, 0.5f);
    }
};
    ...
    float processSample (float x) noexcept
    {
        ...
        switch (mode)
        {
            case 0: // Snout LP
            {
                ...
                s1 = g * hp + bp;
                s2 = g * bp + lp;
                if (! std::isfinite (lp)) { lp = hp = bp = 0.0f; s1 = s2 = 0.0f; }
            ...
            case 2: // Snout Form (dual BP formant morph)
            {
                ...
                s1 = g * hp1 + bp1;
                s2 = g * bp1 + lp1;
                if (! std::isfinite (lp1)) { s1 = s2 = 0.0f; bp1 = 0.0f; }
                ...
                s1b = gb * hp2 + bp2;
                s2b = gb * bp2 + lp2;
                if (! std::isfinite (lp2)) { s1b = s2b = 0.0f; bp2 = 0.0f; }
...
public:
    ...
    struct Output { float lfo1; float curiosity; };
...
public:
                      ...
                      int numSamples) override
    {
        ...
        for (int sample = 0; sample < numSamples; ++sample)
        {
            ...
            for (auto& voice : voices)
            {
                ...
                switch (lfo1Target)
                {
                    case 0: lfoCutoffMod = curOut.lfo1; break;
                    case 1: lfoPitchMod = curOut.lfo1; break;
                    case 2: lfoShapeMod = curOut.lfo1; break;
                    case 3: break; // FX depth — applied globally
                }
    ...
    static void addParametersImpl (std::vector<std::unique_ptr<juce::RangedAudioParameter>>& params)
    {
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_oscA_shape", 1 }, "Bob Osc A Shape",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_oscA_tune", 1 }, "Bob Osc A Tune",
            juce::NormalisableRange<float> (-24.0f, 24.0f, 0.01f), 0.0f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_oscA_drift", 1 }, "Bob Osc A Drift",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_oscB_detune", 1 }, "Bob Osc B Detune",
            juce::NormalisableRange<float> (-100.0f, 100.0f, 0.1f), 0.0f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_oscB_blend", 1 }, "Bob Osc B Blend",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
        params.push_back (std::make_unique<juce::AudioParameterChoice> (
            juce::ParameterID { "bob_oscB_sync", 1 }, "Bob Osc B Sync",
            juce::StringArray { "Off", "On" }, 0));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_oscB_fm", 1 }, "Bob Osc B FM",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));

        // --- Texture ---
        params.push_back (std::make_unique<juce::AudioParameterChoice> (
            juce::ParameterID { "bob_texMode", 1 }, "Bob Texture Mode",
            juce::StringArray { "Dust", "Blanket", "Static", "Breath" }, 1));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_texLevel", 1 }, "Bob Texture Level",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_texTone", 1 }, "Bob Texture Tone",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_texWidth", 1 }, "Bob Texture Width",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_fltReso", 1 }, "Bob Filter Resonance",
            juce::NormalisableRange<float> (0.0f, 0.95f, 0.01f), 0.3f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_fltChar", 1 }, "Bob Filter Character",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_fltDrive", 1 }, "Bob Filter Drive",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_fltEnvAmt", 1 }, "Bob Filter Env Amt",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_ampSustain", 1 }, "Bob Amp Sustain",
        ...
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_curAmount", 1 }, "Bob Curiosity Amount",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));

        // --- Bob Mode ---
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_bobMode", 1 }, "Bob Mode",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));

        // --- Dust Tape ---
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_dustAmount", 1 }, "Bob Dust Amount",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_dustTone", 1 }, "Bob Dust Tone",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));

        // --- Level ---
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_level", 1 }, "Bob Level",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.8f));

        // --- Voice ---
        params.push_back (std::make_unique<juce::AudioParameterChoice> (
            juce::ParameterID { "bob_voiceMode", 1 }, "Bob Voice Mode",
            juce::StringArray { "Poly", "Mono", "Legato" }, 0));
        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { "bob_glide", 1 }, "Bob Glide",
            juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.0f));
        params.push_back (std::make_unique<juce::AudioParameterChoice> (
