...
public:
    ...
    //--------------------------------------------------------------------------
    void trigger(int type, float velocity, float tune, float decay, float body, float snap, float pitchEnvDepth,
                 float sat) noexcept
    {
        type_ = type;
        velocity_ = velocity;
...
private:
    ...
    float processHat(float env) noexcept
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            ...
            if (metalPhases_[i] >= 1.0f)
                metalPhases_[i] -= 1.0f;
        }

        // Ring-modulate in pairs: (0,1), (2,3), (4,5) then sum
        float pair0 = (metalPhases_[0] < 0.5f ? 1.0f : -1.0f) * (metalPhases_[1] < 0.5f ? 1.0f : -1.0f);
        float pair1 = (metalPhases_[2] < 0.5f ? 1.0f : -1.0f) * (metalPhases_[3] < 0.5f ? 1.0f : -1.0f);
        float pair2 = (metalPhases_[4] < 0.5f ? 1.0f : -1.0f) * (metalPhases_[5] < 0.5f ? 1.0f : -1.0f);
