...
{
    void setParams (float attackMs, float releaseMs, float ratio,
                    float thresholdDb, float sampleRate) noexcept
    {
...
struct OfferingAllpass
{
    ...
    {
        delay_ = std::max (1, std::min (delaySamples, kMaxDelay - 1));
    }
...
public:
    ...
    void process (float* buffer, int numSamples, float intensity) noexcept
    {
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            switch (city_)
            {
                case NewYork: sample = processNewYork (sample, intensity); break;
                case Detroit: sample = processDetroit (sample, intensity); break;
                case LA:      sample = processLA (sample, intensity); break;
                case Toronto: sample = processToronto (sample, intensity); break;
                case BayArea: sample = processBayArea (sample, intensity); break;
            }
...
private:
    ...
    float processBayArea (float input, float intensity) noexcept
    {
        ...
        if (shimmerPhase_ >= 1.0f) shimmerPhase_ -= 1.0f;
...
public:
    ...
    //--------------------------------------------------------------------------
    void process (float* buffer, int numSamples, int cityMode,
                  float cityBlend, float cityIntensity) noexcept
    {
        if (cityIntensity < 0.001f) return; // bypass
