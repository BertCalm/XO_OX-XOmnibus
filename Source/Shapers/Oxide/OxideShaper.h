...
// The Flavor Pro of XOceanus — transforms any signal through analog character.
//
...
public:
    ...
    void processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& /*midi*/, int numSamples) override
    {
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            {
            case 0:
                modDrive = clamp(effDrive + lorenzMod * 0.5f, 0.0f, 1.0f);
                break;
            case 1:
                modHarmBal = clamp(effHarmBal + lorenzMod * 0.3f, 0.0f, 1.0f);
                break;
            case 2:
                modMix = clamp(effMix + lorenzMod * 0.3f, 0.0f, 1.0f);
                break;
            }
            ...
            if (R)
                R[s] = dryR * (1.0f - modMix) + satR * modMix;
        }
    ...
    {
        if (channel == 0)
            return harmonicDensityCoupling;
        if (channel == 1)
            return saturationAmountCoupling;
        return 0.0f;
    }
    ...
    void applyCouplingInput(CouplingType type, float amount, const float*, int) override
    {
        ...
        case CouplingType::AmpToFilter:
            couplingDriveMod += amount * 0.3f;
            break;
        case CouplingType::AudioToFM:
            couplingRhoMod += amount * 5.0f;
            break;
        default:
            break;
        }
    ...
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout() override
    {
        ...
        params.push_back(
            std::make_unique<PC>(juce::ParameterID{"oxide_drive_mode", 1}, "Oxide Drive Mode", driveChoices, 0));
        ...
        params.push_back(
            std::make_unique<PC>(juce::ParameterID{"oxide_lorenz_target", 1}, "Oxide Lorenz Target", lorenzTargets, 0));
