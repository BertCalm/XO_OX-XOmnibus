...
public:
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (int b = 0; b < kNumBands; ++b)
            {
                ...
                if (snap[b].tideEnable && snap[b].tideDepth > 0.001f)
                {
                    ...
                    {
                    case 0:
                        freqMod = band.tideOut * snap[b].freq * 0.5f;
                        break; // Freq
                    case 1:
                        gainMod = band.tideOut * 6.0f;
                        break; // Gain (±6dB)
                    case 2:
                        qMod = band.tideOut * 2.0f;
                        break; // Q
                    case 3:
                        charMod = band.tideOut * 0.3f;
                        break; // Character
                    default:
                        break;
                    }
                ...
                float gainLin = (isTilt || snap[b].type == 2 || snap[b].type == 3) // Low/High Shelf
                                    ? 1.0f
                                    : dbToGain(effGain);
    ...
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout() override
    {
        ...
        auto tideShapes = juce::StringArray{"Sine", "Triangle", "Saw", "Rev Saw", "Square", "S&H", "Drift"};
        ...
        for (int b = 0; b < kNumBands; ++b)
        {
            ...
            auto nm = [&](const char* suffix)
            { return juce::String("Observe Band ") + juce::String(b + 1) + " " + suffix; };

            params.push_back(std::make_unique<PF>(juce::ParameterID{id("freq"), 1}, nm("Freq"),
                                                  juce::NormalisableRange<float>(20.0f, 22000.0f, 0.1f, 0.3f),
                                                  kDefaultFreqs[b]));
            ...
            params.push_back(std::make_unique<PC>(juce::ParameterID{id("type"), 1}, nm("Type"), bandTypes, 0));
            ...
            params.push_back(std::make_unique<PF>(juce::ParameterID{id("tide_depth"), 1}, nm("Tide Depth"),
                                                  juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.0f));
            params.push_back(
                std::make_unique<PC>(juce::ParameterID{id("tide_target"), 1}, nm("Tide Target"), tideTargets, 1));
        }
...
private:
    ...
    void setFilterType(CytomicSVF& filter, int type)
    {
        ...
        {
        case 0:
            filter.setMode(CytomicSVF::Mode::Peak);
            break; // Peak
        case 1:
            filter.setMode(CytomicSVF::Mode::Notch);
            break; // Notch
        case 2:
            filter.setMode(CytomicSVF::Mode::LowShelf);
            break; // Low Shelf
        case 3:
            filter.setMode(CytomicSVF::Mode::HighShelf);
            break; // High Shelf
        case 4:
        case 5:
        case 6:
