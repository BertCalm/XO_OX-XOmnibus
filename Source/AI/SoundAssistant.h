...
public:
    ...
    SoundAssistant() : schema(buildDefaultSchema()) {}
    ...
    void textToRecipe(const juce::String& description, const SynthContext& context, RecipeCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded. Please wait.");
            return;
        }
        ...
        sendRequest(prompt,
                    [this, callback = std::move(callback)](const juce::String& response, const juce::String& error)
                    {
                        if (error.isNotEmpty())
                        {
                            RecipeResult result;
    ...
    void adviseCoupling(const SynthContext& context, CouplingCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded.");
            return;
        }
    ...
    void suggestMacros(const SynthContext& context, MacroCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded.");
    ...
    void namePreset(const SynthContext& context, NamingCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded.");
    ...
    void explainSound(const SynthContext& context, const juce::String& userQuestion, ExplanationCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded.");
    ...
    void refineRecipe(const juce::String& userRequest, const RecipeEngine::Recipe& currentRecipe,
                      const SynthContext& context, RefineCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded.");
        ...
        auto prompt = buildRefinePrompt(userRequest, currentRecipe, context, interpretation, targetDNA);

        auto prevDNA = currentRecipe.dna;
        auto interp = interpretation;

        sendRequest(prompt,
                    [this, callback = std::move(callback), prevDNA, interp, currentRecipe](const juce::String& response,
    ...
    void matchSound(const juce::String& description, const SynthContext& context, SoundMatchCallback callback)
    {
        if (!checkRateLimit())
        {
            callbackError(callback, "Rate limit exceeded.");
        ...
        sendRequest(
            prompt,
            [this, callback = std::move(callback), interp](const juce::String& response, const juce::String& error)
            {
                if (error.isNotEmpty())
                {
...
private:
    ...
    {
        conversationHistory.push_back({type, request, juce::Time::getMillisecondCounterHiRes() / 1000.0});
    ...
    using ResponseCallback = std::function<void(const juce::String& response, const juce::String& error)>;
    ...
    void sendRequest(const juce::String& prompt, ResponseCallback callback)
    {
        ...
        if (key.isEmpty())
        {
            callback({}, "No API key configured for " + SecureKeyStore::providerName(preferredProvider));
            return;
        }
    ...
    {
    public:
        RequestThread(SecureKeyStore::Provider provider, const juce::String& apiKey, const juce::String& prompt,
                      ResponseCallback callback, std::atomic<bool>* destroyedFlag)
            : juce::Thread("XO_AI_Request"), provider_(provider), apiKey_(apiKey), prompt_(prompt),
              callback_(std::move(callback)), destroyedFlag_(destroyedFlag)
        {
        }
        ...
        void run() override
        {
            ...
            if (!url.startsWithIgnoreCase("https://"))
            {
                ...
                juce::MessageManager::callAsync(
                    [cb, err, destroyedFlag = destroyedFlag_]()
                    {
            ...
            {
                error = "Network error: could not connect to " + SecureKeyStore::providerName(provider_);
            }
            ...
            juce::MessageManager::callAsync(
                [cb, resp, err, destroyedFlag = destroyedFlag_]()
                {
                    if (!destroyedFlag->load())
                        cb(resp, err);
                });
    ...
    {
        return schema.generatePromptContext(context.activeEngines) + schema.generateSafetyPrompt();
    }
    ...
    juce::String buildTextToRecipePrompt(const juce::String& description, const SynthContext& context,
                                         const NaturalLanguageInterpreter::InterpretedIntent& interpretation) const
    {
        ...
        if (moodHint.isEmpty())
        {
            ...
            if (descLower.contains("pad") || descLower.contains("ambient") || descLower.contains("atmosphere"))
                ...
                moodHint = "Flux";
            else if (descLower.contains("ethereal") || descLower.contains("otherworld") ||
                     descLower.contains("transcend"))
                moodHint = "Aether";
        }
        ...
        prompt +=
            ...
            "  \"variationAxis\": {\"label\": \"Name\", \"targets\": [\"paramId\"], \"minValues\": [0.0], "
            "\"maxValues\": [1.0]},\n"
            "  \"dna\": {\"brightness\": 0.5, \"warmth\": 0.5, \"movement\": 0.5, \"density\": 0.5, \"space\": 0.5, "
            "\"aggression\": 0.5}\n"
            "}\n\n"
            "CRITICAL RULES:\n"
    ...
    juce::String buildExplanationPrompt(const SynthContext& context, const juce::String& question) const
    {
        return buildSystemContext() + "\n\n" + buildSchemaContext(context) + "\n\n" + contextToString(context) +
               "\n\n"
               "User asks: \"" +
    ...
    juce::String buildRefinePrompt(const juce::String& userRequest, const RecipeEngine::Recipe& currentRecipe,
                                   ...
                                   const RecipeEngine::Recipe::SonicDNA& targetDNA) const
    {
        ...
        prompt +=
            juce::String::formatted("  DNA: {br:%.2f, wa:%.2f, mv:%.2f, de:%.2f, sp:%.2f, ag:%.2f}\n",
                                    currentRecipe.dna.brightness, currentRecipe.dna.warmth, currentRecipe.dna.movement,
                                    currentRecipe.dna.density, currentRecipe.dna.space, currentRecipe.dna.aggression);
        ...
        if (!currentRecipe.coupling.empty())
        {
            ...
            for (const auto& c : currentRecipe.coupling)
                prompt += "    - Slot" + juce::String(c.sourceSlot) + "→Slot" + juce::String(c.destSlot) + " " +
                          c.couplingType + " @" + juce::String(c.intensity, 2) + "\n";
        }

        // Add interpreted direction
        prompt += "\nTARGET DNA (interpreted from user's request):\n";
        prompt += juce::String::formatted("  {br:%.2f, wa:%.2f, mv:%.2f, de:%.2f, sp:%.2f, ag:%.2f}\n\n",
                                          targetDNA.brightness, targetDNA.warmth, targetDNA.movement, targetDNA.density,
                                          targetDNA.space, targetDNA.aggression);
    ...
    juce::String buildSoundMatchPrompt(const juce::String& description, const SynthContext& context,
                                       const NaturalLanguageInterpreter::InterpretedIntent& interpretation) const
    {
    ...
    RecipeResult parseRecipeResponse(const juce::String& response) const
    {
                    ...
                    for (const auto& prop : paramsObj->getProperties())
                    {
                        ...
                        if (paramDef != nullptr)
                        {
                            ...
                            if (std::abs(validated - rawValue) > 0.001f)
                            {
                                ...
                                result.validationWarnings.add(paramId + ": AI suggested " + juce::String(rawValue, 3) +
                        ...
                        else
                        {
                            ...
                            result.validationWarnings.add("Parameter '" + paramId +
                                                          "' not in schema — included unchecked.");
                        }
                    }
                }
        ...
        if (auto* couplingArr = obj->getProperty("coupling").getArray())
        {
            for (const auto& c : *couplingArr)
            {
                ...
                {
                    result.validationWarnings.add("Coupling source=dest (slot " + juce::String(route.sourceSlot) +
                                                  ") — skipped.");
                    continue;
                }
                ...
                if (auto* targets = mObj->getProperty("targets").getArray())
                {
                    for (const auto& t : *targets)
                    {
                        ...
                        else
                        {
                            ...
                            macro.targets.push_back(targetId);
                            result.validationWarnings.add("Macro " + macro.label + " target '" + targetId +
        ...
        {
            auto clampDNA = [](const juce::var& v) { return juce::jlimit(0.0f, 1.0f, static_cast<float>(v)); };
                ...
                if (profile != nullptr)
                {
                    for (const auto& p : profile->parameters)
                    {
                        if (p.role == AIParamRole::Level)
                        {
                            ...
                            result.validationWarnings.add("Auto-fixed: set " + p.paramId + " to " +
                                                          juce::String(p.defaultValue, 2) +
                                                          " to prevent silent output.");
                            result.parametersCorrected++;
                            break;
                        }
                    }
        ...
        for (size_t i = 0; i < result.recipe.macros.size(); ++i)
        {
            ...
            auto macroCheck = schema.validateMacroTargets(macro.label, macro.targets, macro.ranges, allParams);
...
public:
    ...
    static void applySmoothed(const RecipeEngine::Recipe& recipe, juce::AudioProcessorValueTreeState& apvts,
                              const RecipeEngine::ApplyCallbacks& callbacks, const RecipeEngine& recipeEngine,
                              double sampleRate)
    {
        ...
        if (!transitions.empty())
        {
            ...
            auto transitionsCopy = std::make_shared<std::vector<ParameterTransition>>(std::move(transitions));
...
private:
    ...
    static void rampParameters(std::shared_ptr<std::vector<ParameterTransition>> transitions,
                               std::shared_ptr<int> currentStep, int totalSteps, int step)
    {
        if (step >= totalSteps)
        {
        ...
        int nextStep = step + 1;
        juce::MessageManager::callAsync([transitions, currentStep, totalSteps, nextStep]()
