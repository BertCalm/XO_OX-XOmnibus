...
public:
    ...
    struct InterpretedIntent
    {
        ...
        struct DNADelta
        {
            ...
            bool isZero() const
            {
                return std::abs(brightness) < 0.01f && std::abs(warmth) < 0.01f && std::abs(movement) < 0.01f &&
                       std::abs(density) < 0.01f && std::abs(space) < 0.01f && std::abs(aggression) < 0.01f;
            }
        };
        DNADelta dnaDelta;
    ...
    static std::vector<RefinementDirection> parseRefinement(const juce::String& userInput)
    {
        ...
        for (const auto& m : adjMappings)
        {
            if (lower.contains(m.word))
            {
                ...
                if (!lower.contains("more ") && !lower.contains("less ") && !lower.contains("reduce ") &&
                    !lower.contains("increase "))
                    dir = m.weight > 0 ? 1.0f : -1.0f;

                directions.push_back({m.dimension, dir * adjustedMagnitude,
                                      juce::String("Adjust ") + m.dimension + " by " +
                                          juce::String(dir * adjustedMagnitude, 2) + " based on \"" + m.word + "\""});
            }
        }
        ...
        for (const auto& [dim, sumCount] : dimensionSums)
        {
            ...
            merged.push_back({dim, avg, "Adjust " + dim + " by " + juce::String(avg, 2)});
        }
    ...
    static RecipeEngine::Recipe::SonicDNA applyDNADelta(const RecipeEngine::Recipe::SonicDNA& current,
    ...
    static InterpretedIntent::DNADelta refinementsToDNADelta(const std::vector<RefinementDirection>& directions)
    {
        InterpretedIntent::DNADelta delta;
        for (const auto& d : directions)
        {
            if (d.dimension == "brightness")
                delta.brightness += d.magnitude;
            else if (d.dimension == "warmth")
                delta.warmth += d.magnitude;
            else if (d.dimension == "movement")
                delta.movement += d.magnitude;
            else if (d.dimension == "density")
                delta.density += d.magnitude;
            else if (d.dimension == "space")
                delta.space += d.magnitude;
            else if (d.dimension == "aggression")
                delta.aggression += d.magnitude;
        }
        // Clamp deltas to [-1, 1]
...
private:
    ...
    static ExpertiseLevel detectExpertise(const juce::String& lower)
    {
        ...
        static const char* expertTerms[] = {
            ...
            "cutoff", "resonance", "q ", "lfo", "fm ", "oscillator", "envelope", "adsr", "detune", "unison",
            "wavetable", "filter", "modulation", "frequency", "amplitude", "phase", "harmonic", "partial", "overtone",
            "formant", "grain", "granular", "bitcrush", "sample rate", "wavefold", "feedback", "delay time",
            "reverb size", "saturation", "drive", "coupling", "index", "ratio", "operator", "morph position",
            // Advanced DSP terms
            "convolution", "impulse response", "fft", "spectral", "comb filter", "allpass", "karplus", "waveguide",
            "phase distort", "ring mod", "vocoder", "cross-modulation", "sideband", "nyquist", "aliasing", "oversampl",
            "denormal", "dc offset", "zero-crossing", "polyphon",
            // XOceanus-specific
            "sonic dna", "mega coupling", "coupling matrix", "engine slot", "macro assign", "dna delta", "crossfade",
            "hot-swap", "mod depth", "mod rate", "key track", "velocity sens",
            // Specific value patterns
            "hz", "khz", "ms ", "db ", "semitone", "cent"};

        static const char* intermediateTerms[] = {
            // Basic audio terms
            "bass", "treble", "reverb", "delay", "chorus", "distortion", "echo", "sustain", "attack", "release",
            "decay", "pitch", "octave", "note", "key", "chord", "arpeggio", "vibrato", "tremolo", "pan", "stereo",
            "mono", "volume", "tone", "eq", "compress", "sidechain", "layer", "stack",
            // Additional intermediate terms
            "preset", "patch", "waveform", "tempo", "bpm", "sync", "portamento", "glide", "velocity", "aftertouch",
            "mod wheel", "expression", "pitch bend", "midi", "automation", "macro", "wet/dry", "mix knob", "input",
            "output", "gain", "highpass", "lowpass", "bandpass", "shelf", "peak", "phaser", "flanger", "overdrive",
            "fuzz", "limiter", "gate", "noise gate", "send", "return", "bus"};
        ...
        if (intermediateScore >= 2 || expertScore >= 1)
    ...
    static bool containsTechnicalContent(const juce::String& lower)
    {
        ...
        static const char* prefixes[] = {"snap_",    "morph_",  "dub_",     "drift_", "bob_",     "fat_",  "poss_",
                                         "perc_",    "ow_",     "opal_",    "orb_",   "organon_", "ouro_", "obsidian_",
                                         "origami_", "oracle_", "obscura_", "ocean_", "optic_",   "oblq_"};
        ...
        if (lower.contains("audiotofm") || lower.contains("audiotoring") || lower.contains("amptopitch") ||
            lower.contains("lfotopitch"))
            return true;
    ...
    static void applySynestheticMappings(const juce::String& lower, InterpretedIntent& result)
    {
        ...
        for (const auto& c : colors)
        {
            ...
            bool isColorContext =
                ...
                lower.contains("like " + juce::String(c.color)) || lower.contains("the " + juce::String(c.color)) ||
                lower.contains(juce::String(c.color) + " light") || lower.contains(juce::String(c.color) + " glow");
    ...
    static bool isAmbiguousColor(const char* color)
    {
        ...
        return juce::String(color) == "blue" || juce::String(color) == "orange" || juce::String(color) == "gold" ||
               juce::String(color) == "silver";
    }
    ...
    static void suggestEngines(const juce::String& lower, InterpretedIntent& result)
    {
        ...
        if (lower.contains("fm ") || lower.contains("bell") || lower.contains("metallic") || lower.contains("chime") ||
            lower.contains("gong") || lower.contains("clang") || lower.contains("frequency modulation") ||
            lower.contains("operator") || lower.contains("ratio") || lower.contains("sideband") ||
            lower.contains("inharmonic"))
        ...
        if (lower.contains("layer") || lower.contains("stack") || lower.contains("blend") ||
            ...
            lower.contains("fuse") || lower.contains("mix") || lower.contains("ensemble") || lower.contains("unison") ||
            lower.contains("supersaw"))
            result.suggestedEngines.addIfNotAlreadyThere("Overdub");

        // Oblong — bass, sub, low end, weight
        if (lower.contains("bass") || lower.contains("sub") || lower.contains("low end") || lower.contains("bottom") ||
            lower.contains("rumble") || lower.contains("808") || lower.contains("deep bass") ||
            lower.contains("sub bass") || lower.contains("wobble") || lower.contains("reese") ||
            lower.contains("growl"))
            result.suggestedEngines.addIfNotAlreadyThere("Oblong");

        // Onset — drums, percussion, transients, hits
        if (lower.contains("drum") || lower.contains("percuss") || lower.contains("hit") || lower.contains("kick") ||
            lower.contains("snare") || lower.contains("hihat") || lower.contains("hi-hat") ||
            lower.contains("cymbal") || lower.contains("tom") || lower.contains("clap") ||
            lower.contains("transient") || lower.contains("impact") || lower.contains("click") ||
            lower.contains("snap") || lower.contains("rim"))
        ...
        if (lower.contains("retro") || lower.contains("8-bit") || lower.contains("chiptune") ||
            lower.contains("pixel") || lower.contains("game") || lower.contains("nes") || lower.contains("snes") ||
            lower.contains("gameboy") || lower.contains("arcade") || lower.contains("16-bit") ||
            lower.contains("bitcrush") || lower.contains("lo-fi digital"))
        ...
        if (lower.contains("physical") || lower.contains("string") || lower.contains("pluck") ||
            ...
            lower.contains("resonant body") || lower.contains("acoustic model") || lower.contains("stiffness") ||
        ...
        if (lower.contains("wavefold") || lower.contains("phase distort") || lower.contains("nonlinear") ||
            lower.contains("waveshap") || lower.contains("fold") || lower.contains("wrap") || lower.contains("casio") ||
            lower.contains("pd synth") || lower.contains("harsh digital"))
        ...
        if (lower.contains("ocean") || lower.contains("water") || lower.contains("tide") || lower.contains("aquatic") ||
            lower.contains("marine") || lower.contains("underwater") || lower.contains("submarine") ||
            lower.contains("coral") || lower.contains("whale") || lower.contains("dolphin") ||
            lower.contains("current") || lower.contains("wave pool"))
            result.suggestedEngines.addIfNotAlreadyThere("Oceanic");

        // Overbite — bite, teeth, jaw, gnaw, chomp
        if (lower.contains("bite") || lower.contains("teeth") || lower.contains("jaw") || lower.contains("gnaw") ||
            lower.contains("chomp") || lower.contains("gnash") || lower.contains("crunch") || lower.contains("chew") ||
    ...
    static void suggestFX(const juce::String& lower, InterpretedIntent& result)
    {
        ...
        if (lower.contains("distort") || lower.contains("saturate") || lower.contains("overdrive") ||
            lower.contains("grit") || lower.contains("fuzz") || lower.contains("clip") || lower.contains("crunch") ||
            lower.contains("tube") || lower.contains("tape") || lower.contains("warm") || lower.contains("analog") ||
            lower.contains("drive") || lower.contains("dirty") || lower.contains("harsh") || lower.contains("bitcrush"))
        ...
        if (lower.contains("eq") || lower.contains("filter") || lower.contains("tone") || lower.contains("low-pass") ||
            lower.contains("high-pass") || lower.contains("band-pass") || lower.contains("notch") ||
            lower.contains("resonan") || lower.contains("formant"))
    ...
    static juce::String buildEnrichedPrompt(const InterpretedIntent& result, const juce::String& originalInput)
    {
        juce::String prompt;
        ...
        if (!result.dnaDelta.isZero())
        {
            ...
            auto addIfNonZero = [&](const char* name, float val)
            {
                if (std::abs(val) >= 0.05f)
                    prompt += juce::String("  ") + name + ": " + (val > 0 ? "+" : "") + juce::String(val, 2) + "\n";
            };
        ...
        if (result.suggestedEngines.size() > 0)
            prompt += "SUGGESTED ENGINES (consider these): " + result.suggestedEngines.joinIntoString(", ") + "\n\n";

        // Add FX hints
        if (result.suggestedFX.size() > 0)
            prompt += "SUGGESTED FX (consider these): " + result.suggestedFX.joinIntoString(", ") + "\n\n";
