...
public:
    ...
    void recordPresetSaved (float brightness, float warmth, float movement,
                            float density, float space, float aggression)
    {
        if (privacy.consent < ConsentLevel::Full) return;
        std::lock_guard<std::mutex> lock (dataMutex);
    ...
    void recordAIQuery (const juce::String& queryCategory)
    {
        ...
        if (queryCategory == "bass")            currentBatch.aiThemes.bassQueries++;
        else if (queryCategory == "pad")        currentBatch.aiThemes.padQueries++;
        else if (queryCategory == "lead")       currentBatch.aiThemes.leadQueries++;
        else if (queryCategory == "fx")         currentBatch.aiThemes.fxQueries++;
        else if (queryCategory == "percussion") currentBatch.aiThemes.percussionQueries++;
        else if (queryCategory == "ambient")    currentBatch.aiThemes.ambientQueries++;
        else if (queryCategory == "aggressive") currentBatch.aiThemes.aggressiveQueries++;
        else                                    currentBatch.aiThemes.experimentalQueries++;
