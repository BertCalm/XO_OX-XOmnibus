...
struct AIParamDef
{
    ...
    float clamp(float value) const noexcept { return std::max(minValue, std::min(maxValue, value)); }

    /// Clamp to the safe range (AI-suggested values go through this)
    float clampSafe(float value) const noexcept { return std::max(minValue, std::min(safeMax, value)); }
    ...
    float validate(float value) const noexcept { return quantize(clampSafe(value)); }

    /// Check if a value is in the sweet spot
    bool isInSweetSpot(float value) const noexcept { return value >= sweetSpotMin && value <= sweetSpotMax; }
};
...
public:
    ...
    void registerEngine(AIEngineProfile profile) { engineProfiles[profile.engineId] = std::move(profile); }
    ...
    void addSafetyRule(AISafetyRule rule) { safetyRules.push_back(std::move(rule)); }
    ...
    ValidationResult validateParameters(const std::map<juce::String, float>& params) const
    {
        ...
        for (const auto& [paramId, value] : params)
        {
            ...
            {
                result.warnings.add(paramId + ": " + juce::String(value, 3) + " → " + juce::String(corrected, 3) +
                                    " (clamped to safe range)");
            }

            if (!def->isInSweetSpot(corrected))
            {
                result.warnings.add(paramId + ": value " + juce::String(corrected, 3) + " is outside sweet spot [" +
                                    juce::String(def->sweetSpotMin, 2) + "-" + juce::String(def->sweetSpotMax, 2) +
                                    "]");
            }
        }
        ...
        for (const auto& rule : safetyRules)
        {
            ...
            for (const auto& cond : rule.conditions)
            {
                ...
                float val = it->second;
                if (cond.above && val <= cond.threshold)
                    triggered = false;
                if (!cond.above && val >= cond.threshold)
                    triggered = false;
            }
            ...
            if (triggered)
            {
                for (const auto& fix : rule.fixes)
                {
                    ...
                    if (it != result.correctedParams.end())
                    {
                        ...
                        {
                            result.warnings.add("Safety rule '" + rule.name + "': " + fix.paramId + " adjusted to " +
    ...
    ValidationResult validateCoupling(const juce::String& couplingType, float intensity) const
    {
        ValidationResult result;
        ...
        {
            result.warnings.add(couplingType + " intensity clamped from " + juce::String(intensity, 3) + " to " +
                                juce::String(corrected, 3) + " — " + constraint->safetyNote);
        }
    ...
    static DNAValidation validateDNAForMood(const RecipeEngine::Recipe::SonicDNA& dna, const juce::String& mood)
    {
        ...
        auto check = [&](float& val, float minV, float maxV, const char* name)
        {
            if (val < minV)
            {
                result.warnings.add(juce::String("DNA ") + name + " (" + juce::String(val, 2) + ") is low for mood '" +
                                    mood + "' — expected >= " + juce::String(minV, 2) + ". Nudging up.");
                val = minV;
                result.adjusted = true;
            }
            if (val > maxV)
            {
                result.warnings.add(juce::String("DNA ") + name + " (" + juce::String(val, 2) + ") is high for mood '" +
                                    mood + "' — expected <= " + juce::String(maxV, 2) + ". Nudging down.");
                val = maxV;
                result.adjusted = true;
            }
        };
    ...
    MacroValidation validateMacroTargets(const juce::String& macroLabel, const std::vector<juce::String>& targets,
                                         ...
                                         const std::map<juce::String, float>& currentParams) const
    {
        ...
        for (size_t i = 0; i < targets.size(); ++i)
        {
            ...
            if (def == nullptr)
                continue; // Unknown param — can't validate

            // Macros shouldn't target discrete params
            if (def->type == AIParamType::Choice || def->type == AIParamType::Toggle)
            {
                result.warnings.add("Macro " + macroLabel + " targets '" + targets[i] + "' which is a " +
            ...
            if (i < ranges.size())
            {
                ...
                if (it != currentParams.end())
                {
                    ...
                    {
                        result.warnings.add("Macro " + macroLabel + " range on '" + targets[i] +
                                            "' may exceed parameter bounds at extremes.");
                    }
                }
            }
        }
    ...
    void addFewShotExample(FewShotExample example) { fewShotExamples.push_back(std::move(example)); }
    ...
    juce::String generateFewShotContext(const juce::String& targetMood = {}) const
    {
        ...
        for (const auto& ex : fewShotExamples)
        {
            ...
            if (++count >= 3)
                break; // Max 3 examples to control prompt size
        }
    ...
    juce::String generatePromptContext(const juce::StringArray& activeEngines) const
    {
        ...
        for (const auto& engineId : activeEngines)
        {
            ...
            ctx += "ENGINE: " + profile->displayName + " (prefix: " + profile->paramPrefix + ")\n";
            ...
            for (const auto& p : profile->parameters)
            {
                ...
                {
                    ctx += "[" + juce::String(p.minValue, 1) + "-" + juce::String(p.maxValue, 1) + "] ";
                    ctx += "default=" + juce::String(p.defaultValue, 2) + " ";
                    ctx += "sweet=[" + juce::String(p.sweetSpotMin, 2) + "-" + juce::String(p.sweetSpotMax, 2) + "]";
                }
        ...
        {
            ctx += type + " (max intensity: " + juce::String(constraint.maxSafeIntensity, 2) + ") — " +
        ...
        for (const auto& engineId : activeEngines)
        {
            ...
            {
                ctx += profile->displayName + " pairs well with: " + profile->recommendedPairings.joinIntoString(", ") +
                       "\n";
            }
            if (!profile->bestSendTypes.isEmpty())
            {
                ctx += profile->displayName + " sends: " + profile->bestSendTypes.joinIntoString(", ") + "\n";
            }
            if (!profile->bestReceiveTypes.isEmpty())
            {
                ctx += profile->displayName + " receives: " + profile->bestReceiveTypes.joinIntoString(", ") + "\n";
            }
        }
...
inline AIParameterSchema buildDefaultSchema()
{
    ...
    // ODDFELIX (OddfeliX) — Percussive transient synthesis
    {
        ...
        p.parameters = {{"snap_oscMode",
                         "Osc Mode",
                         "OddfeliX",
                         AIParamType::Choice,
                         AIParamRole::OscillatorShape,
                         0,
                         2,
                         0,
                         1,
                         1,
                         {"Sine+Noise", "FM", "Karplus-Strong"},
                         0,
                         2,
                         "Sine+Noise=round, FM=metallic, KS=plucky strings",
                         false,
                         2.0f,
                         ""},
                        {"snap_snap",
                         ...
                         0.3f,
                         0.6f,
                         "Transient intensity. Higher = more click.",
                         false,
                         1.0f,
                         ""},
                        {"snap_decay",
                         ...
                         0.1f,
                         0.4f,
                         "Envelope decay time. Short for clicks, longer for toms.",
                         false,
                         8.0f,
                         ""},
                        {"snap_filterCutoff",
                         "Filter Cutoff",
                         "OddfeliX",
                         AIParamType::Frequency,
                         AIParamRole::FilterCutoff,
                         20,
                         20000,
                         2000,
                         0.1f,
                         ...
                         1000,
                         4000,
                         "HPF→BPF cascade cutoff. Lower=warmer, higher=brighter.",
                         false,
                         20000,
                         ""},
                        {"snap_filterReso",
                         ...
                         0.2f,
                         0.5f,
                         "Resonance. Higher adds ring to percussion.",
                         true,
                         0.85f,
                        ...
                        {"snap_detune",
                         "Detune",
                         "OddfeliX",
                         AIParamType::Continuous,
                         AIParamRole::UnisonDetune,
                         0,
                         50,
                         10,
                         0.1f,
                         ...
                         5,
                         15,
                         "Unison detune spread in cents. Wider = bigger.",
                         false,
                         50,
                         ""},
                        {"snap_level",
                        ...
                        {"snap_pitchLock",
                         ...
                         0,
                         1,
                         "When on, pitch doesn't track keyboard — fixed pitch mode.",
                         false,
                         1.0f,
                         ""},
                        {"snap_unison",
                         ...
                         0,
                         2,
                         "Unison voice count. More = wider stereo.",
                         false,
                         2.0f,
                         ""},
                        {"snap_polyphony",
                         ...
                         0,
                         3,
                         "Polyphony mode. 1=mono, 8=full poly.",
                         false,
                         3.0f,
                         ""}};
    ...
    // ODDOSCAR (OddOscar) — Lush pad synthesis
    {
        ...
        p.parameters = {{"morph_scanPos",
                         "Scan Position",
                         "OddOscar",
                         AIParamType::Unipolar,
                         AIParamRole::MorphPosition,
                         0,
                         1,
                         0.3f,
                         0.001f,
                         ...
                         0.2f,
                         0.6f,
                         "Wavetable morph. 0=Sine, 0.33=Saw, 0.66=Square, 1=Noise",
                         false,
                         1.0f,
                         ""},
                        {"morph_filterCutoff",
                        ...
                        {"morph_filterReso",
                         ...
                         0.3f,
                         0.6f,
                         "Ladder resonance. Self-oscillates above 0.9.",
                         true,
                         0.88f,
                        ...
                        {"morph_drift",
                         ...
                         0.1f,
                         0.3f,
                         "Perlin noise pitch drift. Analog warmth feel.",
                         false,
                         1.0f,
                         ""},
                        {"morph_subLevel",
                         ...
                         0.3f,
                         0.5f,
                         "Sub oscillator level (one octave below). Adds warmth and bass.",
                         false,
                         1.0f,
                         ""}};
    ...
    // Coupling constraints
    {
        ...
        schema.registerCouplingConstraint({"LFOToPitch",
                                           0.6f,
        ...
        schema.registerCouplingConstraint(
            {"EnvToMorph",
             ...
             {},
             ""});
        schema.registerCouplingConstraint({"AudioToFM",
                                           0.7f,
                                           "Engine A audio directly FM-modulates Engine B. Creates metallic/bell tones "
                                           "at low intensity, noise at high.",
                                           {},
        ...
        schema.registerCouplingConstraint({"FilterToFilter",
                                           1.0f,
                                           "Engine A filter output feeds Engine B filter input. Cascaded filtering.",
                                           {},
                                           ""});
        schema.registerCouplingConstraint(
            {"AmpToChoke", 1.0f, "Engine A amplitude ducks Engine B. Sidechain/ducking effect.", {}, ""});
        schema.registerCouplingConstraint(
            {"RhythmToBlend", 1.0f, "Engine A rhythm pattern controls Engine B blend. Rhythmic crossfading.", {}, ""});
        schema.registerCouplingConstraint({"EnvToDecay",
                                           0.8f,
                                           "Engine A envelope modulates Engine B decay time. Interactive dynamics.",
                                           {},
                                           "High values can make Engine B inaudible"});
        schema.registerCouplingConstraint({"PitchToPitch",
                                           0.5f,
                                           "Engine A pitch tracks to Engine B pitch. Harmony/tracking.",
                                           {"AudioToFM"},
                                           "Above 0.3 creates strong forced harmony — can clash"});
        schema.registerCouplingConstraint({"AudioToWavetable",
                                           0.8f,
    ...
    // Safety rules
    {
        ...
        schema.addSafetyRule({"resonance_cutoff_boom",
                              "High filter resonance with very low cutoff can cause boomy DC buildup",
                              {{"morph_filterReso", 0.8f, true}, {"morph_filterCutoff", 100.0f, false}},
                              {
        ...
        schema.addSafetyRule({"snap_dc_click",
                              "Very high snap intensity with very short decay can produce DC clicks",
                              {{"snap_snap", 0.9f, true}, {"snap_decay", 0.02f, false}},
                              {
    ...
    auto registerStub = [&](const juce::String& id, const juce::String& prefix, const juce::String& character,
    ...
    schema.addFewShotExample(
        {"Foundation", "Tight punchy kick with analog warmth",
    ...
    schema.addFewShotExample(
        {"Atmosphere", "Warm evolving pad with slow drift and shimmer",
    ...
    schema.addFewShotExample(
        {"Entangled", "Two engines feeding back into each other with chaotic FM",
    ...
    schema.addFewShotExample(
        {"Prism", "Bright crystalline bell with spectral shimmer",
    ...
    schema.addFewShotExample(
        {"Flux", "Restless morphing bass that never sits still",
    ...
    schema.addFewShotExample(
        {"Aether", "Ethereal otherworldly texture floating in space",
