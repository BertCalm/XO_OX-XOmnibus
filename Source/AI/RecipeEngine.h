...
//==============================================================================
// RecipeEngine — One-touch configuration of the entire XOceanus architecture.
//
...
public:
    ...
    void scanRecipeDirectory(const juce::File& directory)
    {
        ...
        auto files = directory.findChildFiles(juce::File::findFiles, true, "*.xorecipe");
    ...
    using SetCouplingCallback = std::function<void(int src, int dst, const juce::String& type, float intensity,
                                                   const std::map<juce::String, float>& params)>;
    using SetMacroCallback =
        std::function<void(int macroIndex, const juce::String& label, const std::vector<juce::String>& targets,
    ...
    bool applyRecipe(const Recipe& recipe, juce::AudioProcessorValueTreeState& apvts,
                     const ApplyCallbacks& callbacks) const
    {
        ...
        if (callbacks.setCoupling)
        {
            ...
            {
                callbacks.setCoupling(route.sourceSlot, route.destSlot, route.couplingType, route.intensity,
                                      route.routeParams);
            }
        }
    ...
    static bool saveRecipe(const Recipe& recipe, const juce::File& file)
    {
        ...
        if (recipe.variationAxis.has_value())
        {
            ...
            for (const auto& t : recipe.variationAxis->targets)
                targets.add(t);
            for (const auto& v : recipe.variationAxis->minValues)
                mins.add(v);
            for (const auto& v : recipe.variationAxis->maxValues)
                maxs.add(v);
