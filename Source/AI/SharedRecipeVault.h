...
public:
    ...
    ShareResult shareRecipe(const juce::String& recipeJSON, const juce::String& description,
                            const juce::StringArray& tags)
    {
        ...
        auto options = juce::URL::InputStreamOptions(juce::URL::ParameterHandling::inPostBody)
                           ...
                           .withExtraHeaders(supabaseHeaders(localCfg) + "Prefer: return=representation\r\n");
    ...
    BrowseResult browse(const BrowseFilter& filter)
    {
        ...
        switch (filter.sort)
        {
        case BrowseFilter::SortBy::Newest:
            endpoint += "&sort=newest";
            break;
        case BrowseFilter::SortBy::Popular:
            endpoint += "&sort=popular";
            break;
        case BrowseFilter::SortBy::StaffPicks:
            endpoint += "&sort=staffpicks";
            break;
        }
    ...
    DownloadResult downloadRecipe(const juce::String& recipeId)
    {
        ...
        juce::String endpoint = localCfg.supabaseUrl +
                                ...
                                "?recipe_id=eq." +
                                juce::URL::addEscapeChars(recipeId, true) + "&select=*";
    ...
    bool deleteOwnRecipe(const juce::String& recipeId)
    {
        ...
        auto options = juce::URL::InputStreamOptions(juce::URL::ParameterHandling::inAddress)
                           .withConnectionTimeoutMs(localCfg.timeoutMs)
                           .withExtraHeaders(supabaseHeaders(localCfg) + "X-HTTP-Method-Override: DELETE\r\n" +
    ...
    void shareAsync(const juce::String& recipeJSON, const juce::String& description, const juce::StringArray& tags)
    {
...
private:
    ...
    static RecipeMetadata parseMetadataFromRow(const juce::var& row)
    {
        ...
        m.sharedTimestamp =
            static_cast<int64_t>(juce::Time::fromISO8601(row.getProperty("shared_at", "").toString()).toMilliseconds());
    ...
    {
    public:
        BrowseThread(SharedRecipeVault& v, BrowseFilter f) : juce::Thread("VaultBrowse"), vault(v), filter(std::move(f))
        {
        }
        ...
        void run() override
        {
            ...
            juce::MessageManager::callAsync(
                [this, result, destroyedFlag = &vault.destroyed_]()
                {
    ...
    {
    public:
        ShareThread(SharedRecipeVault& v, juce::String json, juce::String desc, juce::StringArray t)
            : juce::Thread("VaultShare"), vault(v), recipeJSON(std::move(json)), description(std::move(desc)),
              tags(std::move(t))
        {
        }
