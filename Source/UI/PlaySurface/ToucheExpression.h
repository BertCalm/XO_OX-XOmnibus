...
public:
    ...
    ToucheExpression() { setOpaque(false); }
    ...
    void setSmoothingMs(float ms) { smoothMs = std::max(1.0f, ms); }
    ...
    ExpressionState getState() const { return stateBuffer[readIndex.load(std::memory_order_acquire)]; }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.setFont(10.0f);
        g.drawText(modeName(mode), bounds.reduced(4.0f), juce::Justification::topLeft, false);
        ...
        g.fillRoundedRectangle(bounds.getX(), bounds.getBottom() - fillH, bounds.getWidth(), fillH, 4.0f);
    ...
    void mouseDrag(const juce::MouseEvent& event) override { updateFromMouse(event); }
...
private:
    ...
    void updateFromMouse(const juce::MouseEvent& event)
    {
        ...
        // X axis → lateral (0-1)
        currentState.lateral =
            std::clamp((static_cast<float>(event.x) - bounds.getX()) / bounds.getWidth(), 0.0f, 1.0f);

        // Y axis → pressure (inverted: bottom = 0, top = 1)
        currentState.pressure =
            std::clamp(1.0f - (static_cast<float>(event.y) - bounds.getY()) / bounds.getHeight(), 0.0f, 1.0f);
    ...
    static juce::Colour modeColour(Mode m)
    {
        ...
        case Mode::Timbral:
            return juce::Colour(0xFFA78BFA); // Lavender (Opal)
        case Mode::Spatial:
            return juce::Colour(0xFF00B4A0); // Phosphorescent Teal
        case Mode::Expression:
