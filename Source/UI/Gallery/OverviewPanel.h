...
{
public:
    explicit OverviewPanel(XOceanusProcessor& proc) : processor(proc), chainView(proc)
    {
    ...
    void refresh()
    {
        ...
        for (int i = 0; i < XOceanusProcessor::MaxSlots; ++i)
        {
            ...
            if (eng)
                cachedActiveEngines.push_back({eng->getEngineId(), eng->getAccentColour()});
        }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        if (active.empty())
        {
            ...
            g.drawText("XO", (int)markX, (int)markY, (int)markR * 2, (int)markR * 2, juce::Justification::centred);

            g.setColour(get(textMid()));
            g.setFont(GalleryFonts::body(12.0f));
            float instrY = h * 0.30f + markR + 16.0f;
            g.drawText("Click an engine tile to edit parameters", b.withY(instrY).withHeight(20.0f).toNearestInt(),
            ...
            g.drawText("No engines loaded — use the tiles on the left",
                       b.withY(h * 0.65f).withHeight(20.0f).toNearestInt(), juce::Justification::centred);
            return;
        }
        ...
        juce::ColourGradient wash(accent.withAlpha(0.12f), 0.0f, 0.0f, juce::Colours::transparentBlack, w * 0.70f, 0.0f,
                                  false);
        ...
        // ── Mini arc node diagram — replaces text-row coupling route list ────────
        {
            ...
            if (arcPathDirty)
            {
                ...
                {
                    if (!route.active || route.amount < 0.005f)
                        continue;
            ...
            // Draw each cached arc segment using its stored colour and alpha.
            {
                ...
                for (const auto& route : routes)
                {
                    ...
                    if (arcIdx >= (int)cachedArcColors.size())
                        break;
                    ...
                    g.setColour(cachedArcColors[static_cast<size_t>(arcIdx)].withAlpha(
                        cachedArcAlphas[static_cast<size_t>(arcIdx)]));
            ...
            for (int i = 0; i < MegaCouplingMatrix::MaxSlots; ++i)
            {
                ...
                juce::Colour nodeCol =
                    hasEng ? cachedActiveEngines[static_cast<size_t>(i)].second : juce::Colour(0xFF3A3938); // T4
                ...
                g.drawText(juce::String(i + 1), (int)(nodePos[i].x - 6.0f), (int)(nodePos[i].y - 6.0f), 12, 12,
                           juce::Justification::centred);
            }
