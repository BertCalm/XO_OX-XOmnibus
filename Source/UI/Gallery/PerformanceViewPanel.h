...
public:
              ...
              })
    {
        ...
        bakeBtn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::xoGold).withAlpha(0.15f));
        ...
        clearBtn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::slotBg()));
        clearBtn.setColour(juce::TextButton::textColourOffId, GalleryColors::get(GalleryColors::textMid()));
        ...
        couplingPresetBox.setColour(juce::ComboBox::outlineColourId, GalleryColors::get(GalleryColors::borderGray()));
        couplingPresetBox.setColour(juce::ComboBox::textColourId, GalleryColors::get(GalleryColors::textDark()));
        ...
        for (int r = 0; r < kNumRoutes; ++r)
        {
            ...
            section.activeBtn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::slotBg()));
            section.activeBtn.setColour(juce::TextButton::buttonOnColourId, GalleryColors::get(GalleryColors::xoGold));
            ...
            section.depthSlider.setColour(juce::Slider::thumbColourId, GalleryColors::get(GalleryColors::xoGold));
            section.depthSlider.setColour(juce::Slider::trackColourId, GalleryColors::get(GalleryColors::borderGray()));
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            macroKnobs[i].setColour(juce::Slider::rotarySliderFillColourId, GalleryColors::get(GalleryColors::xoGold));
            macroKnobs[i].setTooltip(juce::String("Macro ") + juce::String(i + 1) + ": " + macroDefs[i].label);
            A11y::setup(macroKnobs[i], juce::String("Macro ") + juce::String(i + 1) + " " + macroDefs[i].label);
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.drawText("PERFORMANCE VIEW", headerArea.reduced(12, 0), juce::Justification::centredLeft);
        ...
        auto routeArea = body.withTrimmedLeft((int)(body.getWidth() * kStripRatio) + 1).reduced(6, 4);
        int routeH = routeArea.getHeight() / kNumRoutes;

        for (int r = 0; r < kNumRoutes; ++r)
        {
            auto sectionRect =
                routeArea.withY(routeArea.getY() + r * routeH).withHeight(routeH).reduced(0, 2).toFloat();
    ...
    void resized() override
    {
        ...
        for (int r = 0; r < kNumRoutes; ++r)
        {
            auto section = routePanel.withY(routePanel.getY() + r * routeH).withHeight(routeH).reduced(4, 4);
    ...
    void handleBake()
    {
        ...
        if (!snapshot.hasActiveRoutes())
        {
            ...
            juce::Timer::callAfterDelay(400,
                                        [safeThis = juce::Component::SafePointer<PerformanceViewPanel>(this)]
                                        {
                                            if (safeThis)
                                            {
        ...
        auto* alertWindow = new juce::AlertWindow(
            "Bake Coupling Preset", "Enter a name for this coupling configuration:", juce::MessageBoxIconType::NoIcon,
            this);
