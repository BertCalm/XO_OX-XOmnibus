...
public:
        ...
        : apvts_(apvts)
    {
        ...
        for (auto* s : {&stepsSlider_, &humanizeSlider_, &baseVelSlider_, &rootNoteSlider_})
        {
            ...
            s->setColour(juce::Slider::trackColourId,      juce::XO::Tokens::Color::accent());
...
public:
    explicit EngineDetailPanel(XOceanusProcessor& proc) : processor(proc), macroHero(proc), waveformDisplay(proc), modMatrix_(proc.getAPVTS()), seqSection_(proc.getAPVTS())
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            adsrSliders[i].setColour(juce::Slider::trackColourId, juce::XO::Tokens::Color::accent());
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        if (!modTabBounds_.isEmpty())
        {
            ...
            g.setColour(juce::XO::Tokens::Color::accent().withAlpha(0.15f));
