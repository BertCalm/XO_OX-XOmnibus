...
public:
    ...
    explicit CouplingInspectorPanel(XOceanusProcessor& proc)
        : processor(proc), apvts(proc.getAPVTS()),
          miniViz(
              ...
              {
                  auto* eng = proc.getEngine(slot);
                  return eng ? eng->getAccentColour() : juce::Colour(0xFF555555);
              })
    {
        ...
        for (int r = 0; r < kNumRoutes; ++r)
        {
            ...
            card.headerBtn.setColour(juce::TextButton::buttonColourId, juce::Colours::transparentBlack);
            card.headerBtn.setColour(juce::TextButton::buttonOnColourId, juce::Colours::transparentBlack);
            card.headerBtn.setColour(juce::TextButton::textColourOffId, GalleryColors::get(GalleryColors::t2()));
            card.headerBtn.setColour(juce::TextButton::textColourOnId, GalleryColors::get(GalleryColors::t1()));
            A11y::setup(card.headerBtn, routeLabel + " expand/collapse",
            ...
            card.activeBtn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::raised()));
            card.activeBtn.setColour(juce::TextButton::buttonOnColourId,
                                     juce::Colour(0xFF2D6A4F)); // muted forest green
            card.activeBtn.setColour(juce::TextButton::textColourOffId, GalleryColors::get(GalleryColors::t3()));
            card.activeBtn.setColour(juce::TextButton::textColourOnId,
                                     juce::Colour(0xFF52B788)); // bright mint when on
            A11y::setup(card.activeBtn, "Route " + juce::String(r + 1) + " enable",
            ...
            card.typeBox.setTooltip("AmpToFilter: source amplitude \xe2\x86\x92 target filter cutoff | "
            ...
            card.typeBox.setColour(juce::ComboBox::arrowColourId, GalleryColors::get(GalleryColors::t3()));
            A11y::setup(card.typeBox, "Route " + juce::String(r + 1) + " coupling type",
            ...
            card.sourceBox.addItemList({"Slot 1", "Slot 2", "Slot 3", "Slot 4", "Slot 5 (Ghost)"}, 1);
            card.sourceBox.setColour(juce::ComboBox::backgroundColourId, GalleryColors::get(GalleryColors::raised()));
            card.sourceBox.setColour(juce::ComboBox::outlineColourId, GalleryColors::border().withAlpha(0.18f));
            card.sourceBox.setColour(juce::ComboBox::textColourId, GalleryColors::get(GalleryColors::t1()));
            card.sourceBox.setColour(juce::ComboBox::arrowColourId, GalleryColors::get(GalleryColors::t3()));
            A11y::setup(card.sourceBox, "Route " + juce::String(r + 1) + " source slot",
            ...
            card.targetBox.addItemList({"Slot 1", "Slot 2", "Slot 3", "Slot 4", "Slot 5 (Ghost)"}, 1);
            card.targetBox.setColour(juce::ComboBox::backgroundColourId, GalleryColors::get(GalleryColors::raised()));
            card.targetBox.setColour(juce::ComboBox::outlineColourId, GalleryColors::border().withAlpha(0.18f));
            card.targetBox.setColour(juce::ComboBox::textColourId, GalleryColors::get(GalleryColors::t1()));
            card.targetBox.setColour(juce::ComboBox::arrowColourId, GalleryColors::get(GalleryColors::t3()));
            A11y::setup(card.targetBox, "Route " + juce::String(r + 1) + " target slot",
        ...
        bakeBtn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::xoGold).withAlpha(0.18f));
        bakeBtn.setColour(juce::TextButton::buttonOnColourId,
                          GalleryColors::get(GalleryColors::xoGold).withAlpha(0.35f));
        bakeBtn.setColour(juce::TextButton::textColourOffId, GalleryColors::get(GalleryColors::xoGold));
        bakeBtn.setColour(juce::TextButton::textColourOnId, GalleryColors::get(GalleryColors::xoGold));
        A11y::setup(bakeBtn, "Bake Coupling", "Save the current coupling route configuration as a named preset");
        ...
        clearBtn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::raised()));
        clearBtn.setColour(juce::TextButton::buttonOnColourId,
                           GalleryColors::get(GalleryColors::raised()).brighter(0.1f));
        clearBtn.setColour(juce::TextButton::textColourOffId, GalleryColors::get(GalleryColors::t3()));
        clearBtn.setColour(juce::TextButton::textColourOnId, GalleryColors::get(GalleryColors::t2()));
        A11y::setup(clearBtn, "Clear Coupling", "Deactivate all coupling routes and reset the overlay");
        ...
        couplingPresetBox.setColour(juce::ComboBox::backgroundColourId, GalleryColors::get(GalleryColors::raised()));
        couplingPresetBox.setColour(juce::ComboBox::outlineColourId, GalleryColors::border().withAlpha(0.18f));
        couplingPresetBox.setColour(juce::ComboBox::textColourId, GalleryColors::get(GalleryColors::t2()));
        couplingPresetBox.setColour(juce::ComboBox::arrowColourId, GalleryColors::get(GalleryColors::t3()));
        A11y::setup(couplingPresetBox, "Coupling Preset Selector", "Choose a saved coupling preset to load");
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.setColour(get(t1()));
        g.drawText("COUPLING ROUTES", juce::Rectangle<int>(kCardMargin, 6, getWidth() - kCardMargin * 2, 20),
        ...
        for (int r = 0; r < kNumRoutes; ++r)
        {
            ...
            juce::Rectangle<float> cardRect(static_cast<float>(kCardMargin), static_cast<float>(y + kCardGap),
            ...
            if (active)
            {
                ...
                juce::Colour barColor =
                    srcEngForBar ? srcEngForBar->getAccentColour() : juce::Colour(0xFF52B788); // fallback mint
                g.setColour(barColor.withAlpha(0.9f));
                g.fillRoundedRectangle(cardRect.getX(), cardRect.getY() + 5.0f, 3.0f, cardRect.getHeight() - 10.0f,
                                       1.5f);
            }
            ...
            // ── Route number label (T2 when inactive, T1 when active) ─────────
            {
                ...
                g.drawText(routeNumStr,
                           juce::Rectangle<int>((int)cardRect.getX() + kInnerPad + (active ? 5 : 3),
                                                (int)cardRect.getY(), 28, kCardCollapsedH - kCardGap),
            ...
            if (!routeCards[r].expanded)
            {
                ...
                if (!active)
                {
                    ...
                    g.drawText(placeholder,
                               juce::Rectangle<float>(cardRect.getX() + kInnerPad + 30.0f, cardRect.getY(),
                ...
                g.drawText(srcName, juce::Rectangle<float>(sx, sy, srcW + 4.0f, sh), juce::Justification::centredLeft,
                           false);
                sx += srcW + 4.0f;
                ...
                g.setColour(get(t4()));
                g.drawText(arrow, juce::Rectangle<float>(sx, sy, arrowW + 2.0f, sh), juce::Justification::centredLeft,
                           false);
                sx += arrowW + 2.0f;
                ...
                g.drawText(tgtName, juce::Rectangle<float>(sx, sy, tgtW + 4.0f, sh), juce::Justification::centredLeft,
                           false);
                sx += tgtW + 4.0f;
                ...
                if (typeShort.isNotEmpty())
                {
                    ...
                    g.drawText(badge, juce::Rectangle<float>(sx, sy, badgeW + 4.0f, sh),
                               juce::Justification::centredLeft, false);
                }
            }
            ...
            if (routeCards[r].expanded)
            {
                ...
                // "TYPE" label
                g.drawText("TYPE",
                           juce::Rectangle<int>((int)cardRect.getX() + kInnerPad,
                                                (int)cardRect.getY() + kHeaderRowH + kRowGap, 50, kLabelH),
                           juce::Justification::centredLeft);
        ...
        // ── Routing hint: strongest active route summary ──────────────────────
        {
            ...
            if (bestRoute >= 0)
            {
                ...
                juce::String hintText = srcName + juce::String(juce::CharPointer_UTF8(" \xe2\x86\x92 ")) + tgtName +
                                        ": " + typeShort + " " + juce::String(bestAmount, 2);
        ...
        g.setColour(border());
        g.drawHorizontalLine(getHeight() - kActionRowH, 0.0f, static_cast<float>(getWidth()));
    ...
    void resized() override
    {
        ...
        for (int r = 0; r < kNumRoutes; ++r)
        {
            ...
            card.activeBtn.setBounds(cardX + cardW - kActiveBtnW, cardY + 3, kActiveBtnW - 4, kHeaderRowH - 6);
            ...
            if (card.expanded)
            {
                ...
                card.targetBox.setBounds(cardX + kInnerPad + halfW + kArrowW, srcTgtY, halfW, kComboRowH);
...
private:
    ...
    static constexpr int kCardExpandedH =
        kCardGap + kHeaderRowH + kRowGap + kLabelH + kTypeRowH + kRowGap + kLabelH + kComboRowH + kInnerPad + 2;
    ...
    juce::String getTypeShortLabel(int r) const
    {
        ...
        if (auto* param = apvts.getRawParameterValue(paramId))
        {
            ...
            static const char* kLabels[] = {"Amp>F", "Amp>P", "LFO>P", "Env>M", "FM",    "Ring", "F>F", "Choke",
                                            "Rhy>B", "Env>D", "P>P",   "WT",    "A>Buf", "KNOT", "TRI"};
            if (idx >= 0 && idx < 15)
                return kLabels[idx];
        }
        return {};
    ...
    void refreshPresetList()
    {
        ...
        if (names.isEmpty())
        {
            ...
            couplingPresetBox.setColour(juce::ComboBox::textColourId, GalleryColors::get(GalleryColors::t3()));
        }
        else
        {
            couplingPresetBox.setTextWhenNothingSelected("Coupling Presets...");
    ...
    void handleBake()
    {
        ...
        auto* dlg = new juce::AlertWindow("Bake Coupling Preset", "Enter a name for this coupling configuration:",
        ...
        dlg->enterModalState(
            ...
            juce::ModalCallbackFunction::create(
                [safeThis = juce::Component::SafePointer<CouplingInspectorPanel>(this), dlg](int result)
                {
    ...
    void performBake(const juce::String& presetName)
    {
        ...
        auto sanitized = presetName.replaceCharacters(" /\\:*?\"<>|", "___________");
        auto file = dir.getChildFile(sanitized + ".xocoupling");

        int suffix = 1;
        while (file.existsAsFile() && suffix < 100)
        {
            file = dir.getChildFile(sanitized + "_" + juce::String(suffix) + ".xocoupling");
            ++suffix;
        }
