...
public:
    ...
    {
        // ── Trigger pads ──────────────────────────────────────────────────────
        auto makePad = [&](juce::TextButton& btn, const juce::String& label, const juce::String& tooltip)
        {
        ...
        fireBtn.onClick = [this]
        {
            if (onFire)
                onFire();
        };
        xoSendBtn.onClick = [this]
        {
            if (onXoSend)
                onXoSend();
        };
        echoCutBtn.onClick = [this]
        {
            if (onEchoCut)
                onEchoCut();
        };
    ...
    void setCpuVisible(bool visible) { cpuLabel.setVisible(visible); }
    ...
    void setSlotActive(int slot, bool active, juce::Colour accent)
    {
        if (slot < 0 || slot >= kNumSlots)
            return;
        if (slotActive[slot] == active && slotAccents[slot] == accent)
            return;
        ...
        for (int i = 0; i < kNumSlots; ++i)
        {
            ...
            if (slotActive[i])
            {
                ...
                for (int gx = 3; gx >= 1; --gx)
                {
                    ...
                    g.fillEllipse(cx - dotR - expand, dotY - dotR - expand, dotDia + expand * 2.0f,
                                  dotDia + expand * 2.0f);
                }
                // Core dot
                g.setColour(ac);
    ...
    struct ShortcutListener : public juce::KeyListener
    {
        ...
        bool keyPressed(const juce::KeyPress& key, juce::Component* /*originatingComponent*/) override
        {
            ...
            {
                if (bar.onFire && bar.fireBtn.isVisible() && bar.fireBtn.isEnabled())
                {
                    bar.onFire();
                    return true;
                }
            }
            if (key == juce::KeyPress('x'))
            {
                if (bar.onXoSend && bar.xoSendBtn.isVisible() && bar.xoSendBtn.isEnabled())
                {
                    bar.onXoSend();
                    return true;
                }
            ...
            {
                if (bar.onEchoCut && bar.echoCutBtn.isVisible() && bar.echoCutBtn.isEnabled())
                {
                    bar.onEchoCut();
                    return true;
                }
            ...
            {
                if (bar.onPanic && bar.panicBtn.isVisible() && bar.panicBtn.isEnabled())
                {
                    bar.onPanic();
                    return true;
                }
...
private:
    ...
    std::array<juce::Colour, kNumSlots> slotAccents = {juce::Colours::transparentBlack, juce::Colours::transparentBlack,
