...
public:
    ...
    void resized() override
    {
        ...
        auto controlStrip = bounds.removeFromBottom (controlH + gap * 2)
                                  .reduced (gap, gap);
    ...
    {
        // ── Background ───────────────────────────────────────────────────────
        const auto bg = GalleryColors::darkMode()
                      ? juce::Colour (GalleryColors::Dark::bg)
        ...
        const juce::Colour gridColor = GalleryColors::darkMode()
                                     ? juce::Colour (GalleryColors::Dark::t4)
                                     : juce::Colour (GalleryColors::Light::borderGray);

        g.setColour (gridColor.withAlpha (0.4f));
        // Horizontal centre line
        g.drawHorizontalLine (juce::roundToInt (midY),
                              canvasBounds.getX(),
        ...
        g.drawHorizontalLine (juce::roundToInt (q1Y),
                              canvasBounds.getX(), canvasBounds.getRight());
        g.drawHorizontalLine (juce::roundToInt (q3Y),
                              canvasBounds.getX(), canvasBounds.getRight());
        ...
        const juce::Colour labelColor = GalleryColors::darkMode()
                                      ? juce::Colour (GalleryColors::Dark::t2)
                                      : juce::Colour (GalleryColors::Light::textMid);

        g.setFont (GalleryFonts::value (11.0f));
        g.setColour (labelColor);
        g.drawText (frameLabel,
                    canvasBounds.reduced (6.0f, 4.0f).toNearestInt(),
...
private:
    ...
    void stepFrame (int delta)
    {
        ...
        const float newPos = (nf > 1)
                           ? static_cast<float> (next) / static_cast<float> (nf - 1)
                           : 0.0f;
    ...
    void showGenerateMenu()
    {
        ...
        menu.showMenuAsync (juce::PopupMenu::Options()
                                .withTargetComponent (generateBtn)
    ...
    void fillCurrentFrameWithShape (int shapeId)
    {
        ...
        {
            for (int s = 0; s < fs; ++s) frame[s] = 0.0f;
            for (int h = 1; h <= 32; ++h)
                for (int s = 0; s < fs; ++s)
                    frame[s] += (1.0f / static_cast<float> (h))
                                * std::sin (k2Pi * static_cast<float> (h)
                                            * static_cast<float> (s) / static_cast<float> (fs));
