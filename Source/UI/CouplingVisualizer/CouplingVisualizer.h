...
// Full rationale: Docs/coupling-ui-architecture-2026-03-21.md §5.
//==============================================================================
namespace CouplingTypeColors {
    ...
    inline juce::Colour forType (CouplingType type)
    {
        switch (type)
        {
            case CouplingType::AmpToFilter:      return juce::Colour (0xFF2196F3); // Cerulean
            case CouplingType::AmpToPitch:       return juce::Colour (0xFF1565C0); // Cobalt
            case CouplingType::LFOToPitch:       return juce::Colour (0xFF7986CB); // Periwinkle
            case CouplingType::EnvToMorph:       return juce::Colour (0xFFBA68C8); // Orchid
            case CouplingType::AudioToFM:        return juce::Colour (0xFFE53935); // Vermilion
            case CouplingType::AudioToRing:      return juce::Colour (0xFFF4511E); // Deep Orange
            case CouplingType::FilterToFilter:   return juce::Colour (0xFF4DB6AC); // Steel Teal
            case CouplingType::AmpToChoke:       return juce::Colour (0xFF546E7A); // Charcoal
            case CouplingType::RhythmToBlend:    return juce::Colour (0xFFFFB300); // Warm Amber
    ...
    inline juce::String displayName (CouplingType type)
    {
        switch (type)
        {
            case CouplingType::AmpToFilter:      return "Amplitude to Filter";
            case CouplingType::AmpToPitch:       return "Amplitude to Pitch";
            case CouplingType::LFOToPitch:       return "LFO to Pitch";
            case CouplingType::EnvToMorph:       return "Envelope to Morph";
            case CouplingType::AudioToFM:        return "Audio to FM";
            case CouplingType::AudioToRing:      return "Audio to Ring Mod";
            case CouplingType::FilterToFilter:   return "Filter to Filter";
            case CouplingType::AmpToChoke:       return "Amplitude to Choke";
            case CouplingType::RhythmToBlend:    return "Rhythm to Blend";
...
// CouplingVisualizer
//==============================================================================
class CouplingVisualizer : public juce::Component,
                           private juce::Timer
{
public:
    ...
    explicit CouplingVisualizer (MegaCouplingMatrix& matrix,
          ...
          getSlotColor   (std::move (slotColorFn))
    {
        ...
        setDescription (
            "Graph view of cross-engine modulation routes. "
    ...
    bool keyPressed (const juce::KeyPress& key) override
    {
        ...
        if ((key == juce::KeyPress::deleteKey || key == juce::KeyPress::backspaceKey)
            && selectedArc >= 0 && selectedArc < (int)cachedRoutes.size())
        {
...
private:
    ...
    void paintNodes (juce::Graphics& g)
    {
        for (int i = 0; i < MegaCouplingMatrix::MaxSlots; ++i)
        {
            ...
            bool isDragTarget = (dragState == DragState::DraggingFromSource
                                 && hoveredPort.valid && hoveredPort.slot == i
                                 && !hoveredPort.isOutput);
            ...
            if (hasEng)
            {
                ...
                float glowAlpha = reducedMotion ? 0.08f
                                                : 0.06f + avgRMS * 0.20f;
            ...
            // --- Node fill ---
            auto fillColour = hasEng ? accent.withAlpha (0.12f)
                                     : juce::Colour (GalleryColors::emptySlot());
            g.setColour (fillColour);
            g.fillEllipse (centre.x - kNodeRadius, centre.y - kNodeRadius,
                           kNodeRadius * 2, kNodeRadius * 2);

            // --- Node ring ---
            float ringWidth = hovered ? 2.5f : (isDragTarget ? 3.0f : 1.8f);
            auto ringColour = isDragTarget
                ? juce::Colour (GalleryColors::xoGold)
                : (hasEng ? accent : juce::Colour (GalleryColors::borderGray()));
            g.setColour (ringColour);
            g.drawEllipse (centre.x - kNodeRadius, centre.y - kNodeRadius,
            ...
            if (hasEng)
            {
                ...
                g.drawText (name, (int)(centre.x - kNodeRadius), (int)(centre.y - 7),
                            (int)(kNodeRadius * 2), 14, juce::Justification::centred);
            }
            else
            {
                g.setColour (juce::Colour (GalleryColors::textMid()));
                g.drawText ("SLOT " + juce::String (i + 1),
                            (int)(centre.x - kNodeRadius), (int)(centre.y - 5),
    ...
    void paintPorts (juce::Graphics& g, int slot,
                     juce::Point<float> centre, juce::Colour accent, bool hasEng)
    {
        if (!hasEng) return; // only show ports when an engine is loaded
        ...
        for (int p = 0; p < kNumPorts; ++p)
        {
            ...
            bool hov = (hoveredPort.valid && hoveredPort.slot == slot
                        && hoveredPort.portIndex == p);
            bool isDT = (dragState == DragState::DraggingFromSource
                         && hoveredPort.valid && hoveredPort.slot == slot
                         && hoveredPort.portIndex == p && !isOut);

            float r = hov ? kPortHoverRadius : kPortRadius;
            auto  c = isDT ? juce::Colour (GalleryColors::xoGold)
                           : (isOut ? accent : accent.withAlpha (0.6f));
    ...
    void paintCouplingArcs (juce::Graphics& g)
    {
        ...
        struct PairCount { int count = 0; int currentIdx = 0; };
        std::array<std::array<PairCount, MegaCouplingMatrix::MaxSlots>,
                   MegaCouplingMatrix::MaxSlots> pairCounts{};

        // First pass: count routes per pair
        for (const auto& route : cachedRoutes)
        {
            if (!route.active || route.amount < 0.001f) continue;
            if (!slotInRange (route.sourceSlot) || !slotInRange (route.destSlot)) continue;
        ...
        for (const auto& route : cachedRoutes)
        {
            bool shouldDraw = route.active && route.amount >= 0.001f
                              && slotInRange (route.sourceSlot) && slotInRange (route.destSlot);
    ...
    void paintSingleArc (juce::Graphics& g,
                         const MegaCouplingMatrix::CouplingRoute& route,
                         float stagger,
                         float rms,
                         bool selected)
    {
        ...
        float toDestAngle = std::atan2 (dstCentre.y - srcCentre.y,
                                        dstCentre.x - srcCentre.x);
        float fromSrcAngle = toDestAngle + juce::MathConstants<float>::pi;

        juce::Point<float> srcPt {
            srcCentre.x + std::cos (toDestAngle) * kNodeRadius,
            srcCentre.y + std::sin (toDestAngle) * kNodeRadius
        };
        juce::Point<float> dstPt {
            dstCentre.x + std::cos (fromSrcAngle) * kNodeRadius,
        ...
        float pulse = reducedMotion
            ? 0.6f + route.amount * 0.4f
            : 0.6f + rms * 0.4f + std::sin (animPhase) * 0.05f;
        ...
        g.strokePath (arc, juce::PathStrokeType (
            strokeW * 3.0f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
        ...
        if (route.isNormalled)
        {
            ...
            juce::PathStrokeType stroke (strokeW * 0.7f,
        ...
        else
        {
            ...
            g.strokePath (arc, juce::PathStrokeType (
                strokeW, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
        }

        // KnotTopology reverse arc (phase-offset pulse)
        if (isKnot && !route.isNormalled)
        {
            float revPulse = 0.6f + rms * 0.4f
                + std::sin (animPhase + juce::MathConstants<float>::pi) * 0.05f;
            revPulse = juce::jlimit (0.0f, 1.0f, revPulse);
            g.setColour (typeColour.withAlpha (baseAlpha * revPulse * 0.6f));
            g.strokePath (reverseArc, juce::PathStrokeType (
                strokeW * 0.6f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
        }
    ...
    void paintArrowhead (juce::Graphics& g,
                         juce::Point<float> controlPt,
                         ...
                         float amount)
    {
        ...
        juce::Point<float> left {
            tip.x - sz * std::cos (angle - spread),
            tip.y - sz * std::sin (angle - spread)
        };
        juce::Point<float> right {
            tip.x - sz * std::cos (angle + spread),
            tip.y - sz * std::sin (angle + spread)
        };
    ...
    void paintDragArc (juce::Graphics& g)
    {
        ...
        float toMouseAngle = std::atan2 (dragCurrentPos.y - srcCentre.y,
                                         dragCurrentPos.x - srcCentre.x);

        juce::Point<float> srcPt {
            srcCentre.x + std::cos (toMouseAngle) * kNodeRadius,
        ...
        if (reducedMotion)
        {
            ...
            g.drawLine (srcPt.x, srcPt.y,
                        dragCurrentPos.x, dragCurrentPos.y, 2.0f);
        }
        else
        {
            juce::Path arc;
    ...
    void paintLegend (juce::Graphics& g, juce::Rectangle<float> bounds)
    {
        ...
        for (const auto& [type, count] : typeCounts)
        {
            ...
            x += 72.0f;
            if (x > stripBounds.getRight() - 70.0f) break; // wrap guard
        }
    }
    ...
    PortHit hitTestPort (juce::Point<float> pos) const
    {
        ...
        {
            if (getSlotName (slot).isEmpty()) continue;
    ...
    bool isNearArc (juce::Point<float> pos,
                    const MegaCouplingMatrix::CouplingRoute& route) const
    {
        ...
        juce::Point<float> p0 {
            srcC.x + std::cos (toDA)  * kNodeRadius,
            srcC.y + std::sin (toDA)  * kNodeRadius
        };
        juce::Point<float> p3 {
            dstC.x + std::cos (fromA) * kNodeRadius,
            dstC.y + std::sin (fromA) * kNodeRadius
        };

        juce::Point<float> chord = p3 - p0;
        float chordLen = std::sqrt (chord.x * chord.x + chord.y * chord.y);
        if (chordLen < 1.0f) return false;

        juce::Point<float> perp { -chord.y / chordLen, chord.x / chordLen };
        float curveOffset = chordLen * kArcCurvature;

        juce::Point<float> p1 {
            p0.x + chord.x * 0.33f + perp.x * curveOffset,
            p0.y + chord.y * 0.33f + perp.y * curveOffset
        };
        juce::Point<float> p2 {
            p0.x + chord.x * 0.67f + perp.x * curveOffset,
    ...
    static juce::Point<float> portPosition (juce::Point<float> nodeCenter, float angle)
    {
        ...
        return {
            nodeCenter.x + std::cos (angle) * portOrbit,
            nodeCenter.y + std::sin (angle) * portOrbit
        };
    }
    ...
    {
        return slot >= 0 && slot < MegaCouplingMatrix::MaxSlots;
    }
    ...
    {
        if (arcIdx < 0 || arcIdx >= (int)cachedRoutes.size()) return;
        const auto& route = cachedRoutes[static_cast<size_t> (arcIdx)];

        juce::PopupMenu menu;
        menu.addSectionHeader (CouplingTypeColors::displayName (route.type)
                               + ": Slot " + juce::String (route.sourceSlot + 1)
                               + " -> Slot " + juce::String (route.destSlot + 1)
    ...
    // Drag state machine
    enum class DragState { Idle, DraggingFromSource };
