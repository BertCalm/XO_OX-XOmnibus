...
public:
    ...
    {
        auto result = rive::File::import({static_cast<const uint8_t*>(data), sizeBytes}, &factory);
...
private:
    ...
    void paintProcedural(juce::Graphics& g)
    {
        ...
        const auto bodyColor = gillPink.withMultipliedSaturation(bodySat).withAlpha(bodyAlpha);
        ...
        static constexpr GillDef kGills[6] = {
            {0.0f, 1.00f, -1.0f, 0}, {0.4f, 0.87f, -1.0f, 1}, {1.1f, 1.13f, -1.0f, 2},
            {0.2f, 0.95f, +1.0f, 0}, {0.8f, 1.07f, +1.0f, 1}, {1.5f, 0.91f, +1.0f, 2},
        };
        ...
        for (auto& gd : kGills)
        {
            ...
            const float swing =
                std::sin(time * gillHz * juce::MathConstants<float>::twoPi * gd.rate + gd.phase) * swingRad;
            ...
            g.setGradientFill(juce::ColourGradient(bodyColor, baseX, baseY, gillTipHi.withAlpha(bodyAlpha * 0.8f), tipX,
                                                   tipY, false));
            g.strokePath(stalk,
                         juce::PathStrokeType(gillThick, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
            ...
            if (R > 14.0f)
            {
                ...
                branches.lineTo(tipX + std::cos(angle + 0.45f) * bLen, tipY + std::sin(angle + 0.45f) * bLen);
                branches.startNewSubPath(tipX, tipY);
                branches.lineTo(tipX + std::cos(angle - 0.45f) * bLen, tipY + std::sin(angle - 0.45f) * bLen);

                g.setColour(gillTipHi.withAlpha(bodyAlpha * 0.65f));
                g.strokePath(branches, juce::PathStrokeType(gillThick * 0.45f, juce::PathStrokeType::curved,
        ...
        g.fillEllipse(cx - bellyw * 0.5f, cy - bellyh * 0.5f + R * 0.18f, bellyw, bellyh);
    ...
    struct JuceRiveFactory : public rive::Factory
    {
        ...
        rive::rcp<rive::RenderShader> makeLinearGradient(float, float, float, float, const rive::ColorInt*,
                                                         const float*, int) override
        {
            return nullptr;
        }

        rive::rcp<rive::RenderShader> makeRadialGradient(float, float, float, const rive::ColorInt*, const float*,
                                                         int) override
        {
            return nullptr;
        }

        rive::rcp<rive::RenderPath> makeRenderPath(rive::RawPath& rawPath, rive::FillRule fill) override
        {
        ...
        rive::rcp<rive::RenderPath> makeEmptyRenderPath() override { return rive::make_rcp<OscarJucePath>(); }

        rive::rcp<rive::RenderPaint> makeRenderPaint() override { return rive::make_rcp<OscarJucePaint>(); }

        rive::rcp<rive::RenderImage> decodeImage(rive::Span<const uint8_t>) override { return nullptr; }
    };
