...
class OutshinePreviewPlayer : public juce::Component, private juce::Timer
{
public:
    OutshinePreviewPlayer()
    {
        ...
        playC4Btn.setColour(juce::TextButton::buttonColourId, GalleryColors::get(GalleryColors::slotBg()));
        playC4Btn.setColour(juce::TextButton::textColourOffId, GalleryColors::get(GalleryColors::textDark()));
    ...
    void setSamples(const std::vector<AnalyzedSample>& samples) { currentSamples = samples; }
    ...
    void playC4()
    {
        ...
        std::unique_ptr<juce::AudioFormatReader> reader(formatManager.createReaderFor(sample->sourceFile));
        if (!reader)
            return;
    ...
    void resized() override { playC4Btn.setBounds(getLocalBounds()); }
...
private:
    ...
    const AnalyzedSample* findSampleForMidi(int midiNote) const
    {
        ...
        for (int i = 0; i < (int)sorted.size(); ++i)
        {
            ...
            int high =
                (i == (int)sorted.size() - 1) ? 127 : (sorted[(size_t)i].midi + sorted[(size_t)(i + 1)].midi) / 2 - 1;
