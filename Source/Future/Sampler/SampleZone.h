...
struct SampleZone
{
    ...
    int getZonesForNote(int note, int velocity, QueryResult& out) const noexcept
    {
        ...
        for (int i = 0; i < numZones_ && out.count < kMaxQueryHits; ++i)
        {
            ...
            if (note < z.lowNote || note > z.highNote)
                continue;
            if (velocity < z.lowVel || velocity > z.highVel)
                continue;

            // Round-robin check
            if (z.groupId >= 0 && z.groupId < kMaxGroups)
            {
                if (z.rrPosition != rrCounters_[z.groupId])
                    continue;
            }
    ...
    {
        if (groupId < 0 || groupId >= kMaxGroups)
            return;
    ...
    void buildChromaticMapping(int lowestNote, int highestNote, int numSamples, float uniformGain = 1.0f) noexcept
    {
        clearZones();
    ...
    void buildVelocityLayers(int lowNote, int highNote, int numLayers, int sampleBaseIndex, int groupId = 0,
                             float uniformGain = 1.0f) noexcept
    {
        if (numLayers <= 0)
            return;
    ...
    bool hasZoneForNote(int note, int velocity) const noexcept
    {
        ...
        {
            const Zone& z = zones_[i];
            if (note >= z.lowNote && note <= z.highNote && velocity >= z.lowVel && velocity <= z.highVel)
                return true;
        }
        return false;
    }
