...
public:
    ...
    Lane& getLane(int index) { return lanes[static_cast<size_t>(std::clamp(index, 0, kMaxLanes - 1))]; }

    const Lane& getLane(int index) const { return lanes[static_cast<size_t>(std::clamp(index, 0, kMaxLanes - 1))]; }
    ...
    void setStepValue(int laneIdx, int step, float value)
    {
        if (laneIdx >= 0 && laneIdx < kMaxLanes && step >= 0 && step < kMaxSteps)
            lanes[static_cast<size_t>(laneIdx)].values[static_cast<size_t>(step)] = std::clamp(value, 0.0f, 1.0f);
    }
    ...
    std::array<ModOutput, kMaxLanes> getModValues(int currentStep) const
    {
        if (sr <= 0.0)
        {
            ...
            for (auto& o : empty)
            {
                o.target = MasterFXSequencer::Target::None;
                o.value = 0.0f;
            }
            return empty;
        }
        ...
        for (int l = 0; l < kMaxLanes; ++l)
        {
            ...
            if (lane.smoothing > 0.01f)
            {
                ...
                float nextVal =
                    lane.muted[static_cast<size_t>(nextStep)] ? 0.0f : lane.values[static_cast<size_t>(nextStep)];
    ...
    void loadSnapshot(const Snapshot& snap) { lanes = snap.lanes; }
        ...
        if (auto* arr = json.getArray())
        {
            for (int l = 0; l < std::min(kMaxLanes, arr->size()); ++l)
            {
                if (auto* lObj = (*arr)[l].getDynamicObject())
                {
                    ...
                    lane.target = static_cast<MasterFXSequencer::Target>(static_cast<int>(lObj->getProperty("target")));
