...
public:
    ...
    GeneratedPatch generate(int activeSlots, Mode mode)
    {
        ...
        if (activeSlots < 2)
            return patch; // Need at least 2 engines to couple
        ...
        static constexpr CouplingType safeTypes[] = {CouplingType::AmpToFilter,  CouplingType::LFOToPitch,
                                                     CouplingType::EnvToMorph,   CouplingType::EnvToDecay,
                                                     CouplingType::PitchToPitch, CouplingType::RhythmToBlend};
        static constexpr CouplingType aggressiveTypes[] = {
            CouplingType::AudioToFM,  CouplingType::AudioToRing, CouplingType::FilterToFilter,
            CouplingType::AmpToPitch, CouplingType::AmpToChoke,  CouplingType::AudioToWavetable};
        ...
        for (int r = 0; r < numRoutes; ++r)
        {
            ...
            float aggressiveChance = (mode == Mode::Subtle) ? 0.1f : (mode == Mode::Moderate) ? 0.3f : 0.5f;
            ...
            if (route.type == CouplingType::AudioToRing || route.type == CouplingType::AmpToChoke)
            {
            ...
            desc += juce::String::formatted(" %d→%d @%.0f%%", route.sourceSlot + 1, route.destSlot + 1,
                                            route.intensity * 100.0f);
        }
    ...
    GeneratedPatch generateGenre(int activeSlots, Genre genre)
    {
        ...
        {
        case Genre::Dub:
            weights = {{CouplingType::AmpToFilter, 0.4f, 0.2f, 0.5f},
            ...
            break;
        case Genre::IDM:
            weights = {{CouplingType::AudioToFM, 0.3f, 0.2f, 0.6f},
            ...
            break;
        case Genre::Ambient:
            weights = {{CouplingType::EnvToMorph, 0.35f, 0.1f, 0.25f},
        ...
        case Genre::Industrial:
            weights = {{CouplingType::AudioToRing, 0.3f, 0.3f, 0.7f},
