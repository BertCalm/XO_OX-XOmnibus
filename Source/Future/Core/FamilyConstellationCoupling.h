...
public:
    ...
    //--------------------------------------------------------------------------
    static void installFamilyRoutes(MegaCouplingMatrix& matrix, const std::unordered_map<std::string, int>& engineSlots)
    {
    ...
    {
        return engineId == kOhm || engineId == kOrphica || engineId == kObbligato || engineId == kOttoni ||
               engineId == kOle;
    }
    ...
    //--------------------------------------------------------------------------
    static int countConstellationEngines(const std::array<SynthEngine*, MegaCouplingMatrix::MaxSlots>& engines)
    {
        int count = 0;
        for (auto* e : engines)
