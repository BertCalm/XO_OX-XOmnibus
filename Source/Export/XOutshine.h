...
//------------------------------------------------------------------------------
// Rebirth Mode — engine-inspired FX chain for velocity-layer generation.
// Included after SampleCategory is defined (RebirthProfiles.h uses it).
// Included before OutshineSettings (RebirthSettings is embedded there).
//------------------------------------------------------------------------------
...
struct OutshineSettings
{
    ...
    // instead of the amplitude taper + one-pole LP filter path.
                                          ...
                                          const juce::File& workDir)
    {
        ...
        for (const auto& s : samples)
        {
            ...
            for (int vel = 0; vel < settings_.velocityLayers; ++vel)
            {
                ...
                {
                    // ----------------------------------------------------------------
                    // Rebirth path — velocity layer generated by engine-inspired FX
                ...
                {
                    // ----------------------------------------------------------------
                    // Classic path — amplitude taper + one-pole LP filter.
                    // ----------------------------------------------------------------
                    for (int ch = 0; ch < nch; ++ch)
                    {
                ...
                // Round-robin variations — always applied on top of shaped
                // (micro-pitch + gain variation + saturation apply regardless of
                // whether the shaped buffer was generated by Rebirth or the classic path)
