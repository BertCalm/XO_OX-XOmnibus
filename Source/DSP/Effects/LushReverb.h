...
public:
    ...
    void prepare (double sampleRate)
    {
        ...
        static constexpr float kLfoRatesHz[kN] = { 0.31f, 0.37f, 0.43f, 0.51f,
                                                     0.59f, 0.67f, 0.79f, 1.12f };
        static constexpr float kLfoDepthsSamples[kN] = { 6.0f, 7.0f, 8.0f, 9.0f,
                                                           10.0f, 11.0f, 12.0f, 13.0f };
        static constexpr float kInitialPhases[kN] = { 0.0f,  0.785f, 1.571f, 2.356f,
                                                        3.141f, 3.927f, 4.712f, 5.497f };
                             ...
                             int    numSamples)
    {
        ...
        {
            if (leftIn  != leftOut)  std::copy (leftIn,  leftIn  + numSamples, leftOut);
            if (rightIn != rightOut) std::copy (rightIn, rightIn + numSamples, rightOut);
            return;
        }
        ...
        for (int n = 0; n < numSamples; ++n)
        {
            ...
            if (std::abs (curDamping - lastDamping) > 1e-5f ||
                std::abs (curDecay   - lastDecay)   > 1e-5f)
            {
            ...
            for (int i = 0; i < kN; ++i)
            {
                ...
                const float lfoVal = lfoTable[phi] + phfrac * (lfoTable[(phi + 1) & (kLfoTableSize - 1)] - lfoTable[phi]);
...
private:
    ...
    static void hadamard8 (float v[kN])
    {
        ...
        {
            const float a0 = v[i];     const float b0 = v[i + 2];
            const float a1 = v[i + 1]; const float b1 = v[i + 3];
            v[i]     = a0 + b0;  v[i + 2] = a0 - b0;
            v[i + 1] = a1 + b1;  v[i + 3] = a1 - b1;
        }
        // Stage 3: 4 butterflies over pairs (0,4)(1,5)(2,6)(3,7)
        {
            const float a0 = v[0]; const float b0 = v[4];
            const float a1 = v[1]; const float b1 = v[5];
            const float a2 = v[2]; const float b2 = v[6];
            const float a3 = v[3]; const float b3 = v[7];
            v[0] = a0 + b0;  v[4] = a0 - b0;
            v[1] = a1 + b1;  v[5] = a1 - b1;
            v[2] = a2 + b2;  v[6] = a2 - b2;
            v[3] = a3 + b3;  v[7] = a3 - b3;
    ...
    //==========================================================================
    static float readLinearInterp (const std::vector<float>& buf, int len,
                                   int writePos, int baseDelay, float fractionalOffset)
    {
        ...
        return buf[static_cast<size_t> (posA)] * (1.0f - frac) +
               buf[static_cast<size_t> (posB)] * frac;
    }
    ...
    //==========================================================================
    float applyShelvingFilter (float x, float& stateLo, float& stateHi,
                               int lineIdx, bool isRight)
    {
        (void) lineIdx; (void) isRight;
    ...
    void updateERTaps (float sizeScale)
    {
        ...
        static constexpr float kErBaseMs[kNumER] = {
            2.0f, 4.8f, 9.1f, 14.7f, 21.3f, 29.5f, 39.4f, 51.0f
        };
        static constexpr float kErGainsConst[kNumER] = {
            0.8f, 0.65f, 0.5f, 0.4f, 0.3f, 0.22f, 0.15f, 0.10f
        };
    ...
    //--------------------------------------------------------------------------
    float processAllpass (float x, std::vector<float> buf[kNumAP],
                          int pos[kNumAP], float feedback)
    {
        float out = x;
        for (int a = 0; a < kNumAP; ++a)
        {
