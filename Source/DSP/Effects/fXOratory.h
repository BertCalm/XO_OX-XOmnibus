    ...
    void processBlock (float* L, float* R, int numSamples)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            for (int t = 0; t < numActive; ++t)
            {
                ...
                float driftOffset = driftVal * drift * 0.5f
                                  * fastSin (static_cast<float> (t) * 2.39996f);  // golden angle stagger
...
private:
    ...
    void getSpatialPan (int tapIndex, int numTaps, float& panL, float& panR) const
    {
        ...
        switch (pattern)
        {
            ...
            {
                int third = (tapIndex < numTaps / 3) ? 0
                          : (tapIndex < 2 * numTaps / 3) ? 1 : 2;
    ...
    void updatePattern()
    {
        ...
        for (int t = 0; t < kMaxTaps; ++t)
        {
            float ratio = (numActive > 1)
                ? static_cast<float> (t) / static_cast<float> (numActive - 1)
                : 0.0f;
