...
public:
    ...
    void setCollapseDecay (float ms)
    {
        collapseDecayMs = clamp (ms, 10.0f, 2000.0f);
    }
    ...
    void processBlock (float* L, float* R, int numSamples)
    {
        ...
        const float decayCoeff = (decaySamples > 1.0f)
            ? fastExp (-6.9078f / decaySamples)  // -60dB over decay time
            : 0.0f;
        ...
        static constexpr float apfBaseFreqs[kNumAPFs] = {
            200.0f, 800.0f, 2400.0f, 6000.0f
        };
