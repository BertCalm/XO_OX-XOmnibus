...
public:
    ...
    void reset()
    {
        ...
        ksWritePos = 0; ksFiltL = ksFiltR = 0.0f; envFollower = 0.0f;
        for (int i = 0; i < 4; ++i)
        {
            std::fill (atmBuf[i].begin(), atmBuf[i].end(), 0.0f);
            atmPos[i] = 0; atmFilt[i] = 0.0f;
        }
        crossLP_L = crossLP_R = 0.0f; tremoloPhase = 0.0f;
        std::fill (slipBufL.begin(), slipBufL.end(), 0.0f);
        std::fill (slipBufR.begin(), slipBufR.end(), 0.0f);
        slipWritePos = 0; slipReadPhase = 0.0f; slipActive = false; slipFadeGain = 0.0f;
    }

    void processBlock (float* L, float* R, int numSamples,
                       float textureBlend, float reverbSize, float tremoloRate,
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            if (textureBlend > 0.01f)
            {
                ...
                for (int i = 0; i < 4; ++i)
                {
                    ...
                    int len = static_cast<int> (atmBuf[i].size());
                    if (len > 0) atmBuf[i][static_cast<size_t> (atmPos[i])] = flushDenormal (fb);
            ...
            if (timeSlip)
            {
                ...
                if (slipReadPhase < 0.0f) slipReadPhase += static_cast<float> (slipBufSize);
                ...
                slipOut = (slipBufL[static_cast<size_t> (idx)] * (1.0f - frac)
                         + slipBufL[static_cast<size_t> (next)] * frac) * slipFadeGain;
            }
            else
            {
                if (slipActive)
                {
                    slipFadeGain -= 0.002f;
                    if (slipFadeGain <= 0.0f) { slipActive = false; slipFadeGain = 0.0f; }
                    else
                    {
                        int idx = static_cast<int> (slipReadPhase);
