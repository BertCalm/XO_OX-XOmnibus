...
public:
    ...
    void processBlock (float* L, float* R, int numSamples)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            float darkMix = (safeBase > 1e-6f)
                ? fastExp (erosionCurve * patinaModulation * fastLog (safeBase))
                : 0.0f;
            ...
            if (darkMix > 0.001f)
            {
                ...
                if (effectiveSR < srF / 16.0f) effectiveSR = srF / 16.0f;
    ...
    void reset()
    {
        darkBPL.reset(); darkBPR.reset();
        darkHPL.reset(); darkHPR.reset();
        patinaLFO.reset();
        envState = 0.0f;
        holdL = holdR = 0.0f;
        holdCounter = 0.0f;
