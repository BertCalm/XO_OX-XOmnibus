...
public:
    ...
    static constexpr float kHarmonics[] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 8.0f, 12.0f};
...
public:
    ...
    float tick(float rateHz, float depthCents)
    {
        ...
        float lfo = 0.6f * fastSin(phase1 * 6.2831853f) + 0.4f * fastSin(phase2 * 6.2831853f);
...
public:
    ...
    void trigger(float burstMs = 2.0f) { remaining = static_cast<int>(sr * burstMs * 0.001f); }
...
public:
    ...
    void trigger(float attackMs = 1.5f) { remaining = static_cast<int>(sr * attackMs * 0.001f); }
...
public:
    ...
    float tick(float breathPressure, float frequency)
    {
        ...
        float breathNoise = (breathPressure > 0.01f) ? noise * std::max(0.0f, 0.06f - breathPressure * 0.06f) : 0.0f;
