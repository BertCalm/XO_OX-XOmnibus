...
public:
    ...
    WavetableOscillator() { std::memset(table, 0, sizeof(table)); }
    ...
    void loadWavetable(const float* data, int numFrames, int frameSize = kFrameSize) noexcept
    {
        ...
        {
            int p = 1;
            while (p < frameSize && p <= kMaxFrameSize)
                p <<= 1;
        ...
        if (totalSamples < kMaxTableSize)
            std::memset(table + totalSamples, 0, static_cast<size_t>(kMaxTableSize - totalSamples) * sizeof(float));
    }
    ...
    void generateBasicTables(double sampleRate) noexcept
    {
        ...
        for (int s = 0; s < fs; ++s)
        {
            ...
            for (int h = 1; h <= maxHarmonic; ++h)
                val += (1.0f / static_cast<float>(h)) * std::sin(k2Pi * static_cast<float>(h) * phase);
            buf[0 * fs + s] = val;
        }
        ...
        for (int s = 0; s < fs; ++s)
        {
            ...
            for (int h = 1; h <= maxHarmonic; h += 2)
            {
                val += sign * (1.0f / static_cast<float>(h * h)) * std::sin(k2Pi * static_cast<float>(h) * phase);
                sign = -sign;
            }
            buf[2 * fs + s] = val;
        }
    ...
    void setPosition(float pos) noexcept { position = (pos < 0.0f) ? 0.0f : (pos > 1.0f) ? 1.0f : pos; }
...
private:
    ...
    float readFrameCubic(int frameIndex, float samplePos) const noexcept
    {
        ...
        if (base + std::max({im1, i00, i1, i2}) >= kMaxTableSize)
            return 0.0f;
