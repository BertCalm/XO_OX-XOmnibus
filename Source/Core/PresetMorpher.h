...
struct Xorshift64 {
    ...
    explicit Xorshift64 (uint64_t seed = 0xDEADBEEFCAFEBABEull)
        : state (seed == 0 ? 0xDEADBEEFCAFEBABEull : seed)
    {}
...
inline bool isIntegerParam (const juce::String& paramId, const juce::var& value)
{
    ...
    static const juce::StringArray integerKeywords {
        ...
        "Pattern", "Rule",    "Legato",  "Glide",   "Bus",
        "Chord",   "Algo",    "Version", "Preset",  "Slot",
        "Index",   "Step",    "Count",   "Number",  "Set",
    };
...
inline juce::var lerpParam (const juce::String& paramId,
                            const juce::var& a,
                            const juce::var& b,
                            float t)
{
...
inline juce::var morphEngineParams (const juce::String& engineNameA,
                                    const juce::var& paramsA,
                                    const juce::String& engineNameB,
                                    const juce::var& paramsB,
                                    float t)
{
...
inline PresetData morph (const PresetData& a, const PresetData& b, float t)
{
    ...
    for (int slot = 0; slot < numSlots; ++slot)
    {
        ...
        // ---- Parameter blending ----
        juce::var paramsA = hasA ? a.parametersByEngine.count (engA) ? a.parametersByEngine.at (engA) : juce::var() : juce::var();
        juce::var paramsB = hasB ? b.parametersByEngine.count (engB) ? b.parametersByEngine.at (engB) : juce::var() : juce::var();
...
inline float computeParamSetFitness (const juce::var& params, const PresetDNA& target)
{
    ...
    static const DNADimension dims[] = {
        DNADimension::Brightness, DNADimension::Warmth,
        DNADimension::Movement,   DNADimension::Density,
        DNADimension::Space,      DNADimension::Aggression
    };
    ...
    for (auto dim : dims)
    {
        ...
        float target_v  = 0.5f;
        switch (dim) {
            case DNADimension::Brightness: target_v = target.brightness; break;
            case DNADimension::Warmth:     target_v = target.warmth;     break;
            case DNADimension::Movement:   target_v = target.movement;   break;
            case DNADimension::Density:    target_v = target.density;    break;
            case DNADimension::Space:      target_v = target.space;      break;
            case DNADimension::Aggression: target_v = target.aggression; break;
        }
...
inline int selectParentForParam (const juce::String& paramId,
                                  const juce::var& valA,
                                  const juce::var& valB,
                                  const PresetDNA& dnaA,
                                  const PresetDNA& dnaB,
                                  const PresetDNA& target,
                                  Xorshift64& rng)
{
    // Gather how strongly this param maps to each DNA dimension.
    static const std::pair<DNADimension, float PresetDNA::*> dimFields[] = {
        { DNADimension::Brightness, &PresetDNA::brightness },
        { DNADimension::Warmth,     &PresetDNA::warmth     },
        { DNADimension::Movement,   &PresetDNA::movement   },
        { DNADimension::Density,    &PresetDNA::density    },
        { DNADimension::Space,      &PresetDNA::space      },
        { DNADimension::Aggression, &PresetDNA::aggression },
    };
...
inline juce::var breedEngineParams (const juce::String& engineName,
                                     const juce::var& paramsA,
                                     const juce::var& paramsB,
                                     const PresetDNA& dnaA,
                                     const PresetDNA& dnaB,
...
inline PresetData breed (const PresetData& parentA,
                          const PresetData& parentB,
                          const PresetDNA& targetDNA,
                          Xorshift64& rng)
{
    ...
    offspring.couplingIntensity = (rng.nextFloat() < 0.5f)
                                  ? parentA.couplingIntensity
                                  : parentB.couplingIntensity;
    ...
    offspring.couplingPairs = (fitnessA >= fitnessB)
                              ? parentA.couplingPairs
                              : parentB.couplingPairs;
    ...
    for (int slot = 0; slot < numSlots; ++slot)
    {
        ...
        juce::var pA = parentA.parametersByEngine.count (engA)
                       ? parentA.parametersByEngine.at (engA) : juce::var();
        juce::var pB = parentB.parametersByEngine.count (engB)
                       ? parentB.parametersByEngine.at (engB) : juce::var();
        ...
        else
        {
            ...
            if (altParams.isObject())
            {
                ...
                if (altObj != nullptr)
                {
                    for (const auto& prop : altObj->getProperties())
                    {
                        ...
                        juce::String baseName = origId.startsWith (altPrefix)
                                                ? origId.substring (altPrefix.length())
            ...
            offspring.parametersByEngine[outEngine] =
                breedEngineParams (outEngine, domParams, juce::var (remapped),
                                   domDNA, altDNA, targetDNA, rng);
        }
    }
...
{
    if (val < 0.0f)   val = -val;
    if (val <= 2.0f)   return 1.0f;
    if (val <= 20.0f)  return val;
    if (val <= 200.0f) return val * 0.5f;
    return val * 0.25f;
}
...
inline PresetData randomize (const PresetData& source,
                              float maxDNAJump,
                              Xorshift64& rng)
{
    ...
    for (auto& [engineName, params] : result.parametersByEngine)
    {
        ...
        for (const auto& prop : obj->getProperties())
        {
            ...
            float budget = (totalW > 0.0f)
                           ? maxDNAJump * std::min (totalW, 2.0f) / 2.0f
                           : maxDNAJump * 0.1f;
...
{
    return std::abs (a.brightness - b.brightness)
         + std::abs (a.warmth    - b.warmth)
         + std::abs (a.movement  - b.movement)
         + std::abs (a.density   - b.density)
         + std::abs (a.space     - b.space)
         + std::abs (a.aggression - b.aggression);
}
...
inline std::vector<std::pair<float, const PresetData*>>
findNearest (const std::vector<PresetData>& library,
             const PresetDNA& targetDNA,
             int count)
{
    struct Ranked {
    ...
    for (int i = 0; i < n; ++i)
        results.emplace_back (ranked[static_cast<size_t>(i)].dist,
                              &library[ranked[static_cast<size_t>(i)].idx]);
...
inline std::vector<std::pair<float, const PresetData*>>
findNearestForEngine (const std::vector<PresetData>& library,
                      const PresetDNA& targetDNA,
                      ...
                      int count)
{
    ...
    for (int i = 0; i < n; ++i)
        results.emplace_back (ranked[static_cast<size_t>(i)].dist,
                              ranked[static_cast<size_t>(i)].ptr);
    return results;
}
...
public:
    ...
    explicit PresetMorpher (uint64_t seed)
        : rng (seed)
    {}
    ...
    PresetData morph (const PresetData& a, const PresetData& b, float t) const
    {
        return morpher::morph (a, b, t);
    }

    /// Produce a sequence of `steps` evenly spaced morphs between a and b.
    /// Includes both endpoints.  steps must be ≥ 2.
    std::vector<PresetData> morphSequence (const PresetData& a,
                                           const PresetData& b,
                                           int steps) const
    {
    ...
    PresetData breed (const PresetData& parentA,
                      const PresetData& parentB,
                      const PresetDNA& targetDNA)
    {
    ...
    std::vector<PresetData> breedPopulation (const PresetData& parentA,
                                              const PresetData& parentB,
                                              const PresetDNA& targetDNA,
                                              int count)
    {
    ...
    std::vector<PresetData> randomizeMultiple (const PresetData& source,
                                               float maxDNAJump,
                                               int count)
    {
        count = std::max (count, 1);
    ...
    static float dnaDistance (const PresetDNA& a, const PresetDNA& b)
    {
        return morpher::dnaDistance (a, b);
    }
    ...
    static std::vector<std::pair<float, const PresetData*>>
    findNearest (const std::vector<PresetData>& library,
                 const PresetDNA& targetDNA,
                 int count = 5)
    {
