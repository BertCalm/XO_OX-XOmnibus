...
public:
    ...
    //--------------------------------------------------------------------------
    void processBlock(juce::AudioBuffer<float>& buffer, int numSamples, double ppqPosition = -1.0, double bpm = 0.0)
    {
        ...
        auto applySeqMod = [&](float baseValue, MasterFXSequencer::Target paramTarget, float maxOffset) -> float
        {
            float offset = 0.0f;
            if (sequencer.getTarget1() == paramTarget)
                offset += seqMod1;
            if (sequencer.getTarget2() == paramTarget)
                offset += seqMod2;
            return clamp(baseValue + offset * maxOffset, 0.0f, 1.0f);
        };

        // Bipolar variant (for spectral tilt: -1..+1)
        auto applySeqModBipolar = [&](float baseValue, MasterFXSequencer::Target paramTarget, float maxOffset) -> float
        {
            float offset = 0.0f;
            if (sequencer.getTarget1() == paramTarget)
                offset += seqMod1;
        ...
        // ====================================================================
        float effectiveSatDrive = applySeqMod(satDrive, MasterFXSequencer::Target::SatDrive, 0.5f);

        if (effectiveSatDrive > 0.001f)
        {
            auto mode = static_cast<Saturator::SaturationMode>(clamp(static_cast<int>(satMode), 0, 3));
        ...
        // Stage 4: Spectral Tilt
        // ====================================================================
        float effectiveTilt = applySeqModBipolar(tiltAmount, MasterFXSequencer::Target::SpectralTilt, 0.8f);
        ...
        // Stage 5: Transient Designer
        // ====================================================================
        float effectiveTDAttack = applySeqModBipolar(tdAttack, MasterFXSequencer::Target::TransientAttack, 0.6f);
        ...
        // Stage 6: fXOsmosis (Membrane Transfer)
        // ====================================================================
        float effectiveOsmMix = applySeqMod(osmMix, MasterFXSequencer::Target::OsmosisMix, 0.6f);
        ...
        // ====================================================================
        float effectiveDelayMix = applySeqMod(delayMix, MasterFXSequencer::Target::DelayMix, 0.8f);
        float effectiveDelayFB = applySeqMod(delayFB, MasterFXSequencer::Target::DelayFeedback, 0.4f);
        ...
        // ====================================================================
        float effectiveDopplerDist = applySeqMod(dopplerDist, MasterFXSequencer::Target::DopplerDistance, 0.7f);
        ...
        // ====================================================================
        float effectiveReverbMix = applySeqMod(reverbMix, MasterFXSequencer::Target::ReverbMix, 0.6f);
        ...
        if (fshiftMix > 0.001f)
        {
            ...
            freqShifter.setMode(static_cast<FrequencyShifter::Mode>(clamp(static_cast<int>(fshiftMode), 0, 2)));
        ...
        // Stage 13: fXOneiric (Dream State)
        // ====================================================================
        float effectiveOnMix = applySeqMod(onMix, MasterFXSequencer::Target::OneiricMix, 0.6f);
        ...
        // ====================================================================
        float effectiveModDepth = applySeqMod(modDepth, MasterFXSequencer::Target::ModDepth, 0.7f);
        ...
        // Stage 15: Granular Smear
        // ====================================================================
        float effectiveSmear = applySeqMod(smearAmt, MasterFXSequencer::Target::GranularSmear, 0.6f);
        ...
        // Stage 16: Harmonic Exciter
        // ====================================================================
        float effectiveExcDrive = applySeqMod(excDrive, MasterFXSequencer::Target::ExciterDrive, 0.5f);
        ...
        // Stage 18: Psychoacoustic Width
        // ====================================================================
        float effectivePWidth = applySeqMod(pWidthAmt, MasterFXSequencer::Target::PsychoWidth, 0.6f);
        ...
        // ====================================================================
        float effectiveCompMix = applySeqMod(compMix, MasterFXSequencer::Target::CompMix, 0.5f);
        ...
        // ====================================================================
        {
            const bool mathActive =
                (mfx_ecMix > 0.001f || mfx_vsMix > 0.001f || mfx_qsMix > 0.001f || mfx_adMix > 0.001f);
            if (mathActive)
            {
                mathFX.processBlock(
                    L, R, numSamples,
        ...
        // ====================================================================
        {
            const bool boutiqueActive =
                (bfx_anMix > 0.001f || bfx_daMix > 0.001f || bfx_acMix > 0.001f || bfx_smMix > 0.001f);
            if (boutiqueActive)
            {
    ...
    void processBlock(juce::AudioBuffer<float>& buffer, int numSamples) { processBlock(buffer, numSamples, -1.0, 0.0); }
