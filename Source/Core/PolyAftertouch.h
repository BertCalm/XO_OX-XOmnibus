...
public:
    ...
    void updateBlock(int numSamples)
    {
        ...
        for (int v = 0; v < kMaxVoices; ++v)
        {
            ...
            // Smooth pressure
            float coeff = (target > smoothedPressure[static_cast<size_t>(v)]) ? smoothAttack : smoothRelease;

            // Block-rate approximation
            for (int i = 0; i < numSamples; ++i)
                smoothedPressure[static_cast<size_t>(v)] += coeff * (target - smoothedPressure[static_cast<size_t>(v)]);
    ...
    {
        return voiceMods[static_cast<size_t>(std::clamp(voiceIndex, 0, kMaxVoices - 1))];
    }

    float getSmoothedPressure(int voiceIndex) const
    {
        return smoothedPressure[static_cast<size_t>(std::clamp(voiceIndex, 0, kMaxVoices - 1))];
    }
