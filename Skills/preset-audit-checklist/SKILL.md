...
|--------|------|---------------|
| Sound Design Guide | `Docs/xomnibus_sound_design_guides.md` → engine section | Key parameters, macro mapping, sonic thesis |
...
| Existing Presets | `Presets/XOmnibus/` (grep for engine name) | Current library size, mood coverage |
...
- [ ] No duplicates across entire `Presets/XOmnibus/` tree
...
Or manually: no two presets share the same name across the entire `Presets/XOmnibus/` tree.
...
```
Read: Docs/xomnibus_sound_design_guides.md → {engine} section
Read: Source/Engines/{Engine}/{Engine}Engine.h → parameter list
```

### Step 2: Gather all presets
```
Glob: Presets/XOmnibus/**/*{engine}*.xometa
