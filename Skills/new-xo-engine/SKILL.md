...
**Purpose:** End-to-end guide for designing, scaffolding, and integrating a new engine into XOmnibus — from concept brief through gallery installation.
...
Use this skill when:
- Creating a brand-new engine for XOmnibus
- Scaffolding an engine concept into code
- Integrating a standalone XO instrument into the XOmnibus gallery
...
Every XOmnibus engine starts as a concept, gets designed, then built and integrated. This skill walks through all 5 phases:
...
Reference: `Docs/xomnibus_new_engine_process.md` for the full standalone→gallery dual-target workflow.
...
### 3.1 Register in XOmnibusProcessor.cpp
...
static bool registered_{Name} = xomnibus::EngineRegistry::instance().registerEngine(
    "{Name}", []() -> std::unique_ptr<xomnibus::SynthEngine> {
        return std::make_unique<xomnibus::{Name}Engine>();
    });
```
...
Add engine row to `Docs/xomnibus_master_specification.md` section 3.1.
...
```
[ ] Engine compiles in XOmnibus build
...
[ ] XOmnibusProcessor.cpp updated (include + registration)
```
